#!/usr/bin/env bun
/**
 * Soverign CLI Entry Point
 *
 * Usage:
 *   soverign start [--port N] [-d|--detach]   Start the daemon
 *   soverign stop [--port N]                  Stop the running daemon
 *   soverign status                           Show daemon status
 *   soverign uninstall                        Remove SOVERIGN (detects install method)
 *   soverign doctor                           Check environment & connectivity
 *   soverign version                          Print version
 *   soverign help                             Show this help
 *
 * First-time setup happens in the dashboard at http://localhost:3142
 * after `soverign start` — there is no longer a CLI wizard.
 */
...
function printHelp(): void {
  console.log(`
${c.cyan('Soverign')} ${c.dim(`v${getVersion()}`)}
Just A Rather Very Intelligent System

${c.bold('Usage:')}
  soverign <command> [options]

${c.bold('Commands:')}
  ${c.cyan('start')}     Start the SOVERIGN daemon
  ...
  ${c.cyan('update')}    Update SOVERIGN (dispatches based on install method)
  ${c.cyan('uninstall')} Remove SOVERIGN (dispatches based on install method)
...
${c.bold('Start options:')}
  ...
  --data-dir <path> Override data directory (default: ~/.soverign)
...
${c.bold('Examples:')}
  ...
  soverign start --port 8080      Start on custom port
  soverign restart                Restart with same settings
  soverign logs -f                Follow live log output
  soverign update                 Update to latest version
  soverign uninstall              Remove SOVERIGN from this machine
  soverign doctor                 Check if everything is working
`);
}

function assertSupportedPlatform(): void {
  if (process.platform !== 'win32') return;
  console.error(c.red('Native Windows installs are not supported for the SOVERIGN daemon.'));
  console.error(c.dim('Use WSL2 for the Bun install, or run SOVERIGN with Docker on Windows.'));
...
async function cmdStart(args: string[]): Promise<void> {
  ...
  const cfgPath = join(homedir(), '.soverign', 'config.yaml');
  if (!_exists(cfgPath)) {
  ...
  if (!detach) {
    ...
    if (!acquireLock(process.pid)) {
      console.log(c.yellow('SOVERIGN is already running'));
      console.log(c.dim('  Stop it first with: soverign stop'));
      process.exit(1);
    }
    ...
    if (existingPid) {
      console.log(c.yellow(`SOVERIGN is already running (PID ${existingPid})`));
      console.log(c.dim('  Stop it first with: soverign stop'));
      process.exit(1);
    }

    // Run in background — spawn a detached child process with log file
    console.log(c.cyan('Starting Soverign daemon...'));

    const logPath = getLogPath();
    const logFile = Bun.file(logPath);

    const daemonArgs = [join(PACKAGE_ROOT, 'bin/soverign.ts'), 'start', '--no-open'];
    ...
    if (runningPid) {
      console.log(c.green(`✓ SOVERIGN daemon started (PID ${runningPid})`));
      console.log(c.dim(`  Dashboard: http://localhost:${port ?? 3142}`));
      console.log(c.dim(`  Logs:      ${logPath}`));
      console.log(c.dim(`  Stop with: soverign stop`));
...
async function cmdStop(args: string[] = []): Promise<void> {
  ...
  if (!pid) {
    ...
    if (cleanup.terminated.length > 0 || cleanup.forced.length > 0) {
      ...
      console.log(c.green(`✓ SOVERIGN was not locked, but the port is now clear.${details}`));
    } else {
      console.log(c.yellow('SOVERIGN is not running.'));
    }
    return;
  }

  console.log(c.cyan(`Stopping SOVERIGN daemon (PID ${pid})...`));
  try {
    ...
    releaseLock();
    if (!cleanup.released) {
      console.error(c.red(`✗ SOVERIGN stopped but port ${port} is still occupied.`));
      process.exit(1);
    }
    ...
    const details = cleanup.forced.length > 0
      ...
      : cleanup.terminated.length > 0
        ? ` Cleaned up lingering listener(s) on port ${port}: ${cleanup.terminated.join(', ')}.`
        : '';
    console.log(c.green(`✓ SOVERIGN daemon stopped.${details}`));
  } catch (err) {
...
function cmdStatus(): void {
  const pid = isLocked();
  if (pid) {
    console.log(`${c.green('●')} SOVERIGN is ${c.green('running')} (PID ${pid})`);
    ...
    console.log(`${c.red('●')} SOVERIGN is ${c.red('stopped')}`);
    console.log(c.dim(`  Start with: soverign start`));
  }
}
...
function cmdLogs(args: string[]): void {
  ...
  if (!existsSync(logPath)) {
    console.log(c.yellow('No log file found. Start the daemon first: soverign start'));
    return;
  }
...
switch (command) {
  ...
  case 'onboard':
    ...
    console.log(c.dim('    1. Run: soverign start'));
  ...
  default:
    ...
    console.log(c.dim('Run "soverign help" for usage information.'));
    process.exit(1);
}
