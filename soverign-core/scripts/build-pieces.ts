...
import { buildAllSoverignPieces } from "../src/workflows/runner/engine-runtime/build-pieces";

const force = process.argv.includes("--force");

const start = Date.now();
const results = await buildAllSoverignPieces({ force });
const elapsed = Date.now() - start;

if (results.length === 0) {
  console.log("No Soverign pieces found under packages/pieces/soverign/.");
  process.exit(0);
}
