/**
 * Encrypted secrets store for SOVERIGN.
 *
 * Stores secrets in an AES-256-GCM encrypted file (~/.soverign/.secrets.enc)
 * with a random key stored in ~/.soverign/.secrets.key (chmod 600).
 *
...
const KEY_PATH = join(SOVERIGN_DIR, '.secrets.key');
const SECRETS_PATH = join(SOVERIGN_DIR, '.secrets.enc');
const ALGORITHM = 'aes-256-gcm';
const IV_LENGTH = 12;
const TAG_LENGTH = 16;

function ensureDir(): void {
  mkdirSync(SOVERIGN_DIR, { recursive: true, mode: 0o700 });
  try { chmodSync(SOVERIGN_DIR, 0o700); } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.warn(`[Keychain] Failed to chmod ${SOVERIGN_DIR} to 700: ${message}`);
  }
}
