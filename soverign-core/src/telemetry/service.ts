...
export type TelemetryServiceOptions = {
  ...
  sender?: Sender;
  /** Override debug logging (tests). Defaults to the SOVERIGN_TELEMETRY_DEBUG env. */
  debug?: boolean;
};
...
export class TelemetryService implements Service {
  ...
  constructor(private readonly opts: TelemetryServiceOptions) {
    ...
    this.debug = opts.debug ?? envFlag(process.env.SOVERIGN_TELEMETRY_DEBUG);
  }
    ...
    if (firstRun) {
      console.log(
        [
          ...
          '| SOVERIGN sends a small anonymous ping at startup and every hour     |',
          ...
          '| or run with SOVERIGN_TELEMETRY=0 (DO_NOT_TRACK=1 is honored too).    |',
          '+-------------------------------------------------------------------+',
          '',
        ].join('\n'),
      );
      ...
      console.log(
        '[Telemetry] Anonymous metrics on (opt out: telemetry.enabled=false or SOVERIGN_TELEMETRY=0).',
      );
    }
  }
}
