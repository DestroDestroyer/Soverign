...
export function resolveTelemetryEnabled(
  config: Pick<SoverignConfig, 'telemetry'>,
  ...
  if (env.SOVERIGN_TELEMETRY !== undefined) {
    if (isFalsy(env.SOVERIGN_TELEMETRY)) {
      return { enabled: false, reason: 'SOVERIGN_TELEMETRY opt-out' };
    }
    return { enabled: true, reason: 'SOVERIGN_TELEMETRY opt-in' };
  }
