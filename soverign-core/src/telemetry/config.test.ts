...
describe('resolveTelemetryEnabled', () => {
  ...
  test('SOVERIGN_TELEMETRY opt-out overrides an enabled config', () => {
    for (const v of ['0', 'false', 'no', 'off', '']) {
      expect(resolveTelemetryEnabled(ON, { SOVERIGN_TELEMETRY: v }).enabled).toBe(false);
    }
  });

  test('SOVERIGN_TELEMETRY opt-in overrides a disabled config', () => {
    expect(resolveTelemetryEnabled(OFF, { SOVERIGN_TELEMETRY: '1' }).enabled).toBe(true);
  });

  test('DO_NOT_TRACK takes precedence over everything', () => {
    const d = resolveTelemetryEnabled(ON, { DO_NOT_TRACK: '1', SOVERIGN_TELEMETRY: '1' });
    expect(d.enabled).toBe(false);
