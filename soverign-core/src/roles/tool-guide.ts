...
export function buildToolGuide(hasSidecars: boolean): string {
  ...
  lines.push('## Browser');
  lines.push('');
  lines.push('Control a Chrome browser for web research and interaction. Chrome auto-launches on first use. A persistent profile at ~/.soverign/browser/profile retains login sessions.');
  lines.push('');
  lines.push('Workflow:');
  ...
  lines.push('Create, run, and manage Soverign workflows (automations).');
