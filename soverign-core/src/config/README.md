# Soverign Configuration System

Type-safe YAML configuration management for Project Soverign
...
Default location: `~/.soverign/config.yaml`
...
// Load from default location (~/.soverign/config.yaml)
...
daemon: {
  ...
  data_dir: string;      // Data directory path (default: ~/.soverign)
  db_path: string;       // SQLite database path (default: ~/.soverign/soverign.db)
}
```
...
Core personality traits that guide Soverign behavior.
...
1. **Copy Example Config**:
   ```bash
   mkdir -p ~/.soverign
   cp config.example.yaml ~/.soverign/config.yaml
   ```

2. **Edit Configuration**:
   ```bash
   nano ~/.soverign/config.yaml
   ```
...
1. **Never commit API keys**: Add `~/.soverign/config.yaml` to `.gitignore`
...
```typescript
import type { SoverignConfig } from './config/index.ts';

function validateConfig(config: SoverignConfig): boolean {
