...
function makeConfig(overrides: Partial<SoverignConfig> = {}): SoverignConfig {
  return { ...structuredClone(DEFAULT_CONFIG), ...overrides };
}

function withOpenAIProvider(config: SoverignConfig, key: string): SoverignConfig {
...
describe('resolveRealtimeVoice', () => {
  ...
  test('does not fall back to env vars - key must come from a configured provider', () => {
    ...
    const config = makeConfig();
    config.voice!.realtime = { enabled: true };
    const prevSoverign = process.env.SOVERIGN_OPENAI_KEY;
    const prevOpenAI = process.env.OPENAI_API_KEY;
    process.env.SOVERIGN_OPENAI_KEY = 'env-key';
    ...
    try {
      ...
      if (prevSoverign === undefined) delete process.env.SOVERIGN_OPENAI_KEY; else process.env.SOVERIGN_OPENAI_KEY = prevSoverign;
