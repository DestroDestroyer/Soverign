...
export class GitHubManager {
  ...
  getToken(): string | null {
    return process.env.SOVERIGN_GITHUB_TOKEN ?? getSecret(TOKEN_KEY);
  }
  ...
  hasToken(): boolean {
    return !!process.env.SOVERIGN_GITHUB_TOKEN || hasSecret(TOKEN_KEY);
  }
  ...
  private async githubFetch(method: string, path: string, body?: unknown): Promise<Response> {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'SOVERIGN-SiteBuilder',
