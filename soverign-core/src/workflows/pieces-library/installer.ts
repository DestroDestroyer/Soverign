/**
 * Pieces installer + manifest. Manages `~/.soverign/pieces/`:
 *
...
const ROOT_ENV = "SOVERIGN_PIECES_DIR";

/** Default `~/.soverign/pieces`. Override with `SOVERIGN_PIECES_DIR` for tests. */
export function piecesBaseDir(): string {
  const override = process.env[ROOT_ENV];
  if (override) return resolve(override);
  return resolve(homedir(), ".soverign", "pieces");
}

export interface InstalledPiece {
  /** Soverign-side id; matches the catalog entry's `id`. */
  id: string;
