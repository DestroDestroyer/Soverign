...
This directory owns the list of activepieces community pieces that Soverign
users can install at runtime via the Library tab in the Workflows room.
The catalog is split into two tiers:

- **Verified** -- hand-reviewed by a Soverign maintainer + smoke-tested
...
The catalog is the *only* path by which pieces reach a Soverign install.
...
3. Daemon writes the requested piece into `~/.soverign/pieces/installed.json`,
   then synthesizes `~/.soverign/pieces/package.json` from the manifest and
   runs `bun install` in that directory.
4. Bun resolves `^0.12.2` against npm to a concrete version (e.g. 0.12.3)
   and places it under `~/.soverign/pieces/node_modules/@activepieces/piece-gmail/`.
...
Docker: `~/.soverign` should be mounted as a persistent volume. The manifest
...
```ts
{
  id: "gmail",                        // stable Soverign-side id; do not rename
  ...
  vettedVersion: "0.12.3",            // exact version Soverign verified
...
upstream patch fixes between Soverign releases.
...
3. **Native-deps check.** If transitive deps include native bindings,
   ...
   Anything found needs verification that it loads under the Soverign Bun
...
5. **Gated install integration test**:
   ...
   ```sh
   SOVERIGN_TEST_PIECES_LIBRARY=1 bun test src/workflows/pieces-library/integration.test.ts
   ```
...
- A future Soverign release widens it back once upstream fixes.
...
uninstall explicitly via the Library UI; future Soverign releases can ship a
