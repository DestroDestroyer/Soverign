 ...
 * `require()`d. Skipped unless `SOVERIGN_TEST_PIECES_LIBRARY=1` because it
 ...
 *     the engine-end-to-end suite for Soverign-native pieces.
 *
...
const ENABLED = process.env.SOVERIGN_TEST_PIECES_LIBRARY === "1";
...
beforeEach(() => {
  if (!ENABLED) return;
  tempDir = mkdtempSync(join(tmpdir(), "soverign-pieces-integration-"));
});
...
describe("pieces-library integration (gated on SOVERIGN_TEST_PIECES_LIBRARY=1)", () => {
    ...
    test.skipIf(!ENABLED)(
      ...
      async () => {
        ...
        writeFileSync(
          ...
          JSON.stringify({
            name: "soverign-pieces-integration",
            private: true,
            type: "commonjs",
