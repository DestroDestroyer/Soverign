 ...
 *   - No handler-side authn/authz: Soverign is single-tenant, the dashboard is
...
export interface CreateWorkflowRoutesOptions {
  ...
  /**
   * Optional piece catalog. Either the legacy `SoverignPieceRegistry` (during
   ...
   * report which `SoverignConnectionSource` adapters are registered (e.g.
   * `soverign:google` is wired) so the dashboard's piece-side auth picker
   * can highlight reusable Soverign-managed credentials. The repo-backed
   ...
   *
   * When omitted, install/uninstall still mutate `~/.soverign/pieces/` and the
 ...
 * race on the shared `~/.soverign/pieces/package.json` + bun-install
...
export function createWorkflowRoutes(opts: CreateWorkflowRoutesOptions = {}): WorkflowRouteMap {
  ...
  return {
        ...
        trapErrors(() => {
          ...
          const list = opts.pieceRegistry.list().map((p) => ({
            ...
            actions: Object.values(p.actions).map((a) => ({
              ...
              // Optional declared output sample (Soverign extension to AP).
            ...
            triggers: p.triggers
              ? Object.values(p.triggers).map((t) => ({
                  ...
                  // Dynamic-output triggers (soverign-trigger:on_event):
    ...
    // *curated* (Soverign-vetted) community piece the user can opt into
    ...
    //
    // Install / uninstall mutate `~/.soverign/pieces/` (manifest + bun
    ...
    // Event-buffer overflow signal for the `soverign-trigger:on_event`
    ...
    // ------------------------------------------------------------- connections
    // CRUD over `app_connection` rows + a list of registered Soverign
    ...
    "/api/workflows/connections": {
      GET: () =>
        trapErrors(() => {
          ...
          return ok({ connections: list, soverignSources: sources });
        }),
      POST: (req) =>
        trapErrors(async () => {
