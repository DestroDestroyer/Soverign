/**
 * Phase F end-to-end smoke: spawn the engine, load Soverign pieces from disk,
 ...
 * of the Soverign pieces is mechanical. Skipped when the engine bundle isn't on
...
import { buildAllSoverignPieces } from "./build-pieces";
...
const PIECE_TEST_NAME = "@soverignpieces/piece-soverign-test";
const PIECE_ASK_NAME = "@soverignpieces/piece-soverign-ask";
const PIECE_TOOL_NAME = "@soverignpieces/piece-soverign-tool";
const PIECE_NOTIFY_NAME = "@soverignpieces/piece-soverign-notify";
const PIECE_CONTEXT_NAME = "@soverignpieces/piece-soverign-context";
const PIECE_AGENT_NAME = "@soverignpieces/piece-soverign-agent";
const PIECE_TRIGGER_NAME = "@soverignpieces/piece-soverign-trigger";
...
describe("Engine end-to-end (F gate)", () => {
  ...
  test.skipIf(skipE2eTests)(
    "manual trigger + soverign-ask action calls daemon's /v1/soverign/llm/chat",
    async () => {
      llmCalls = [];
      const flow = createFlow({ projectId: DEFAULT_IDS.project });
      const trigger: FlowTriggerNode = {
...
describe("Engine end-to-end (G+H pieces)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (buildOptIn) await buildAllSoverignPieces();
    runtime = new EngineRuntime({ api, bundlePath: cached.bundlePath });
  });
