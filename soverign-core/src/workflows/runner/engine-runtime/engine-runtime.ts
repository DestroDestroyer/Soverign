...
import type { FlowVersion as SoverignFlowVersion } from "../../db/repos/flow-version";
...
export interface EngineRuntimeOptions {
   ...
   * `~/.soverign/workflow-codes`. The engine appends `${flowVersionId}/${stepName}/index.js`.
   */
  baseCodeDir?: string;
  /**
   ...
   * `packages/pieces/soverign/<name>/dist/package.json`.
   */
  cwd?: string;
  /** Search roots for vendored pieces. Default: vendored `packages/pieces` tree. */
  customPiecesPaths?: string[];
  /** CSV of dev-piece names exposed via AP_DEV_PIECES. Default: soverign pieces. */
  devPieces?: string[];
...
export interface ExecuteFlowOnHandleOptions {
  ...
  flowVersion: SoverignFlowVersion | UpstreamFlowVersion;
...
/**
 * Walk the vendored Soverign pieces directory and return each piece's
 * short alias (the part the engine's `getPieceNameFromAlias` strips
 * `piece-` from). Used as the default `devPieces` list so adding a
 * new Soverign piece doesn't require an engine-runtime edit.
 *
 ...
 * dir").
 */
function discoverSoverignDevPieces(): string[] {
  ...
  for (const name of readdirSync(root).sort()) {
    ...
    try {
      ...
      if (typeof pkg.name === "string" && pkg.name.length > 0) {
        ...
        // expects: `@soverignpieces/piece-soverign-foo` -> `soverign-foo`.
...
export class EngineRuntime {
  ...
  constructor(opts: EngineRuntimeOptions) {
    ...
    // packages/pieces/soverign/*/dist/package.json without further env setup.
    ...
    //
    // Auto-discover Soverign pieces from the vendored tree so adding a
    ...
    this.devPieces = opts.devPieces ?? discoverSoverignDevPieces();
  }
...
function isUpstreamFlowVersion(
  v: SoverignFlowVersion | UpstreamFlowVersion,
