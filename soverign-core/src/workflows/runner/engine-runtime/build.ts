 ...
 *
 * Staging lives outside the repo (under `~/.soverign/cache/engine-build`) so
...
const CACHE_ROOT = resolve(homedir(), ".soverign/cache");
 ...
 * from SOVERIGN (e.g. axios in pieces/common feeds the HTTP node, which webhook
...
function buildStagingPackageJson(): string {
  ...
  return JSON.stringify(
    {
      name: "soverign-engine-build-staging",
      private: true,
      type: "commonjs",
      dependencies: deps,
...
const PATCHED_VENDOR_SOURCES = [
  ...
  // Soverign-only `outputSample` extension on actions + the matching
  ...
  // Soverign-only BranchOperator additions (TEXT_MATCHES_REGEX +
  ...
  // Soverign: upstream sets process.env.NODE_TLS_REJECT_UNAUTHORIZED='0'
