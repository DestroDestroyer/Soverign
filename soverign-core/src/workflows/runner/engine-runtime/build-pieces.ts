/**
 * Bundle each Soverign-authored piece (under `packages/pieces/soverign/*`) into
...
async function ensureStagingInstalled(): Promise<void> {
  ...
  // `buildAllSoverignPieces()` in parallel, and if pieces won the race
...
/**
 * Build every piece directly under `packages/pieces/soverign/`. Returns the
 ...
 */
export async function buildAllSoverignPieces(
  opts: BuildPieceOptions = {},
): Promise<BuildPieceResult[]> {
...
export const PIECE_BUILD_PATHS = {
  soverignPiecesRoot: resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/soverign"),
} as const;
