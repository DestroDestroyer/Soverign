 ...
 *         direct-subscribe path for `soverign-trigger:on_event` (kept until
...
export interface TriggerManagerDeps {
  /**
   * In-process event bus. Used by the legacy `soverign-trigger:on_event`
   ...
   * supported is `soverign-trigger:on_event`, which falls back to direct
...
export class TriggerManager {
  ...
  private async register(flow: FlowRow): Promise<void> {
    ...
    if (trigger.type === "PIECE_TRIGGER") {
      ...
      // Engine-less fallback for soverign-trigger:on_event. In
      ...
      // `manager.test.ts:"TriggerManager: soverign-trigger on_event"`.
      ...
      // engine.
      if (pieceName === "soverign-trigger") {
        return this.registerSoverignEvent(flow.id, versionId, trigger);
      }
      this.log(
  ...
  private registerSoverignEvent(flowId: string, versionId: string, trigger: TriggerNode): void {
    if (trigger.settings?.triggerName !== "on_event") {
      this.log(
        `flow ${flowId}: soverign-trigger has triggerName="${trigger.settings?.triggerName}"; only "on_event" is supported`,
      );
      return;
    }
    const input = (trigger.settings?.input ?? {}) as Record<string, unknown>;
  ...
  /**
   * Engine-managed trigger fire (polling sources, e.g. soverign-trigger
