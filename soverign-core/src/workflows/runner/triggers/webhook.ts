 ...
 * against an optional HMAC-SHA256 secret (X-Soverign-Signature header).
 */
...
export class WebhookManager {
  ...
  async handleRequest(workflowId: string, req: Request): Promise<Response> {
    ...
    if (route.secret) {
      const signature = req.headers.get('x-soverign-signature') ?? req.headers.get('X-Soverign-Signature');
