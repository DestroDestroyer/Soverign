/**
 * `ask` action -- POST to the daemon's `/v1/soverign/llm/chat` endpoint with
...
export const askAction = createAction({
  name: "ask",
  displayName: "Ask",
  description: "Send a prompt to Soverign's LLM and receive the reply.",
  ...
  props: {
    ...
    system: Property.LongText({
      displayName: "System",
      description:
        "Extra system instructions APPENDED to the standard Soverign system prompt. Use this to bias the reply (e.g. \"answer in JSON\"). Turn on Override to replace the Soverign prompt entirely instead.",
      required: false,
    }),
    overrideSystem: Property.Checkbox({
      displayName: "Override Soverign system prompt",
      description:
        "When ON, the System field above becomes the ONLY system prompt -- Soverign's identity, role, personality, and vault context are not sent. Use for generic LLM tasks (text transforms, summarisation of plain inputs) where Soverign context would bias the reply. Leave OFF (default) when you want the model to answer as Soverign.",
      required: false,
      defaultValue: false,
    }),
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/soverign/llm/chat";
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `soverign-ask: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }
