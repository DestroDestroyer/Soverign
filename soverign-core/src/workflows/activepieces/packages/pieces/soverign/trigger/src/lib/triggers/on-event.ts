/**
 * `on_event` polling trigger -- fires the workflow when a Soverign event of the
 ...
 *
 * Why not sub-minute: Soverign's own CronScheduler supports an `@every <n>(s|m|h)`
 * extension, BUT the engine validates a trigger's cron with `cron-validator`'s
 * `isValidCron` inside `setSchedule` BEFORE Soverign's scheduler ever sees it --
...
const CURSOR_KEY = "soverign-trigger:on-event:since";
...
async function poll(
  ...
  const url = trimSlash(apiUrl) + "/v1/soverign/events/poll";
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `soverign-trigger.on_event: daemon responded ${response.status}: ${text.slice(0, 500)}`,
    );
  }
...
export const onEventTrigger = createTrigger({
  name: "on_event",
  displayName: "On Soverign event",
  description:
    "Fire the workflow when a Soverign event of the given type is published. Use the daemon's event-type catalog (awareness.*, commitment.*, voice.*, tool.*) to pick a value.",
  type: TriggerStrategy.POLLING,
  props: {
    eventType: Property.ShortText({
      displayName: "Event type",
      description: "Fully-qualified Soverign event type (e.g. awareness.context_changed).",
      required: true,
    }),
