/**
 * `SoverignTelegramConnectionSource` -- bridges Soverign's existing Telegram
 * bot token (configured in `~/.soverign/config.yaml` under
 * `channels.telegram.bot_token`) into the workflow runtime's
 * `CredentialResolver`. When a piece asks for `soverign:telegram`, the source
 * returns the same token Soverign is using for the inbound bot.
 *
...
export const SOVERIGN_TELEGRAM_PREFIX = "soverign:telegram";

export class SoverignTelegramConnectionSource implements SoverignConnectionSource {
  ...
  canResolve(externalId: string): boolean {
    return externalId === SOVERIGN_TELEGRAM_PREFIX || externalId.startsWith(`${SOVERIGN_TELEGRAM_PREFIX}:`);
  }
