/**
 * Coverage for `SoverignTelegramConnectionSource`. Mirrors the shape of the
 * google-source tests: canResolve predicate + resolve happy/null paths.
 */

import { describe, expect, test } from "bun:test";
import { SoverignTelegramConnectionSource, SOVERIGN_TELEGRAM_PREFIX } from "./telegram-source";

describe("SoverignTelegramConnectionSource", () => {
  test("canResolve matches soverign:telegram and any soverign:telegram:<sub>", () => {
    const src = new SoverignTelegramConnectionSource(() => "abc");
    expect(src.canResolve(SOVERIGN_TELEGRAM_PREFIX)).toBe(true);
    expect(src.canResolve(`${SOVERIGN_TELEGRAM_PREFIX}:bot_a`)).toBe(true);
    expect(src.canResolve("soverign:google")).toBe(false);
    expect(src.canResolve("custom-id")).toBe(false);
  });

  test("resolve returns the token wrapped as SECRET_TEXT when configured", async () => {
    const src = new SoverignTelegramConnectionSource(() => "12345:abcdef");
    const out = await src.resolve(SOVERIGN_TELEGRAM_PREFIX);
    expect(out).toEqual({
      type: "SECRET_TEXT",
  ...
  test("resolve returns null when no token is configured", async () => {
    const src = new SoverignTelegramConnectionSource(() => null);
    expect(await src.resolve(SOVERIGN_TELEGRAM_PREFIX)).toBeNull();
  });

  test("resolve returns null when the token is empty string", async () => {
    const src = new SoverignTelegramConnectionSource(() => "");
    expect(await src.resolve(SOVERIGN_TELEGRAM_PREFIX)).toBeNull();
  });

  test("token closure reads live (rotation is picked up without rebuilding the source)", async () => {
    let current: string | null = "old-token";
    const src = new SoverignTelegramConnectionSource(() => current);
    const first = await src.resolve(SOVERIGN_TELEGRAM_PREFIX);
    expect((first?.value as { secret_text?: string } | undefined)?.secret_text).toBe("old-token");
    current = "rotated-token";
    const second = await src.resolve(SOVERIGN_TELEGRAM_PREFIX);
  ...
  test("piping through the engine's V0 unwrap yields the raw token string", async () => {
    const src = new SoverignTelegramConnectionSource(() => "live-bot-token");
    const resolved = await src.resolve(SOVERIGN_TELEGRAM_PREFIX);
