 ...
 *
 *   1. If the externalId starts with `soverign:`, dispatch to a registered
 *      `SoverignConnectionSource` that pulls live from Soverign' existing stores
 *      (Google OAuth file, Telegram/Discord bot token in config, etc.). This
 *      means a user who already wired Gmail into Soverign sees a "Soverign Gmail"
 ...
 *
 * The adapter does not import Soverign modules directly; sources are injected so
...
/** Prefix that marks a connection as "managed by Soverign itself". */
export const SOVERIGN_PREFIX = "soverign:";
...
/**
 * A source of Soverign-managed credentials. One per backing store (Google file,
 * channel config, etc.). Sources only handle externalIds with the SOVERIGN_PREFIX.
 */
export interface SoverignConnectionSource {
...
/** Composite resolver: Soverign sources first, then the workflow DB. */
export class CredentialResolver {
  private sources: SoverignConnectionSource[] = [];

  register(source: SoverignConnectionSource): void {
  ...
  list(): readonly SoverignConnectionSource[] {
    return this.sources;
  }

  async resolve(input: ResolveInput): Promise<ResolvedConnection | null> {
    if (input.externalId.startsWith(SOVERIGN_PREFIX)) {
...
// Live Soverign credential sources -- one per backing store -- live in their
