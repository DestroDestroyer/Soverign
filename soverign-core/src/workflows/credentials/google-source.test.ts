/**
 * Coverage for `SoverignGoogleConnectionSource`. The refresh-token dance
...
import { SoverignGoogleConnectionSource, SOVERIGN_GOOGLE_PREFIX } from "./google-source";
...
describe("SoverignGoogleConnectionSource", () => {
  test("returns null when GoogleAuth is unauthenticated", async () => {
    ...
    const src = new SoverignGoogleConnectionSource(auth);
    expect(await src.resolve(SOVERIGN_GOOGLE_PREFIX)).toBeNull();
  });

  test("canResolve matches soverign:google + soverign:google:<sub>", () => {
    const src = new SoverignGoogleConnectionSource(makeFakeAuth({ authed: false, freshAccessToken: "x", tokens: null }));
    expect(src.canResolve("soverign:google")).toBe(true);
    expect(src.canResolve("soverign:google:gmail")).toBe(true);
    expect(src.canResolve("soverign:telegram")).toBe(false);
  });
  ...
  test("calls getAccessToken so an expired cache surfaces the refreshed token", async () => {
    ...
    const src = new SoverignGoogleConnectionSource(makeFakeAuth(state));
    const resolved = await src.resolve(SOVERIGN_GOOGLE_PREFIX);
