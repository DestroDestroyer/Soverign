 ...
 *   1. `soverign:*` external ids -> live Soverign-managed credential sources
 ...
 *
 * Per CredentialResolver contract for Soverign-managed Google connections, the
...
export function createConnectionsRoute(deps: ConnectionsRouteDeps): RouteHandler {
  return async (ctx: RouteContext) => {
    ...
    // needs the pieceName for app_connection lookups. For soverign:* external
