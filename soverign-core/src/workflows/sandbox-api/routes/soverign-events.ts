/**
 * `/v1/soverign/events/poll` -- backs the `soverign-trigger` `on_event` polling
 * trigger.
 *
 * Two request shapes:
 *
...
export interface SoverignEvent {
  id: string;
  eventType: string;
...
export interface SoverignEventsRouteDeps {
  eventsPoll?: EventsPollFn;
}

export function createSoverignEventsPollRoute(
  deps: SoverignEventsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.eventsPoll) {
      return err("soverign events.poll not configured", 503);
    }
