/**
 * `/v1/soverign/context/*` -- backs the `soverign-context` piece's four read-only
 * actions. One route per action so each can validate its own envelope; all
 * four delegate to a single injected `SoverignContextProvider` shaped exactly
 * like the legacy adapter, so the daemon can supply a `SoverignContextProviderAdapter`
 * verbatim.
 *
 * Routes:
 *   POST /v1/soverign/context/vault-search
 *   POST /v1/soverign/context/vault-get-entity
 *   POST /v1/soverign/context/awareness-recent
 *   POST /v1/soverign/context/commitments-list
 */
...
export interface SoverignContextProvider {
...
export interface SoverignContextRouteDeps {
...
export function createSoverignContextVaultSearchRoute(
  deps: SoverignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("soverign context not configured", 503);
...
export function createSoverignContextVaultGetEntityRoute(
  deps: SoverignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("soverign context not configured", 503);
...
export function createSoverignContextAwarenessRecentRoute(
  deps: SoverignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("soverign context not configured", 503);
...
export function createSoverignContextCommitmentsListRoute(
  deps: SoverignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("soverign context not configured", 503);
