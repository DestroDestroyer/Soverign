/**
 * `/v1/soverign/agent/delegate` -- backs the `soverign-agent` piece's `delegate`
...
export interface SoverignAgentRouteDeps {
  agentDelegate?: AgentDelegateFn;
}

export function createSoverignAgentDelegateRoute(
  deps: SoverignAgentRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.agentDelegate) {
      return err("soverign agent.delegate not configured", 503);
    }
