/**
 * `/v1/soverign/workflows/start` -- backs the `soverign-trigger` piece's
...
export interface SoverignWorkflowsRouteDeps {
  workflowsStart?: WorkflowsStartFn;
}

export function createSoverignWorkflowsStartRoute(
  deps: SoverignWorkflowsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.workflowsStart) {
      return err("soverign workflows.start not configured", 503);
    }
    ...
    try {
      ...
      // Typed errors from SoverignWorkflowRunnerAdapter carry a `code`
