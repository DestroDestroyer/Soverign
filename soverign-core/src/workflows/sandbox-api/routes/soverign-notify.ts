/**
 * `/v1/soverign/notify` -- backs the `soverign-notify` piece's `notify` action.
 *
...
export interface SoverignNotifyRouteDeps {
  notify?: NotifyFn;
}

export function createSoverignNotifyRoute(
  deps: SoverignNotifyRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.notify) {
      return err("soverign notify not configured", 503);
    }
