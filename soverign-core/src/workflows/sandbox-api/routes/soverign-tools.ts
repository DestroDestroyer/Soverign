/**
 * `/v1/soverign/tools/invoke` -- backs the `soverign-tool` piece's `invoke` action.
 *
...
export interface SoverignToolsRouteDeps {
  toolsInvoke?: ToolsInvokeFn;
}

export function createSoverignToolsInvokeRoute(
  deps: SoverignToolsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.toolsInvoke) {
      return err("soverign tools.invoke not configured", 503);
    }
