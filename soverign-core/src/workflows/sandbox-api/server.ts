 ...
 *
 * Plus a parallel set of `/v1/soverign/*` endpoints for our own ported pieces
 * (added in F-H).
 *
...
import {
  createSoverignLlmChatRoute,
  type LlmChatFn,
} from "./routes/soverign-llm";
import {
  createSoverignToolsInvokeRoute,
  type ToolsInvokeFn,
} from "./routes/soverign-tools";
import {
  createSoverignNotifyRoute,
  type NotifyFn,
} from "./routes/soverign-notify";
import {
  createSoverignContextVaultSearchRoute,
  createSoverignContextVaultGetEntityRoute,
  createSoverignContextAwarenessRecentRoute,
  createSoverignContextCommitmentsListRoute,
  type SoverignContextProvider,
} from "./routes/soverign-context";
import {
  createSoverignAgentDelegateRoute,
  type AgentDelegateFn,
} from "./routes/soverign-agent";
import {
  createSoverignEventsPollRoute,
  type EventsPollFn,
} from "./routes/soverign-events";
import {
  createSoverignWorkflowsStartRoute,
  type WorkflowsStartFn,
} from "./routes/soverign-workflows";
...
export interface SandboxApiServices {
  ...
  /**
   * LLM chat backend for the `soverign-ask` piece. If unset, the endpoint
   * returns 503; the daemon wires this in at startup with the LLMManager.
   */
  llmChat?: LlmChatFn;
  /**
   * Tool invocation backend for the `soverign-tool` piece. If unset, the
   * endpoint returns 503. The daemon wires this in with `ToolRegistry`.
   */
  toolsInvoke?: ToolsInvokeFn;
  /**
   * Notification backend for the `soverign-notify` piece. If unset, returns 503.
   */
  notify?: NotifyFn;
  /**
   * Read-only Soverign state provider for the `soverign-context` piece. If unset,
   * each context route returns 503.
   */
  contextProvider?: SoverignContextProvider;
  /**
   * Sub-agent delegation backend for the `soverign-agent` piece. If unset, the
   * endpoint returns 503.
   */
  agentDelegate?: AgentDelegateFn;
  /**
   * Recent-events poll backend for the `soverign-trigger` `on_event` trigger.
   * If unset, returns 503.
   */
  eventsPoll?: EventsPollFn;
  /**
   * Workflow start backend for the `soverign-trigger` `run_workflow` action.
   * If unset, returns 503.
   */
...
export class SandboxApi {
  ...
  /**
   * Patch service backends after construction. Each `/v1/soverign/*` route's
  ...
  private registerRoutes(): void {
    this.routes.push(
      ...
      // /v1/soverign/* routes pass `this.services` itself (not a snapshot) so
      ...
      {
        path: "/v1/soverign/llm/chat",
        method: "POST",
        handler: createSoverignLlmChatRoute(this.services),
      },
      {
        path: "/v1/soverign/tools/invoke",
        method: "POST",
        handler: createSoverignToolsInvokeRoute(this.services),
      },
      {
        path: "/v1/soverign/notify",
        method: "POST",
        handler: createSoverignNotifyRoute(this.services),
      },
      {
        path: "/v1/soverign/context/vault-search",
        method: "POST",
        handler: createSoverignContextVaultSearchRoute(this.services),
      },
      {
        path: "/v1/soverign/context/vault-get-entity",
        method: "POST",
        handler: createSoverignContextVaultGetEntityRoute(this.services),
      },
      {
        path: "/v1/soverign/context/awareness-recent",
        method: "POST",
        handler: createSoverignContextAwarenessRecentRoute(this.services),
      },
      {
        path: "/v1/soverign/context/commitments-list",
        method: "POST",
        handler: createSoverignContextCommitmentsListRoute(this.services),
      },
      {
        path: "/v1/soverign/agent/delegate",
        method: "POST",
        handler: createSoverignAgentDelegateRoute(this.services),
      },
      {
        path: "/v1/soverign/events/poll",
        method: "POST",
        handler: createSoverignEventsPollRoute(this.services),
      },
      {
        path: "/v1/soverign/workflows/start",
        method: "POST",
        handler: createSoverignWorkflowsStartRoute(this.services),
      },
    );
  }
