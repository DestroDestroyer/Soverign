/**
 * Adapter: PieceNotifier over Soverign' ChannelService + WebSocketService +
 ...
 *                  dropped here so a vanilla Soverign without telegram doesn't
...
export interface NotifierDeps {
  ...
  /**
   * Optional voice / TTS surface. When provided, `soverign-notify` calls with
   ...
   * connected (e.g. `{"telegram"}` on a Soverign with only telegram wired).
...
export class SoverignNotifierAdapter implements PieceNotifier {
...
function expandChannels(requested: PieceNotifyChannel[], deps: NotifierDeps): Set<string> {
  ...
  for (const c of requested) {
    if (c === "auto") {
      ...
      // Soverign without telegram/discord would report two failures the
      // user can't act on.
      out.add("dashboard");
...
function titleForPriority(p: "urgent" | "normal" | "low"): string {
  if (p === "urgent") return "Soverign (urgent)";
  return "Soverign";
}
