/**
 * `M7AgentDelegator` -- backs `soverign-agent.delegate` with the full M7 sub-agent
 ...
 * action should expose it via a separate piece (e.g., `soverign-tool` or
 * `soverign-notify` + waitpoint), not piggy-back on the sub-agent.
 */
