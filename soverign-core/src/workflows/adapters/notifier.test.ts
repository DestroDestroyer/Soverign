/**
 * `SoverignNotifierAdapter` covers the four channels the workflow notifier
...
import { SoverignNotifierAdapter, type NotifierDeps } from "./notifier";
...
describe("SoverignNotifierAdapter: voice channel", () => {
  test("delivers via sendVoice when wired", async () => {
    const spoken: string[] = [];
    const adapter = new SoverignNotifierAdapter(
      makeDeps({
        sendVoice: async (t) => {
          spoken.push(t);
        },
  ...
  test("reports failed when sendVoice isn't wired", async () => {
    const adapter = new SoverignNotifierAdapter(makeDeps());
...
describe("SoverignNotifierAdapter: channel expansion", () => {
  ...
  test('"auto" only fans out to *connected* external channels when the dep is wired', async () => {
    // Soverign with only telegram wired -> auto goes to dashboard + telegram,
  ...
  test('"auto" on a Soverign with no external channels connected only delivers to dashboard', async () => {
    let m8Called = false;
    const adapter = new SoverignNotifierAdapter(
      makeDeps({
