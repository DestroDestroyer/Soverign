/**
 * `PieceCatalog` -- canonical view of installed pieces (Soverign-native + future
 ...
 * `SoverignPieceRegistry` (`{name, displayName, description, actions, triggers}`
...
/**
 * Piece-name + trigger-name pair for soverign-trigger:on_event. Hoisted so
 * the dynamic-sample injector and the composer prompt agree on the key.
 */
export const SOVERIGN_ON_EVENT_TRIGGER = {
  piece: "@soverignpieces/piece-soverign-trigger",
  trigger: "on_event",
} as const;
...
export interface PieceCatalogAction {
   ...
   * `createAction({ outputSample: ... })` (Soverign extension to upstream AP).
...
export interface PieceCatalogTrigger extends PieceCatalogAction {
   ...
   * soverign-trigger `on_event` is the canonical case (the `payload`
 ...
 * soverign-ask, schedule) leave it undefined and the editor renders no
...
export interface PieceCatalogEntry {
  /** Upstream package name -- e.g. `@soverignpieces/piece-soverign-ask`. */
  name: string;
  displayName: string;
  description: string;
   ...
   * (soverign-ask, schedule, webhook, code).
   */
  auth?: PieceCatalogAuth;
}

/**
 * Structural interface that both `PieceCatalog` and `SoverignPieceRegistry`
 ...
 *         resolution for soverign-trigger:on_event).
 *   v4 -- promoted soverign-trigger:on_event's `eventType` input field
 ...
 *         dropdown).
 *   v6 -- promoted soverign-trigger:run_workflow's `flow` input from
...
export function computeCatalogCacheKey(opts: {
  ...
  // soverign-trigger:on_event projection synthesizes its
...
/** Convert an upstream `PieceMetadata` blob into the legacy `SoverignPiece`-style entry. */
export function metadataToCatalogEntry(meta: RawPieceMetadata | unknown): PieceCatalogEntry {
  ...
  // Apply any Soverign-specific catalog enrichments after the generic
  // projection finishes. Today this covers two upgrades on the
  // soverign-trigger piece (on_event eventType -> enum, run_workflow
  // flow -> flow_ref). Centralising the gating keeps the generic
  // projection above piece-agnostic and gives the next "Soverign tweak"
  // a single named home.
  enrichSoverignCatalogEntry(out);
...
/**
 * Apply Soverign-specific catalog enrichments AFTER the generic
 ...
 * trigger, so an upstream rename (or a Soverign-piece source edit that
 * drops a sub-action) silently no-ops rather than crashing the catalog
 * build.
 *
 * The set of enrichments here is small and well-known:
 *   - soverign-trigger:on_event   -- attach `dynamicSampleData` and
 *                                  promote `eventType` to enum.
 *   - soverign-trigger:run_workflow -- promote `flow` to flow_ref.
 *
 ...
 */
export function enrichSoverignCatalogEntry(entry: PieceCatalogEntry): void {
  if (entry.name !== SOVERIGN_ON_EVENT_TRIGGER.piece) return;
  const onEvent = entry.triggers?.[SOVERIGN_ON_EVENT_TRIGGER.trigger];
  if (onEvent) {
...
/**
 * Build the `dynamicSampleData` map for soverign-trigger:on_event. Each
...
/**
 * Rewrite the `flow` input field on soverign-trigger's `run_workflow`
...
function rawActionToCatalogAction(
  ...
  // (Soverign extension); triggers use upstream `sampleData`. Both can be
