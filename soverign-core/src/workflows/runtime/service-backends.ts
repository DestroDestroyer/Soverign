/**
 * Glue layer: wraps existing Soverign adapters into the function-shape that the
 * SandboxApi service-backend slots expect. Each `/v1/soverign/*` route takes a
...
import { SoverignLlmClient } from "../adapters/llm-client";
import { SoverignToolRegistryAdapter } from "../adapters/tool-registry";
import { SoverignNotifierAdapter, type NotifierDeps } from "../adapters/notifier";
import { SoverignContextProviderAdapter } from "../adapters/context-provider";
import { LlmOnlyAgentDelegator } from "../adapters/agent-delegator";
import { M7AgentDelegator } from "../adapters/m7-agent-delegator";
import { SoverignWorkflowRunnerAdapter } from "../adapters/workflow-runner";
import type { LlmChatFn } from "../sandbox-api/routes/soverign-llm";
import type { ToolsInvokeFn } from "../sandbox-api/routes/soverign-tools";
import type { NotifyFn } from "../sandbox-api/routes/soverign-notify";
import type { SoverignContextProvider } from "../sandbox-api/routes/soverign-context";
import type { AgentDelegateFn } from "../sandbox-api/routes/soverign-agent";
import type { EventsPollFn } from "../sandbox-api/routes/soverign-events";
import type { WorkflowsStartFn } from "../sandbox-api/routes/soverign-workflows";
...
export interface BuildServiceBackendsOptions {
  ...
  /** Recent-events buffer for `soverign-trigger:on_event` polling. */
  ...
  /**
   * M7 sub-agent dependencies. When all of these are supplied, `soverign-agent.delegate`
  ...
  /**
   * Optional callback that builds the Soverign-flavoured system prompt for
   * a workflow LLM call. When set, the `soverign-ask` piece will pass this
   * prompt to the LLM so the model knows it's Soverign (role, personality,
   ...
   */
  buildSoverignSystemPrompt?: (userMessage: string) => string;
}

export function buildSandboxServiceBackends(
  opts: BuildServiceBackendsOptions,
): SandboxApiServices {
  const llmClient = new SoverignLlmClient(opts.llmManager);
  const llmChat: LlmChatFn = async (req) => {
    // System-prompt composition:
    //   - overrideSystem=true       : use `req.system` only. Soverign
    ...
    //                                 by Soverign's identity).
    //   - `req.system` set, default : Soverign prompt + "\n\n" + req.system.
    //                                 Lets the user steer the reply (e.g.
    //                                 "respond in JSON") while keeping the
    //                                 Soverign identity.
    //   - no `req.system`           : Soverign prompt alone. Default for
    //                                 plain "ask Soverign" steps.
    ...
    const soverignSystem = opts.buildSoverignSystemPrompt
      ? opts.buildSoverignSystemPrompt(req.prompt)
      : undefined;
    let system: string | undefined;
    if (req.overrideSystem) {
      system = req.system;
    } else if (req.system && soverignSystem) {
      system = `${soverignSystem}\n\n${req.system}`;
    } else {
      system = req.system ?? soverignSystem;
    }
  ...
  const toolAdapter = opts.toolRegistry
    ? new SoverignToolRegistryAdapter(opts.toolRegistry)
    : null;
  ...
  const notifierAdapter = new SoverignNotifierAdapter(notifierDeps);
  ...
  const contextAdapter = new SoverignContextProviderAdapter();
  const contextProvider: SoverignContextProvider = {
    vaultSearch: (input) =>
  ...
  const eventsPoll: EventsPollFn = async (req) => {
    ...
    // The route's `SoverignEvent` types `id` as a string (consistent with all
  ...
  const runnerAdapter = new SoverignWorkflowRunnerAdapter();
