 ...
 *   - Engine-level (gated on SOVERIGN_TEST_ENGINE_BUILD=1): spawn the real
 *     engine, run EXTRACT_PIECE_METADATA against the seven vendored Soverign
...
import { SoverignPieceRegistry } from "../soverign-pieces/types";
...
import { buildAllSoverignPieces } from "../runner/engine-runtime/build-pieces";
...
describe("PieceCatalog (unit)", () => {
  test("PieceCatalog.list/get round-trips initial entries", () => {
    ...
    expect(catalog.get("@soverignpieces/piece-soverign-test")?.displayName).toBe("Soverign: Test");
  ...
  test("metadataToCatalogEntry maps actions + triggers verbatim", () => {
    ...
    expect(entry.name).toBe("@soverignpieces/piece-x");
  ...
  test("metadataToCatalogEntry attaches dynamicSampleData to soverign-trigger:on_event", () => {
    // soverign-trigger's on_event surfaces an envelope whose payload shape
  ...
  test("SoverignPieceRegistry structurally satisfies the PieceLookup interface", () => {
    // Compile-time + runtime check: assigning a SoverignPieceRegistry instance
    ...
    const reg = new SoverignPieceRegistry();
    reg.register({
      name: "p",
      displayName: "P",
      description: "p",
...
const buildOptIn = process.env.SOVERIGN_TEST_ENGINE_BUILD === "1";
...
const piecesAlreadyBuilt = existsSync(
  resolve(
    ...
    "pieces/soverign/test/dist/src/index.js",
  ),
);
...
describe("PieceCatalog (engine end-to-end)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (!piecesAlreadyBuilt && buildOptIn) await buildAllSoverignPieces();
  ...
  test.skipIf(skipE2eTests)(
    "extracts metadata for every Soverign-native piece via the real engine",
    async () => {
      const root = resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/soverign");
      ...
      try {
        ...
        expect(names).toContain("@soverignpieces/piece-soverign-ask");
        expect(names).toContain("@soverignpieces/piece-soverign-tool");
        expect(names).toContain("@soverignpieces/piece-soverign-notify");
        expect(names).toContain("@soverignpieces/piece-soverign-context");
        expect(names).toContain("@soverignpieces/piece-soverign-agent");
        expect(names).toContain("@soverignpieces/piece-soverign-trigger");
        expect(names).toContain("@soverignpieces/piece-soverign-test");
        // ask: single action `ask` with at least the `prompt` field.
        const ask = catalog.get("@soverignpieces/piece-soverign-ask");
        ...
        const trigger = catalog.get("@soverignpieces/piece-soverign-trigger");
