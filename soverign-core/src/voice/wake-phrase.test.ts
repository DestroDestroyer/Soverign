...
describe('containsWakePhrase', () => {
  test('matches the bare wake phrase', () => {
    expect(containsWakePhrase('soverign')).toBe(true);
    expect(containsWakePhrase('Soverign')).toBe(true);
    expect(containsWakePhrase('SOVERIGN')).toBe(true);
  });

  test('matches when the wake phrase appears mid-sentence', () => {
    expect(containsWakePhrase('Hey Soverign, how are you')).toBe(true);
    expect(containsWakePhrase('Tell Soverign to send the email')).toBe(true);
    expect(containsWakePhrase('I told soverign already')).toBe(true);
  });

  test('respects word boundaries (does not match substrings)', () => {
    expect(containsWakePhrase('soverignson')).toBe(false);
    expect(containsWakePhrase('starsoverign')).toBe(false);
    expect(containsWakePhrase('antisoverignt')).toBe(false);
  });

  test('treats punctuation as a word boundary', () => {
    expect(containsWakePhrase('Hello, Soverign.')).toBe(true);
    expect(containsWakePhrase('"Soverign!"')).toBe(true);
    expect(containsWakePhrase('(soverign)')).toBe(true);
    expect(containsWakePhrase('Soverign?')).toBe(true);
  });
