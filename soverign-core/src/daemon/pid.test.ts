...
const READY_SIGNAL = join(tmpdir(), 'soverign-test-lock-ready');
const HOLDER_SCRIPT = join(tmpdir(), 'soverign-test-lock-holder.ts');
...
describe('Process Lock Manager', () => {
  ...
  describe('acquireLock', () => {
    ...
    test('creates ~/.soverign dir if missing', () => {
  ...
  describe('path getters', () => {
    test('getPidPath returns ~/.soverign/soverign.pid', () => {
      expect(getPidPath()).toBe(join(homedir(), '.soverign', 'soverign.pid'));
    });

    test('getLogPath returns path and creates logs dir', () => {
      const logPath = getLogPath();
      expect(logPath).toBe(join(SOVERIGN_DIR, 'logs', 'soverign.log'));
      expect(existsSync(join(SOVERIGN_DIR, 'logs'))).toBe(true);
    });

    test('getLogDir returns logs directory', () => {
      expect(getLogDir()).toBe(join(SOVERIGN_DIR, 'logs'));
    });
  });
