...
Core daemon infrastructure for the SOVERIGN system.
...
```bash
# Run with defaults (port 3142, ~/.soverign data directory)
bun run src/daemon/index.ts

# Run with custom configuration
bun run src/daemon/index.ts --port 3142 --data-dir ~/my-soverign-data
...
| `--data-dir` | `~/.soverign` | Data directory |
| `--db-path` | `~/.soverign/soverign.db` | Database file path |
...
```
[2026-02-22T22:17:03.230Z] Initializing database at /home/user/.soverign/soverign.db
...
- **Location**: `~/.soverign/soverign.db` (configurable)

Schema is managed by `~/soverign/src/vault/schema.ts`.
...
timeout 3 bun run src/daemon/index.ts --data-dir /tmp/test-soverign
```
