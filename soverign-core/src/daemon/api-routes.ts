...
import type { SoverignConfig } from '../config/types.ts';
...
export type ApiContext = {
   ...
   * completion, the dashboard renders a "Restart Soverign" banner when
   ...
   * show the "Restart Soverign" fallback banner.
   */
...
export function createApiRoutes(ctx: ApiContext): Record<string, unknown> {
  return {
    ...
    // after a Soverign update or when swapping LLM providers.
    ...
    '/api/onboarding/reset': {
      POST: async (req: Request) => {
        try {
          ...
          const clientCacheKeys = ['soverign:notif-read', 'soverign:palette-recent'];
          if (scope === 'all') {
            clientCacheKeys.push('soverign:v2:workspaces-ui');
            clientCacheKeys.push('soverign:room-layout');
          }
    ...
    '/api/onboarding/skip': {
      POST: async () => {
        try {
          ...
          // Best-effort service start so the "Restart Soverign" banner
    ...
    '/api/onboarding/setup': {
      POST: async (req: Request) => {
        try {
          ...
          //    "Welcome to Soverign" reply is spoken immediately.
          if (body.tts) {
            try {
          ...
          return json({
            ...
            setup_completed_at: now,
            post_setup_services_started: postSetupStarted,
            message: 'Setup complete. Soverign is ready.',
          });
        } catch (err) {
          return errorFromException(err);
        }
    ...
    '/api/system/autostart/restart': {
      POST: () => {
        ...
        if (!isAutostartInstalled()) {
          return error('SOVERIGN keepalive mode is not installed yet.', 400);
        }
        ...
        return json({
          ok: true,
          message: `Restarting the SOVERIGN 24/7 ${getAutostartName()} service.`,
        });
      },
    },
    ...
    '/api/content/:id/attachments': {
      ...
      POST: async (req: Request & { params: { id: string } }) => {
        try {
          ...
          // Save file to ~/.soverign/content/<id>/
          const baseDir = path.join(os.homedir(), '.soverign', 'content', contentId);
    ...
    '/api/content/files/:contentId/:filename': {
      GET: async (req: Request & { params: { contentId: string; filename: string } }) => {
        ...
        const baseDir = path.join(os.homedir(), '.soverign', 'content');
        ...
        try {
          ...
          return new Response(
            `<html><body style="font-family:system-ui;text-align:center;padding:60px">
              <h1>SOVERIGN Google Authorization Complete!</h1>
    ...
    '/api/auth/google/disconnect': {
      POST: async () => {
        try {
          const tokensPath = path.join(os.homedir(), '.soverign', 'google-tokens.json');
          if (existsSync(tokensPath)) {
            const { unlinkSync } = await import('node:fs');
            unlinkSync(tokensPath);
          }
          return json({ ok: true, message: 'Disconnected. Restart SOVERIGN to deactivate observers.' });
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
    ...
    '/api/config/channels': {
      ...
      POST: async (req: Request) => {
        try {
          ...
          return json({ ok: true, message: 'Channel config saved. Restart SOVERIGN to apply changes.' });
        } catch (err) {
    ...
    '/api/config/stt': {
      ...
      POST: async (req: Request) => {
        try {
          ...
          return json({ ok: true, message: 'STT config saved. Restart SOVERIGN to apply changes.' });
        } catch (err) {
     ...
     * "grant Soverign email access" and have it persist.
     *
    ...
    '/api/awareness/captures/:id/image': {
      GET: async (req: Request & { params: { id: string } }) => {
        ...
        if (!capture.sidecar_id) {
          const soverignDir = path.join(os.homedir(), '.soverign');
          if (!isWithin(path.resolve(capture.image_path), path.resolve(soverignDir))) {
        ...
        try {
          ...
          if (project?.githubUrl) {
            const meta = require('node:fs').readFileSync(
              require('node:path').join(projectPath, '.soverign-project.json'), 'utf-8'
            );
            const parsed = JSON.parse(meta);
            if (parsed.github) {
              parsed.github.lastPushedAt = Date.now();
              await Bun.write(require('node:path').join(projectPath, '.soverign-project.json'), JSON.stringify(parsed, null, 2));
            }
          }
