...
export class AgentService implements Service, IAgentService {
  ...
  constructor(config: SoverignConfig) {
    this.config = config;
  ...
  private registerProviders(): void {
    ...
    if (this.llmManager.hasConversationTier()) {
      ...
      // gating, and Soverign-specific feature knowledge. Uses processTaskCall
  ...
  private buildPersona(): string {
    ...
    const name = p?.assistant_name ?? 'SOVERIGN';
    return [
   ...
   * paths (workflow's `soverign-ask` piece, etc.) can give the LLM the same
   * Soverign identity + role + personality + vault context that chat gets.
   *
   ...
   * live-voice framing (~100 tokens). Tools stay available, so SOVERIGN can still
  ...
  buildRealtimeVoiceInstructions(): string {
    const name = this.config.personality?.assistant_name?.trim() || this.role?.name || 'SOVERIGN';
  ...
  private buildPromptContext(userMessage?: string): PromptContext {
    ...
    const context: PromptContext = {
      ...
      currentTime: new Date().toISOString().slice(0, 16) + 'Z',
