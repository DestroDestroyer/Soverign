/**
 * Process Lock Manager for Soverign Daemon
 *
...
const LOG_DIR = join(SOVERIGN_DIR, 'logs');
const LOCK_PATH = join(SOVERIGN_DIR, 'soverign.pid');
const LOG_PATH = join(LOG_DIR, 'soverign.log');

/**
 * Resolve the lock-file path for a given data dir. The daemon itself always
 * runs against `~/.soverign` (no override path today), but the encryption
...
export function lockPathFor(dataDir?: string): string {
  ...
  return join(dataDir, 'soverign.pid');
}
...
if (process.platform !== 'win32') {
  try {
    ...
    flock = compiled.symbols;
  } catch (err) {
    console.warn('[PID] Failed to compile flock: ', err);
  }
}
...
export function acquireLock(pid: number): boolean {
  if (process.platform === 'win32') {
    try {
      mkdirSync(SOVERIGN_DIR, { recursive: true });
 ...
 * default probes `~/.soverign/soverign.pid`.
 */
 ...
 * always targets the default `~/.soverign/soverign.pid` and is only used by the
 ...
 * daemon itself after port resolution so `soverign stop` knows exactly which
