 ...
 * Soverign Daemon
 *
 * Main entry point for the SOVERIGN daemon process.
...
// Constants
const DEFAULT_PORT = 3142;  // SOVERIGN port
const DEFAULT_DATA_DIR = path.join(os.homedir(), '.soverign');
...
Options:
  ...
  --db-path <path>         Database file path (default: ~/.soverign/soverign.db)
  --data-dir <path>        Data directory (default: ~/.soverign)
...
Example:
  bun run src/daemon/index.ts --port 3142 --data-dir ~/.soverign
        `);
        process.exit(0);
    }
  }
...
/**
 * Start the SOVERIGN daemon
 */
export async function startDaemon(userConfig?: Partial<DaemonConfig>): Promise<void> {
  // Load config from YAML (with defaults)
  let soverignConfig;
  try {
    soverignConfig = await loadConfig();
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`\n[Daemon] Failed to parse config file: ${message}`);
    console.error('[Daemon] Fix the YAML syntax in ~/.soverign/config.yaml or delete it to use defaults.\n');
    process.exit(1);
  }

  // Determine data directory: CLI args > config file > default
  const dataDir = userConfig?.dataDir ?? soverignConfig.daemon.data_dir ?? DEFAULT_DATA_DIR;

  // If user specified a custom data dir but no db path, use soverign.db in that dir
  const dbPath = userConfig?.dbPath ?? soverignConfig.daemon.db_path ?? path.join(dataDir, 'soverign.db');

  // Merge configuration
  const port = userConfig?.port ?? soverignConfig.daemon.port ?? DEFAULT_PORT;
  ...
  // Record the actual bound port in the lockfile so `soverign stop` knows which
  // port to verify even when the daemon was started with --port, SOVERIGN_PORT,
  ...
  try {
    ...
    // 2.0a. Seed default LLM settings if none exist
    const { getSetting, setSetting } = await import('../vault/settings.ts');
    if (!getSetting('llm.providers')) {
      logWithTimestamp('No LLM settings found in database, seeding Ollama Local (qwen2.5:1.5b) as default...');
      ...
      setSetting('llm.default', 'ollama:qwen2.5:1.5b');
    ...
    // workflow_job / trigger_event) to the shared Soverign DB. Idempotent.
    ...
    mergeLLMSettingsIntoConfig(soverignConfig);
    ...
    const heartbeatConfig = soverignConfig.heartbeat;
    const reactor = new EventReactor();
    const coalescer = new EventCoalescer();

    // 4b. Create GoogleAuth if configured
    let googleAuth: GoogleAuth | null = null;
    if (soverignConfig.google?.client_id && soverignConfig.google?.client_secret) {
      googleAuth = new GoogleAuth(soverignConfig.google.client_id, soverignConfig.google.client_secret);
    ...
    const agentService = new AgentService(soverignConfig);
    ...
    const channelService = new ChannelService(soverignConfig, agentService);
    ...
    const sidecarManager = new SidecarManager(soverignConfig.daemon.data_dir.replace('~', os.homedir()));
    ...
    const brainSource: 'env' | 'config' | 'default' = process.env.SOVERIGN_BRAIN_DOMAIN
      ? 'env'
      : soverignConfig.daemon.brain_domain
        ? 'config'
        : 'default';
    const brainDomain = soverignConfig.daemon.brain_domain ?? `localhost:${config.port}`;
    ...
    if (soverignConfig.tts?.enabled) {
      const { createTTSProvider } = await import('../comms/voice.ts');
      const ttsProvider = createTTSProvider(soverignConfig.tts);
      if (ttsProvider) {
        wsService.setTTSProvider(ttsProvider);
        console.log(`[Daemon] TTS enabled: ${soverignConfig.tts.voice ?? 'en-US-AriaNeural'}`);
      }
    }

    // 8d. Wire STT provider for voice input via dashboard
    if (soverignConfig.stt) {
      const { createSTTProvider } = await import('../comms/voice.ts');
      const sttProvider = createSTTProvider(soverignConfig.stt);
      if (sttProvider) {
        wsService.setSTTProvider(sttProvider);
        console.log(`[Daemon] STT for voice input: ${soverignConfig.stt.provider}`);
      }
    }

    // 8e. Wire Authority & Autonomy Engine
    const authorityConfig = soverignConfig.authority ?? { default_level: 3 };
    ...
    setCorsOrigin(soverignConfig.daemon.port);
    ...
    systemCron = new SystemCronService(sharedEventBus, soverignConfig.cron);
    systemCron.start();
    ...
    // canonical PieceCatalog. /v1/soverign/* service backends are wired below
    ...
    // Build the credential resolver early so we can register Soverign-managed
    // sources (Google OAuth file, etc.) before pieces start asking for
    // connections. Each `soverign:<source>` external id dispatches to the
    // matching source; non-prefixed ids fall through to the `app_connection`
    // repo (encrypted at rest).
    const credentialResolver = new CredentialResolver();
    if (googleAuth) {
      const { SoverignGoogleConnectionSource } = await import(
        "../workflows/credentials/google-source.ts"
      );
      credentialResolver.register(new SoverignGoogleConnectionSource(googleAuth));
      logWithTimestamp(
        "Workflow credential resolver: registered soverign:google source",
      );
    }
    // soverign:telegram bridges the same bot token the daemon's Telegram
    // adapter is using -- pieces (activepieces telegram-bot, custom flows)
    // can reference `soverign:telegram` instead of asking the user to create
    ...
    if (soverignConfig.channels?.telegram?.enabled && soverignConfig.channels.telegram.bot_token) {
      const { SoverignTelegramConnectionSource } = await import(
        "../workflows/credentials/telegram-source.ts"
      );
      credentialResolver.register(
        new SoverignTelegramConnectionSource(
          () => soverignConfig.channels?.telegram?.bot_token ?? null,
        ),
      );
      logWithTimestamp(
        "Workflow credential resolver: registered soverign:telegram source",
      );
    }
    try {
      ...
      console.error(
        [
          ...
          `           - Disk full or ~/.soverign/cache unwriteable`,
    ...
    const authToken = soverignConfig.auth?.token;
    if (authToken) {
    ...
    if (toolRegistry) {
      ...
      // legacy `SoverignLlmClient`/`SoverignToolRegistryAdapter` classes since
      ...
      const composerToolRegistry = toolRegistry
        ? {
            ...
            // correct `params` and reject a soverign-tool:invoke step that omits a
      ...
      const manageWorkflowTool = createManageWorkflowTool({
        ...
        // Gmail/Calendar via Soverign tools).
    ...
    // 10.1. Workflow runtime: wire `/v1/soverign/*` service backends and start
    ...
    if (workflowSandboxApi && workflowEngineRuntime) {
      ...
      // `soverign-agent.delegate` runs the full sub-agent loop (LLM + tool
      ...
      const backends = buildSandboxServiceBackends({
        ...
        auditTrail,
        emergencyController,
        // Give the soverign-ask piece the same Soverign-flavoured system
        // prompt the chat agent uses, so workflow LLM calls answer as
        // Soverign rather than as the bare base model. `"workflow"` is the
        // channel slug -- not a real channel, just a key for personality
        // overrides if any are configured.
        buildSoverignSystemPrompt: (userMessage) =>
    ...
    const inSetupMode = !soverignConfig.onboarding?.setup_completed_at;
    if (inSetupMode) {
    ...
    const startPostSetupServices = async (): Promise<void> => {
      ...
      const bgAgentService = new BackgroundAgentService(soverignConfig, agentService.getLLMManager());
      ...
      if (soverignConfig.awareness?.enabled !== false && !config.noLocalTools) {
    ...
    const startAwarenessService = async (): Promise<void> => {
      ...
      try {
          ...
          (event) => {
            ...
            if (event.type === 'suggestion_ready') {
              ...
              if (hasWsClients) {
                ...
                sendDesktopNotification(`SOVERIGN: ${title}`, body, { urgency: 'normal' });
                ...
                sendDesktopNotification(`SOVERIGN: ${title}`, body, { urgency: 'critical', expireMs: 30000 });
              }
            }
              ...
              if (errorText.length > 5) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    sendDesktopNotification(`SOVERIGN: Fix for ${appName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
              ...
              if (compositeScore >= 0.7 && (appCategory === 'code_editor' || appCategory === 'terminal')) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    wsService.broadcastNotification(solutionText, 'urgent');
                    sendDesktopNotification(`SOVERIGN: Help for ${sAppName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
                    const voiceText = solution
                      .replace(/#{1,6}\s*/g, '')
                      .replace(/\*{1,2}([^*]+)\*{1,2}/g, '$1')
                      .replace(/`([^`]+)`/g, '$1')
        ...
        sidecarManager.onConnect((sidecarId) => {
          const cfg = soverignConfig.awareness;
          if (!cfg) return;
        ...
        if (soverignConfig.awareness?.overlay_autolaunch !== false) {
          try {
    ...
    if (soverignConfig.sites?.enabled !== false) {
      try {
        const { SiteBuilderService } = await import('../sites/service.ts');
        const sitesConfig = soverignConfig.sites ?? {
          enabled: true,
    ...
    const goalsConfig = soverignConfig.goals;
    ...
    logWithTimestamp(`SOVERIGN daemon running on port ${config.port}`);
    console.log('');
