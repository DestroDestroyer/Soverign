  ...
  private async handleChat(msg: WSMessage, ws?: ServerWebSocket<unknown>): Promise<WSMessage | void> {
    ...
    if (this.isSetupMode()) {
      return {
        ...
        payload: {
          code: 'setup_required',
          message: 'Finish first-run setup before chatting with Soverign.',
        },
        id: msg.id,
        timestamp: Date.now(),
      };
...
1. **Explicit existing-project name.** If the user named an existing project (e.g. "in soverign-landing", "edit the SP500 calculator"), use that one.
2. **Name keyword match.** If the user did NOT ask for a new project AND did NOT name one explicitly, but their request shares a meaningful keyword with an existing project's name (request mentions "landing page" + "soverign-landing" exists → use soverign-landing; request mentions "calculator" + "SP500 ROI Calculator" exists → use that), pick that project.
...
- Do NOT fabricate file paths, line counts, or "your file is now at X" descriptions without a successful site_write_file call backing them.`;
    ...
    try {
      ...
      const speakNextSentence = async () => {
        ...
        // sentence contains "Soverign" so it can suppress the wake
        ...
        if (!ttsStartSent) {
          ...
          // Subsequent sentence in the same turn that contains "Soverign" —
          // notify so the UI can flip suppression on mid-turn. (We never
          // un-suppress mid-turn because earlier audio with "Soverign" may
  ...
  private getRealtimeBudget(): RealtimeBudgetTracker {
    if (!this.realtimeBudget) {
      const dataDir = this.agentService.getConfig().daemon?.data_dir || join(homedir(), '.soverign');
  ...
  private async handleInterviewMessage(
    ...
    // "Soverign is thinking…" forever with the composer disabled.
    ...
    if (willSpeak && this.ttsProvider) {
      ...
      try {
        this.wsServer.sendToClient(ws, {
          ...
          // are short and we don't need "Soverign" interrupts here.
          payload: { requestId, containsWake: true },
          id: requestId,
          timestamp: Date.now(),
        });
...
function ackForRoomAction(ra: { room: string; action: string; args?: Record<string, unknown> }): string {
  ...
  switch (ra.action) {
    ...
    case 'enable_telegram':
      return `Enabling Telegram. Restart Soverign to apply.`;
    case 'disable_telegram':
      return `Disabling Telegram. Restart Soverign to apply.`;
    case 'enable_discord':
      return `Enabling Discord. Restart Soverign to apply.`;
    case 'disable_discord':
      return `Disabling Discord. Restart Soverign to apply.`;
    case 'set_stt_provider':
      return a.provider
        ? `Setting STT to ${String(a.provider)}. Restart Soverign to apply.`
    ...
    case 'restart_daemon':
      return `Restarting Soverign. The dashboard will reconnect in a few seconds.`;
