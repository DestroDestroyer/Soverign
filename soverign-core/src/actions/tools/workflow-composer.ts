 ...
 * by prompting the configured Soverign LLM with the piece catalog + a schema
...
const AGENT_PIECE_SHORT = "soverign-agent";
...
const TOOL_PIECE_SHORT = "soverign-tool";
 ...
 * catalog AND rejected in validation -- the LLM was picking `soverign-test:echo`
 ...
 */
const COMPOSER_EXCLUDED_PIECES = new Set(["soverign-test", "soverign-validate"]);
...
/** One parameter of a Soverign tool, as surfaced to the composer. */
...
/**
 * A Soverign tool's invocation contract: its name plus the parameters it accepts.
 * The composer lists these so the LLM wires `soverign-tool { toolName, params }`
...
export interface ComposeDeps {
  ...
  /**
   * Optional list of registered Soverign tool names. When present, surfaced in
   * the planner prompt so the LLM can wire `soverign-tool { toolName: '...' }`
   ...
   * `soverign-tool:invoke` step's `params`. Falls back to `toolNames` (names
   * only) when absent.
   */
   ...
   * a `soverign-agent:delegate` step, and (b) rejects any `delegate` step whose
...
function buildSystemPrompt(catalog: string, toolsText: string, rolesText: string): string {
  return [
    "You are the Soverign workflow composer. Convert the user's description into a workflow definition.",
    "",
    ...
    "  - For event triggers: pieceName='soverign-trigger', triggerName='on_event', input.eventType='<event type>'.",
    ...
    "  - For soverign-trigger:on_event, the trigger output is an event envelope shaped",
    ...
    "  - When the user asks for an integration that isn't a registered piece (Gmail, Slack, ...), use the `soverign-tool` piece with `toolName` set to a registered Soverign tool. Available tools are listed below.",
    rolesText
      ? "  - To hand a goal to a sub-agent, use the `soverign-agent` piece's `delegate` action. Its optional `input.role` MUST be one of the specialist role ids listed below VERBATIM (e.g. `research-analyst`, not `researcher`). If none fits, OMIT `role` and the default agent handles it."
      : "  - To hand a goal to a sub-agent, use the `soverign-agent` piece's `delegate` action with `input.goal`. Omit `input.role` to use the default agent.",
...
/**
 * Render the Soverign tool listing for the prompt. Prefers the richer `tools`
...
function renderTools(
  ...
  if (tools && tools.length > 0) {
    const header =
      "Available Soverign tools (call via `soverign-tool { toolName, params: { ... } }`; include EVERY param marked REQUIRED):";
    const lines = [header];
  ...
  if (toolNames && toolNames.length > 0) {
    return [
      "Available Soverign tools (call via `soverign-tool { toolName, params }`):",
      ...toolNames.map((n) => `  - ${n}`),
    ].join("\n");
  }
  return "";
...
/**
 * List the valid specialist role ids for a `soverign-agent:delegate` step. The
...
function renderSpecialistRoles(roles: ComposerSpecialistRole[] | undefined): string {
  ...
  return [
    "Available specialist sub-agent roles (for `soverign-agent:delegate` -> `input.role`; use the id verbatim):",
    ...roles.map((r) => {
...
function renderCatalog(registry: PieceLookup): string {
  ...
  for (const piece of registry.list()) {
    ...
    for (const trigger of Object.values(piece.triggers ?? {})) {
      ...
      // also set `outputSample` (Soverign extension). Either is a valid
  ...
  // Workflow event-type catalog (used by soverign-trigger:on_event flows).
  lines.push("");
  lines.push("Available event types for soverign-trigger:on_event (settings.input.eventType):");
...
function validateStep(
  ...
  // tells the LLM to use forms like `soverign-trigger` and `soverign-tool`
  // (because that's the short identity humans use), but the catalog
  // is keyed by the full npm package name like
  // `@soverignpieces/piece-soverign-trigger`. Without this resolution the
  // composer would lose every Soverign-piece flow on a validation error
  ...
  // Delegate-role check: a `soverign-agent:delegate` step's optional `role` must
  ...
  // Invoke-params check: a `soverign-tool:invoke` step names a tool in
