...
export interface ComposerToolRegistry {
   ...
   * `soverign-tool:invoke` step can't omit a tool's required param (e.g. `action`)
...
export interface ManageWorkflowDeps {
   ...
   * Soverign tools so the LLM can wire `soverign-tool { toolName: '...' }` correctly
   ...
   * roles in its prompt and rejects a `soverign-agent:delegate` step whose `role`
...
export function createManageWorkflowTool(deps: ManageWorkflowDeps = {}): ToolDefinition {
  return {
    ...
    description: [
      "Create, run, and manage the user's Soverign workflows (automations).",
      "",
