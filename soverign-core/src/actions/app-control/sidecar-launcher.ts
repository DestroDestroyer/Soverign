 ...
 * Checks: ~/.soverign/sidecar/ then repo build output.
 */
  ...
  if (WSLBridge.isWSL()) {
    ...
    try {
      ...
      if (userProfile && !userProfile.includes('%')) {
        ...
        const wslPath = `/mnt/${drive}${rest}/.soverign/sidecar/desktop-bridge.exe`;
    ...
    candidates.push('/mnt/c/Users/' + (process.env.USER ?? 'user') + '/.soverign/sidecar/desktop-bridge.exe');
  } else {
    // Native Windows
    candidates.push(join(homedir(), '.soverign', 'sidecar', 'desktop-bridge.exe'));
  }
