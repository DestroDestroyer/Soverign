...
async function testActionLayer() {
  console.log('Testing Soverign Action Layer\n');
  ...
  try {
    const result = await executor.execute('echo "Hello from SOVERIGN"');
  ...
  try {
    const result = await registry.execute('echo', { message: 'Hello SOVERIGN!' });
