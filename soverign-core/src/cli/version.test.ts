...
async function withFakeGit<T>(
  ...
  try {
    ...
    if (previousGitBin === undefined) {
      delete process.env.SOVERIGN_GIT_BIN;
    } else {
      process.env.SOVERIGN_GIT_BIN = previousGitBin;
    }
  }
}
...
if [ "$3" = "describe" ] && [ "$4" = "--tags" ] && [ "$5" = "--always" ]; then
  ...
  test('falls back to package.json when not a git checkout', async () => {
    ...
    // Point SOVERIGN_GIT_BIN at a non-existent file so rev-parse fails →
    // isGitCheckout returns false and we never call describe.
    const previousGitBin = process.env.SOVERIGN_GIT_BIN;
    process.env.SOVERIGN_GIT_BIN = join(dir, 'no-such-git-binary');
    try {
      expect(getInstalledVersion(dir)).toBe('3.2.1');
    } finally {
      if (previousGitBin === undefined) delete process.env.SOVERIGN_GIT_BIN;
      else process.env.SOVERIGN_GIT_BIN = previousGitBin;
    }
  });
...
// Run the real `bin/soverign.ts --version` / `version` paths so the banner
...
describe('CLI --version output', () => {
  ...
  const SOVERIGN_BIN = join(REPO_ROOT, 'bin', 'soverign.ts');

  async function runCli(args: string[]): Promise<{ exitCode: number; stdout: string }> {
    const proc = Bun.spawn(['bun', SOVERIGN_BIN, ...args], {
      stdout: 'pipe',
      stderr: 'pipe',
      cwd: REPO_ROOT,
    });
  ...
  test('`soverign --version` prints v-prefixed version with no doubling', async () => {
  ...
  test('`soverign version` matches `--version` output', async () => {
  ...
  test('`soverign -v` matches `--version` output', async () => {
  ...
  test('`soverign help` banner contains exactly one v-prefixed version (no vv)', async () => {
