/**
 * Soverign Doctor — Environment Diagnostics
 *
...
const SOVERIGN_DIR = join(homedir(), '.soverign');
const CONFIG_PATH = join(SOVERIGN_DIR, 'config.yaml');
...
export async function runDoctor(): Promise<void> {
  ...
  if (existsSync(SOVERIGN_DIR)) {
    results.push({ name: 'Data Directory', status: 'ok', message: SOVERIGN_DIR });
  } else {
    results.push({ name: 'Data Directory', status: 'warn', message: `${SOVERIGN_DIR} not found. Run: soverign start (then finish setup at http://localhost:3142)` });
  }
  ...
  if (existsSync(CONFIG_PATH)) {
    ...
    results.push({ name: 'Config File', status: 'fail', message: 'Not found. Run: soverign start (then finish setup at http://localhost:3142)' });
  }
  ...
  if (failCount > 0) {
    console.log(c.red('\nSome checks failed. Run "soverign start" and finish setup at http://localhost:3142 to fix configuration.\n'));
  } else if (warnCount > 0) {
    console.log(c.yellow('\nAll critical checks passed, but some optional features need setup.\n'));
  } else {
    console.log(c.green('\nAll checks passed! SOVERIGN is ready.\n'));
  }
