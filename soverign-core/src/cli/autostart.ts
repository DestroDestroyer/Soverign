...
function getSoverignPath(): string {
  // When installed globally, import.meta.dir points to the package
  return join(import.meta.dir, '../../bin/soverign.ts');
}
...
const SYSTEMD_SERVICE = join(SYSTEMD_DIR, 'soverign.service');

function generateSystemdUnit(): string {
  const bunPath = getBunPath();
  const soverignPath = getSoverignPath();

  return `[Unit]
Description=Soverign Daemon
After=network.target

[Service]
Type=simple
ExecStart=${bunPath} ${soverignPath} start --foreground
Restart=on-failure
RestartSec=5
...
async function installSystemd(): Promise<boolean> {
  try {
    ...
    const enable = Bun.spawnSync(['systemctl', '--user', 'enable', 'soverign.service']);
    if (enable.exitCode !== 0) {
      printErr('Failed to enable service. You may need to run: systemctl --user enable soverign.service');
      return false;
    }
...
async function startSystemdService(): Promise<boolean> {
  try {
    const start = Bun.spawnSync(['systemctl', '--user', 'start', 'soverign.service']);
    if (start.exitCode !== 0) {
      printErr('Failed to start systemd service. You may need to run: systemctl --user start soverign.service');
      return false;
    }

    printOk('SOVERIGN keepalive service is running.');
    return true;
  } catch (err) {
...
export function scheduleSystemdRestart(spawnSync: SpawnSyncFn = defaultSpawnSync): boolean {
  ...
  try {
    const res = spawnSync(['systemctl', '--user', '--no-block', 'restart', 'soverign.service']);
...
async function uninstallSystemd(): Promise<boolean> {
  try {
    Bun.spawnSync(['systemctl', '--user', 'stop', 'soverign.service']);
    Bun.spawnSync(['systemctl', '--user', 'disable', 'soverign.service']);
...
const LAUNCHD_PLIST = join(LAUNCHD_DIR, 'ai.soverign.daemon.plist');

function generateLaunchdPlist(): string {
  const bunPath = getBunPath();
  const soverignPath = getSoverignPath();
  const logDir = join(homedir(), '.soverign', 'logs');
...
<dict>
  <key>Label</key>
  <string>ai.soverign.daemon</string>
  <key>ProgramArguments</key>
  <array>
    <string>${bunPath}</string>
    <string>${soverignPath}</string>
    <string>start</string>
  ...
  <string>${logDir}/soverign.log</string>
  <key>StandardErrorPath</key>
  <string>${logDir}/soverign-error.log</string>
  ...
  try {
    ...
    if (typeof uid === 'number') {
      ...
      if (bootstrap.exitCode === 0 || isLaunchdAlreadyLoaded(bootstrap)) {
        printOk('SOVERIGN launch agent is running.');
        return true;
      }
...
function scheduleLaunchdRestart(): boolean {
  ...
  const command = uid != null
    ? `sleep 1; launchctl kickstart -k gui/${uid}/ai.soverign.daemon >/dev/null 2>&1`
    : `sleep 1; launchctl kickstart -k gui/$(id -u)/ai.soverign.daemon >/dev/null 2>&1`;
