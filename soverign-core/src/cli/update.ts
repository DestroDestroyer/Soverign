/**
 * `soverign update` — install-method-aware updater.
 *
 * Dispatches based on how SOVERIGN was installed:
 *   docker      refuse, point user at host-side `docker pull`
 *   bun-global  `bun update -g @usesoverign/brain`
...
/**
 * Spawn the daemon in a detached process, same pattern as `soverign start -d`.
...
function restartDaemonDetached(packageRoot: string): void {
  ...
  const child = spawn('bun', [join(packageRoot, 'bin/soverign.ts'), 'start', '--no-open'], {
    detached: true,
...
function updateDocker(info: InstallMethodInfo): UpdateResult {
  console.log(c.yellow('SOVERIGN is running inside a Docker container.'));
  console.log('');
  console.log('  `soverign update` does not apply to container installs — the container is the unit of update.');
  console.log('  From your host, run:');
  console.log(c.dim('    docker pull <your-soverign-image>'));
  console.log(c.dim('    docker rm -f soverign && docker run -d ... <your-soverign-image>'));
  console.log('');
...
function updateDev(info: InstallMethodInfo, packageRoot: string): UpdateResult {
  console.log(c.yellow('SOVERIGN is running from a developer checkout.'));
  console.log('');
  console.log('  `soverign update` does not manage dev checkouts — updating your working tree');
...
function updateUnknown(info: InstallMethodInfo): UpdateResult {
  console.log(c.red('Could not determine how SOVERIGN was installed.'));
  console.log('');
  console.log('  `soverign update` needs to know whether to run `bun update -g`, `git pull`,');
  console.log('  or redirect you to `docker pull`. Run `soverign doctor` to see what was detected.');
  console.log('');
  console.log('  Update manually with one of:');
  console.log(c.dim('    bun update -g @usesoverign/brain              # if installed via bun'));
...
function updateBunGlobal(
  ...
  console.log(c.dim('Running `bun update -g @usesoverign/brain`...\n'));

  const result = spawn(['bun', 'update', '-g', '@usesoverign/brain']);
