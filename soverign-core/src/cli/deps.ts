...
/**
 * Check for core CLI tools SOVERIGN relies on for common workflows.
 */
...
export function checkGoogleAuth(): DepStatus {
  const tokensPath = join(homedir(), '.soverign', 'google-tokens.json');
...
export async function setupGoogleOAuth(config: any): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    try {
        ...
        async fetch(req) {
          ...
          if (url.pathname === '/api/auth/google/callback') {
            ...
            try {
              ...
              return new Response(
                '<html><body><h1>SOVERIGN Google Authorization Complete!</h1><p>You can close this tab.</p></body></html>',
                { headers: { 'Content-Type': 'text/html' } }
              );
            } catch (err) {
              clearTimeout(timeout);
      ...
      printInfo('Stop the SOVERIGN daemon first, or run later with: bun run setup:google');
      resolve(false);
      return;
    }
