...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'soverign-uninstall-test-'));
...
describe('createCleanupPlan — method-specific paths', () => {
  test('script install schedules dataDir for removal', () => {
    const plan = createCleanupPlan(join(fakeHome, '.soverign', 'daemon'), {
      env: {},
      homeDir: fakeHome,
      detect: fakeDetect('script'),
    });
    expect(plan.method).toBe('script');
    expect(plan.removablePaths).toContain(join(fakeHome, '.soverign'));
    expect(plan.runBunUninstall).toBe(false);
  });

  test('bun-global install schedules dataDir only; package handled by bun', () => {
    const packageRoot = join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@usesoverign', 'brain');
  ...
  test('dev checkout is never added to removablePaths', () => {
    const devPath = join(workDir, 'work', 'soverign');
...
describe('createCleanupPlan — env overrides', () => {
  test('honors SOVERIGN_HOME for data dir', () => {
    const customDataDir = join(workDir, 'custom-soverign');
    const plan = createCleanupPlan(join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@usesoverign', 'brain'), {
      env: { SOVERIGN_HOME: customDataDir },
      homeDir: fakeHome,
      detect: fakeDetect('bun-global'),
    });
    expect(plan.dataDir).toBe(customDataDir);
    expect(plan.removablePaths).toContain(customDataDir);
    // Must not include the default ~/.soverign when SOVERIGN_HOME is set.
    expect(plan.removablePaths).not.toContain(join(fakeHome, '.soverign'));
  });

  test('honors BUN_INSTALL for wrapper candidates', () => {
    const customBun = join(workDir, 'custom-bun');
    const wrapperPath = join(customBun, 'bin', 'soverign');
  ...
  test('default wrapper candidates live under ~/.bun/bin', () => {
    const defaultWrapper = join(fakeHome, '.bun', 'bin', 'soverign');
...
describe('buildCleanupScript — executes correctly against a fixture tree', () => {
  test('unlinks wrappers, removes dataDir, skips package dir when flag off', async () => {
    const dataDir = join(workDir, 'fake-home', '.soverign');
    const wrapperPath = join(workDir, 'fake-bun', 'bin', 'soverign');
    ...
    const tempDir = mkdtempSync(join(tmpdir(), 'soverign-uninstall-exec-'));
  ...
  test('missing wrapper is a soft no-op, not a failure', async () => {
    const dataDir = join(workDir, 'fake-home-2', '.soverign');
    const missingWrapper = join(workDir, 'does-not-exist', 'soverign');
    mkdirSync(dataDir, { recursive: true });
