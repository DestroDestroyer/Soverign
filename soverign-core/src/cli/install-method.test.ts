...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'soverign-install-method-'));
...
describe('detectInstallMethod — docker signals', () => {
  test('SOVERIGN_INSTALL_METHOD=docker wins even when marker says otherwise', () => {
    const root = join(workDir, 'app');
    writeMarker(root, { method: 'bun-global' });
    const info = detectInstallMethod(root, {
      env: { SOVERIGN_INSTALL_METHOD: 'docker' },
      dockerEnvPath: noDockerEnv,
      homeDir: fakeHome,
    });
    expect(info.method).toBe('docker');
    expect(info.reason).toContain('SOVERIGN_INSTALL_METHOD');
  });
...
describe('detectInstallMethod — path inference', () => {
  test('package root under ~/.bun/install/global is bun-global', () => {
    const root = join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@usesoverign', 'brain');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,
  ...
  test('~/.soverign/daemon with .git is script install', () => {
    const root = join(fakeHome, '.soverign', 'daemon');
  ...
  test('~/.soverign/daemon without .git is unknown (half-installed)', () => {
    const root = join(fakeHome, '.soverign', 'daemon');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
  ...
  test('arbitrary git checkout is dev', () => {
    const root = join(workDir, 'work', 'soverign');
    mkdirSync(join(root, '.git'), { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,
