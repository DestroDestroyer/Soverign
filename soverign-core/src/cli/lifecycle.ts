 ...
 */
export function readConfiguredPort(configPath = join(homedir(), '.soverign', 'config.yaml')): number | null {
  try {
...
/**
 * Resolve which port `soverign stop` should verify.
 *
 * Precedence (highest first):
 *   1. The port the running daemon recorded in its lockfile (authoritative).
 *   2. `SOVERIGN_PORT` env var (matches `applyEnvOverrides` in the config loader).
 *   3. An explicit `--port N` passed on the stop command line.
 *   4. `daemon.port` from `~/.soverign/config.yaml`.
 ...
 * env var beats `--port` so a user with a persistent `SOVERIGN_PORT` in their
...
export function resolveStopPort(options?: {
  ...
  const fromEnv = validPort(env.SOVERIGN_PORT);
