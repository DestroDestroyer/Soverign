/**
 * `soverign uninstall` — install-method-aware uninstaller.
 *
...
function resolveDataDir(env: NodeJS.ProcessEnv, home: string): string {
  return env.SOVERIGN_HOME ? resolve(env.SOVERIGN_HOME) : join(home, '.soverign');
}
...
function resolveWrapperCandidates(env: NodeJS.ProcessEnv, home: string): string[] {
  ...
  return ['soverign', 'soverign.cmd', 'soverign.ps1'].map((name) => join(bin, name));
}
...
export function createCleanupPlan(
  ...
  if (method === 'script') {
    // dataDir already covers resolvedPackageRoot at ~/.soverign/daemon, so only
  ...
  return {
    ...
    // ~/.soverign-uninstall.log sits next to, not inside, the data dir so it
    // survives the removal and is there for the user to inspect on failure.
    logPath: join(home, '.soverign-uninstall.log'),
  };
}
...
// must be self-contained — no imports from the SOVERIGN codebase, since the
...
// Give the parent soverign CLI 1.5s to fully exit and release the wrapper /
...
if (payload.runBunUninstall) {
  try {
    const r = spawnSync(payload.bunPath, ['uninstall', '-g', '@usesoverign/brain'], {
      stdio: 'pipe',
      env: { ...process.env },
    });
...
// soverign-uninstall-* directories across repeated uninstall attempts.
try {
...
async function schedulePackageRemoval(plan: CleanupPlan): Promise<void> {
  const tempDir = mkdtempSync(join(tmpdir(), 'soverign-uninstall-'));
...
function printPlan(plan: CleanupPlan): void {
  ...
  if (plan.runBunUninstall) {
    console.log(c.dim(`  • Run: bun uninstall -g @usesoverign/brain`));
  }
  if (plan.method === 'dev') {
...
function refuseDocker(plan: CleanupPlan): void {
  console.log(c.yellow('\nSOVERIGN is running inside a Docker container.\n'));
  console.log('  `soverign uninstall` does not apply to container installs — the');
  console.log('  container is the unit of uninstall.');
  console.log('');
  console.log('  From your host, run:');
  console.log(c.dim('    docker rm -f soverign'));
  console.log(c.dim('    docker volume rm soverign-data   # if you want to delete data too'));
  console.log('');
