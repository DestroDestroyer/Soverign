...
[![CI](https://github.com/vierisid/soverign/actions/workflows/test.yml/badge.svg)](https://github.com/vierisid/soverign/actions/workflows/test.yml)
[![bun](https://img.shields.io/npm/v/@usesoverign/brain?label=bun&logo=bun&color=%23f9f1e1)](https://bun.sh/packages/@usesoverign/brain)
...
- [SOVERIGN](#soverign)
  ...
  - [🔍 What Makes SOVERIGN Different](#-what-makes-soverign-different)
  ...
  - [🚀 Usage](#-usage)
    - [Updating](#updating)
    - [Removing SOVERIGN](#removing-soverign)
...
## 🔍 What Makes SOVERIGN Different

| Feature | Typical AI Assistant | SOVERIGN |
|---|---|---|
...
```bash
bun install -g @usesoverign/brain   # Install the daemon
soverign start -d                   # Start as background daemon
```
...
Don't want to deal with servers, DNS, or TLS certificates? We've partnered with **[opencove.host](https://opencove.host)** — a managed hosting platform built specifically for SOVERIGN.
...
- **Up and running in under 5 minutes** — spin up your SOVERIGN instance and start using it immediately
...
**Research while you work** — Ask SOVERIGN to deep-dive a topic. It runs browser searches, reads pages, and compiles a summary in the background while you focus on other things.

**Automate across machines** — Run the daemon on your home server. Connect sidecars on your work laptop and your desktop. SOVERIGN can move files between them, run scripts on your server, and open apps on your laptop — all from one conversation.
...
**Desktop co-pilot** — SOVERIGN watches your screen via the sidecar. If it sees you struggling with an error message or a complex form, it proactively offers help or fills in fields for you.

**Goal accountability** — Define OKRs in the Goals dashboard. SOVERIGN plans your day each morning, checks in during the evening, and escalates if you're falling behind — like a personal drill sergeant.
...
```bash
bun install -g @usesoverign/brain
soverign start
```

The first time you run `soverign start`, the daemon boots in setup mode and the dashboard at `http://localhost:3142` guides you through LLM provider, voice (TTS) choice, a conversational profile interview, and a 10-minute spotlight tour.

> **Restart after first-time setup:** The daemon constructs background services (heartbeat, commitments, awareness) at boot, gated on setup having already been completed. Once you finish setup in the dashboard, those services don't activate until the next start — the dashboard shows a banner reminding you. Run `soverign restart` (or stop/start) to bring them online. This will go away in a follow-up that constructs the services in-process at setup completion.

> **Note:** Native Windows installs are blocked for the SOVERIGN daemon. On Windows, use WSL2 for the Bun install above, or use the Docker install instead.

### Docker

Run SOVERIGN on any OS with a single command — no Bun or dependencies required. Install [Docker Desktop](https://www.docker.com/products/docker-desktop/) (Windows, macOS, Linux) if you don't have Docker yet.

```bash
docker run -d --name soverign \
  -p 3142:3142 \
  -v soverign-data:/data \
  ghcr.io/vierisid/soverign:latest
```

The image is available on [GHCR](https://ghcr.io/vierisid/soverign). Non-LLM configuration can be provided via environment variables or by mounting a `config.yaml` into the `/data` volume. LLM providers, API keys, and model routing are configured from the settings dashboard (open http://localhost:3142 after first boot) and stored in the database + encrypted keychain - they are not set via env vars or `config.yaml`.

> **Note:** Docker runs in an isolated container, so the daemon inside it cannot access your host desktop, browser, or clipboard directly. You must still install the [sidecar](#️-sidecar-setup) on each machine where you want SOVERIGN to have desktop awareness and automation capabilities.

### One-liner

```bash
curl -fsSL https://raw.githubusercontent.com/vierisid/soverign/main/install.sh | bash
```
...
The install script sets up Bun, clones the repo, and links the `soverign` CLI. Then finish setup in your browser at `http://localhost:3142`.
...
```bash
git clone https://github.com/vierisid/soverign.git ~/.soverign/daemon
cd ~/.soverign/daemon
bun install
bun run build:ui
bun link
...
```bash
soverign start            # Start in foreground
soverign start -d         # Start as background daemon
soverign start --port 3142 # Start on a specific port
soverign stop             # Stop the daemon
soverign status           # Check if running
soverign doctor           # Verify environment & connectivity
soverign logs -f          # Follow live logs
```
...
`soverign update` detects how you installed SOVERIGN and runs the right update command. Equivalent manual commands per install method:

| Install method | `soverign update` dispatches to |
| --- | --- |
| Bun global (`bun install -g @usesoverign/brain`) | `bun update -g @usesoverign/brain` |
| `install.sh` (git clone under `~/.soverign/daemon`) | `git pull --ff-only` + `bun install` |
| Docker | Refused — run `docker pull <image> && docker rm -f soverign && docker run ...` on the host |
| Developer checkout | Refused — run `git pull` yourself |

Run `soverign doctor` to see what was detected and the exact commands for your install.

### Removing SOVERIGN

`soverign uninstall` stops the daemon, removes autostart hooks, deletes `~/.soverign`, and — where applicable — runs the correct package-manager uninstall. It does **not** touch sidecars.

| Install method | `soverign uninstall` dispatches to |
| --- | --- |
| Bun global | `bun uninstall -g @usesoverign/brain` + side-effect cleanup |
| `install.sh` | `rm -rf ~/.soverign/daemon` + CLI wrapper + side-effect cleanup |
| Docker | Refused — run `docker rm -f soverign` (and optionally `docker volume rm soverign-data`) on the host |
| Developer checkout | Side-effect cleanup only — your checkout is left in place |

> **Tip:** If you already ran `bun uninstall -g @usesoverign/brain` without going through `soverign uninstall`, your daemon may still be running and `~/.soverign` is still on disk. Run `soverign doctor` before uninstalling to see what will be cleaned up, or stop the daemon and remove `~/.soverign` manually.

---

## 🖥️ Sidecar Setup

The sidecar is what gives SOVERIGN physical reach beyond the machine it runs on. It is a lightweight agent that you install on any machine — your laptop, a dev server, a home PC — and it connects back to the central daemon over an authenticated WebSocket. Each sidecar gives SOVERIGN access to that machine's desktop, browser, terminal, filesystem, clipboard, and screenshots.

This means you can run the daemon on an always-on server and still interact with your desktop machines as if SOVERIGN were running locally. Enroll as many sidecars as you want.
...
```bash
bun install -g @usesoverign/sidecar
```

**Or download the binary** from [GitHub Releases](https://github.com/vierisid/soverign/releases) for your platform (macOS, Linux, Windows).

### 2. Enroll in the dashboard

1. Open the SOVERIGN dashboard at `http://localhost:3142`
...
```bash
soverign-sidecar --token <your-token>
```
...
**Memory & Knowledge** — Vault knowledge graph (entities, facts, relationships) stored in SQLite. Extracted automatically after each response. Injected into the system prompt so SOVERIGN always remembers what matters.
...
SOVERIGN stores its configuration at `~/.soverign/config.yaml`. Open the dashboard at `http://localhost:3142` after `soverign start` for guided setup — it walks through LLM provider, voice, and a profile interview the first time. The Settings room lets you tweak channels, personality, and authority later.

```yaml
daemon:
  port: 3142
  data_dir: "~/.soverign"
  db_path: "~/.soverign/soverign.db"
...
This separation means SOVERIGN stays reachable 24/7 on a server while still being able to see your screen, type in your apps, and manage files on any machine where a sidecar is running.
...
- **Package**: `@usesoverign/brain` (published to npm registry, installable via bun)
...
- [Website](https://usesoverign.dev) — Project homepage and documentation
- [GitHub Issues](https://github.com/vierisid/soverign/issues) — Bug reports and feature requests

---

## 📊 Telemetry

SOVERIGN sends **anonymous** usage metrics so the project can measure its unique
...
```yaml
# ~/.soverign/config.yaml
telemetry:
  enabled: false
```

```bash
SOVERIGN_TELEMETRY=0   # or the cross-tool standard: DO_NOT_TRACK=1
```
...
SOVERIGN includes a built-in authority engine that gates every action at runtime. All tool executions are logged in an audit trail, and sensitive operations require explicit approval via the dashboard, Telegram, or Discord. Emergency pause and kill controls are always available.
...
Soverign is distributed under the [Soverign Source Available License 2.0](LICENSE) (based on RSALv2).

The Soverign codebase incorporates third-party components under their own licenses. Those components retain their original licenses for any party who extracts them as a standalone work; the combined Soverign distribution is governed by the Soverign Source Available License. See [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) for the full list and per-component license text.
