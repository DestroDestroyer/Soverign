# Soverign — Complete Migration & Operations Package

This folder contains the complete audit, migration details, credentials, and custom helper scripts for the **Soverign** project. Use this folder to migrate the project to another location or set up a new workspace.
...
- **OAuth Authorization Token Details:** Saved in the local [drive_token.json](file:///C:/Ai/project/soverign-migration-info/drive_token.json) file.

### 2. Instagram Saved Reels Link
The target collection used to extract Soverign AI features:
...
- **Solution:** 
  - Rewrote the Electron main process [main.js](file:///C:/Ai/project/soverign-desktop/main.js) and launch scripts to boot natively on Windows.
  - The daemon is now started natively using `bun run src/daemon/index.ts` inside the `soverign-core` directory.
  - Migrated the daemon log streamer in Electron to tail the Windows-native log file (`~/.soverign/soverign.log`) using PowerShell's native `Get-Content -Wait` command.
  - Replaced WSL-localhost paths with Windows-native user profile path (`~/.soverign/config.yaml`) for reading/writing configuration files.
...
- **Solution:** Modified [install_shortcut.ps1](file:///C:/Ai/project/soverign-desktop/install_shortcut.ps1) to delete the old shortcut and create a new `Soverign.lnk` shortcut pointing to the native Electron executable (`C:\Ai\project\soverign-desktop\node_modules\electron\dist\electron.exe`) with arguments targeting the correct `soverign-desktop` project folder.
...
- **Solution:** Created the `fetch_url` tool in [builtin.ts](file:///C:/Ai/project/soverign-core/src/actions/tools/builtin.ts), implementing a native fetch engine with automatic HTML-to-text sanitization (stripping script/style tags) and JSON formatting.
...
1. **[run_soverign.bat](file:///C:/Ai/project/soverign-migration-info/scripts/run_soverign.bat):** Native Windows launcher script. It checks if the Soverign Daemon is active on port 3142, starts it in the background if not, and then launches the Electron desktop console.
2. **[stop_soverign.bat](file:///C:/Ai/project/soverign-migration-info/scripts/stop_soverign.bat):** Native Windows script to find and terminate any process running on port 3142, effectively shutting down the Soverign Daemon.
3. **[install_shortcut.ps1](file:///C:/Ai/project/soverign-migration-info/scripts/install_shortcut.ps1):** PowerShell script to install a working, clean desktop shortcut called `Soverign.lnk` pointing to the local Electron app folder.
4. **[enum_windows.ps1](file:///C:/Ai/project/soverign-migration-info/scripts/enum_windows.ps1):** Low-level Win32 API script that lists all currently open desktop windows with their titles and process IDs.
5. **[get_active_window.ps1](file:///C:/Ai/project/soverign-migration-info/scripts/get_active_window.ps1):** Script that returns details (bounds, process name, title, PID) of the window currently in focus.
6. **[list_windows.ps1](file:///C:/Ai/project/soverign-migration-info/scripts/list_windows.ps1):** Script that lists details of all visible window frames on the screen.
