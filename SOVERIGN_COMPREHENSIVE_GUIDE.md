# Soverign Application Comprehensive Guide & Codebase

This document serves as the absolute knowledge base, architectural reference, and unified source code repository for the **Soverign AI Desktop Application**. It consolidates all the information, codebase structures, troubleshooting procedures, and updates implemented across both the `soverign-core` and `soverign-desktop` layers.

---

## 1. Architectural Overview & Services

Soverign is a unified, local-first personal AI assistant built using **Electron (Frontend Desktop console)** and **Bun (Backend Daemon service)**. 
...
- **Daemon (`soverign-core`)**: The Bun backend process that runs the sqlite database, runs LLM agent workflows, manages observers (file watcher, clipboard, process list), and exposes a websocket server on port 3142.
...
- **Problem:** The Soverign Desktop Console would open but show a blank page saying "Not found".
...
- **Fix:** Enhanced `soverign-core/src/config/loader.ts` to wrap parsing in `try/catch`. If config reading or parsing fails, it logs a warning, falls back to `DEFAULT_CONFIG` defaults to keep the daemon online, and automatically rewrites/repairs `config.yaml` with clean default values.

---

## 3. Comprehensive Source Code

Below is the complete source code for all primary files that define the Soverign App, grouped by file path.
...
## soverign-desktop/scripts/install-windows-tasks.ps1
...
## soverign-core/src/authority.ts
...
## soverign-core/src/automation.ts
...
## soverign-core/src/awareness.ts
...
## soverign-core/src/interfaces.ts
...
## soverign-core/src/repository.ts
...
## soverign-core/src/telemetry.ts
...
## soverign-core/ui/index.html

```html
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Soverign Daemon</title><style>body { background-color: #0b0b1a; color: #e0e0ff; font-family: sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; } .container { text-align: center; padding: 2rem; border: 1px solid #3b3b8a; border-radius: 8px; background: rgba(0,0,0,0.4); } h1 { color: #00f2fe; }</style></head><body><div class="container"><h1>Soverign Core Daemon</h1><p>The daemon is running and successfully connected.</p></div></body></html>

```

---

## soverign-core/src/config/loader.ts
...
## soverign-core/src/config/types.ts
 ...
 * there is no separate realtime key. When disabled (default) SOVERIGN uses the
 * standard STT -> text LLM -> TTS pipeline.
 *
 * See docs/GPT_REALTIME_2_INTEGRATION.md.
 */
export type RealtimeVoiceConfig = {
  /** Master opt-in. Default false. Env: SOVERIGN_REALTIME_VOICE. */
  enabled: boolean;
...
export type AuthConfig = {
  /** Shared secret token. If unset, auth is disabled (open access). Env: SOVERIGN_AUTH_TOKEN */
  token?: string;
};
 ...
 * `enabled: false`, the `SOVERIGN_TELEMETRY=0` env var, or the community
...
/**
 * Onboarding completion state — persists in `~/.soverign/config.yaml` so
...
export type SoverignConfig = {
  ...
  daemon: {
     ...
     *
     * Precedence: `SOVERIGN_BRAIN_DOMAIN` env var > this field > internal
...
export const DEFAULT_CONFIG: SoverignConfig = {
  user: {
    name: '',
  },
  telemetry: {
  ...
  sites: {
    enabled: true,
    projects_dir: '~/.soverign/projects',
...
## soverign-core/src/util/fs-secure.ts
...
## soverign-desktop/renderer/index.html
...
<head>
  ...
  <title>Soverign Desktop Console</title>
  ...
  <div class="app-container">
    ...
    <aside class="sidebar">
        ...
        <div class="control-group">
          ...
          <div class="service-card" id="daemon-card">
            <div class="service-header">
              <span class="service-name">Soverign Daemon</span>
      ...
      <div class="sidebar-footer">
        <span>v1.1.0 • Soverign Core</span>
      </div>
    </aside>
    ...
    <main class="main-content">
      ...
      <header class="main-header">
        ...
        <div class="header-title">Soverign Console</div>
      </header>
      ...
      <div class="splash-screen" id="splash-view">
        <div class="splash-content">
          ...
          <p>The Soverign daemon is offline. Click Boot to start the local AI engine on this Windows PC.</p>
          <button id="btn-boot-system" class="btn btn-primary btn-lg">🚀 Boot Soverign</button>
      ...
      <div class="webview-container hidden" id="dashboard-view">
        <webview id="soverign-webview" src="about:blank" style="width:100%; height:100%"></webview>
      </div>
        ...
        <div class="settings-modal">
          ...
          <div class="modal-body">
            ...
            <p style="font-size: 0.8em; color: var(--text-muted); margin-bottom: 10px;">
              Install Soverign as a native Windows scheduled task to run in the background 24/7.
            </p>
...
## soverign-desktop/renderer/index.css
...
body.soverign-activated .brand-text h1 {
  background: linear-gradient(to right, #1d4ed8, #2563eb);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
}

body.soverign-activated .control-group h3 {
  color: #4b5563;
  font-weight: 700;
}

body.soverign-activated .service-card,
body.soverign-activated .config-card {
...
body.soverign-activated .service-card:hover,
body.soverign-activated .config-card:hover {
  border-color: rgba(37, 99, 235, 0.3);
  box-shadow: 0 4px 20px rgba(37, 99, 235, 0.08);
  transform: translateY(-1px);
}

body.soverign-activated .service-name,
body.soverign-activated .config-card label,
body.soverign-activated label {
  color: #1f2937;
  font-weight: 600;
}

body.soverign-activated .status-text {
  color: #4b5563;
}

body.soverign-activated select,
body.soverign-activated input {
...
body.soverign-activated select:focus,
body.soverign-activated input:focus {
  border-color: #2563eb !important;
  box-shadow: 0 0 8px rgba(37, 99, 235, 0.15) !important;
}

body.soverign-activated .btn-outline {
  border-color: rgba(0, 0, 0, 0.12);
  color: #4b5563;
  font-weight: 600;
}

body.soverign-activated .btn-outline:hover {
  border-color: #2563eb;
  color: #2563eb;
  background: rgba(37, 99, 235, 0.05);
}

body.soverign-activated .sidebar-footer {
  border-top: 1px solid rgba(0, 0, 0, 0.06);
  color: #4b5563;
}

/* Advanced Settings Modal Glassmorphism */
body.soverign-activated .settings-overlay {
  background: rgba(255, 255, 255, 0.2);
  backdrop-filter: blur(10px);
  -webkit-backdrop-filter: blur(10px);
}

body.soverign-activated .settings-modal {
...
body.soverign-activated .modal-header h2 {
  background: linear-gradient(to right, #1d4ed8, #2563eb);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
}

body.soverign-activated .settings-field label {
  color: #374151;
}

body.soverign-activated .settings-field input {
  background: rgba(255, 255, 255, 0.7);
  border: 1px solid rgba(0, 0, 0, 0.15);
  color: #111827;
}

body.soverign-activated .settings-field small {
  color: #6b7280;
}

body.soverign-activated .settings-checkbox label {
  color: #374151;
}
...
body.soverign-activated .particles-container {
  display: block;
}
...
body.soverign-activated .btn-sidebar-toggle {
...
body.soverign-activated .btn-sidebar-toggle:hover {
  border-color: #2563eb;
  color: #2563eb;
...
## soverign-desktop/renderer/renderer.js
