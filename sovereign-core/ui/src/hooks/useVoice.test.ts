...
describe("matchesSpeechWakePhrase (strict; used during TTS playback)", () => {
  test("accepts direct wake phrases", () => {
    expect(matchesSpeechWakePhrase("Sovereign")).toBe(true);
    expect(matchesSpeechWakePhrase("hey sovereign")).toBe(true);
    expect(matchesSpeechWakePhrase("Sovereign, stop")).toBe(true);
    expect(matchesSpeechWakePhrase("Hey Sovereign, hold on")).toBe(true);
  });

  test("rejects longer sentences that merely mention sovereign (echo protection)", () => {
    expect(matchesSpeechWakePhrase("Sovereign is already working on that")).toBe(false);
    expect(matchesSpeechWakePhrase("Can you tell Sovereign to send that")).toBe(false);
    expect(matchesSpeechWakePhrase("I said Sovereign in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePhrase("Hey Sovereign can you help")).toBe(false);
  });
});

describe("matchesSpeechWakePrefix (loose; used when idle)", () => {
  test("accepts bare wake phrases", () => {
    expect(matchesSpeechWakePrefix("Sovereign")).toBe(true);
    expect(matchesSpeechWakePrefix("hey sovereign")).toBe(true);
  });

  test("accepts wake phrase followed by any command in one utterance", () => {
    expect(matchesSpeechWakePrefix("hey sovereign turn off the lights")).toBe(true);
    expect(matchesSpeechWakePrefix("sovereign what's the weather")).toBe(true);
    expect(matchesSpeechWakePrefix("Hey Sovereign, play some music")).toBe(true);
    expect(matchesSpeechWakePrefix("sovereign can you help me")).toBe(true);
  });

  test("rejects utterances that merely contain sovereign mid-sentence", () => {
    expect(matchesSpeechWakePrefix("Can you tell Sovereign to send that")).toBe(false);
    expect(matchesSpeechWakePrefix("I said Sovereign in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePrefix("that was Sovereign talking")).toBe(false);
  });
...
describe("matcher normalization (shared by both matchers)", () => {
  ...
  test("collapses stacked punctuation and multiple spaces", () => {
    expect(matchesSpeechWakePhrase("Sovereign... stop!!")).toBe(true);
    expect(matchesSpeechWakePrefix("hey   sovereign    play   music")).toBe(true);
  });

  test("is not fooled by mid-word 'sovereign' substrings", () => {
    // "starsovereign" is one token; without a word boundary it must not match.
    expect(matchesSpeechWakePrefix("starsovereign")).toBe(false);
    expect(matchesSpeechWakePhrase("starsovereign stop")).toBe(false);
  });
});
...
describe("planContainsWakeFlip — regression boundary for the mid-turn cooldown stamp bug", () => {
  ...
  // and a later sentence introduces "Sovereign", the daemon flips containsWake
