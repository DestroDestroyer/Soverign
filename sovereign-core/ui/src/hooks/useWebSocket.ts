...
export type VoiceCallbacks = {
   ...
   *  "Sovereign". UI uses it to suppress the wake-word recognizer for the
   *  duration of the playback so TTS doesn't self-trigger via mic echo. */
  onTTSStart: (requestId: string, containsWake: boolean) => void;
  /** Mid-turn flip: a later sentence in the same turn contains "Sovereign". */
...
function createSidecarNotice(payload: SidecarEventPayload, timestamp?: number): ChatMessage & { notice?: SystemNotice } {
  ...
  const notice: SystemNotice = {
    ...
    text: reason
      ? `Sovereign sidecar disconnected: ${reason}. Dashboard features may be delayed until it reconnects.`
      : "Sovereign sidecar disconnected. Dashboard features may be delayed until it reconnects.",
    level: "warning",
  };
...
const TASK_EVENT_CAP = 100;
const CONTENT_EVENT_CAP = 100;
...
export function useWebSocket() {
  ...
  const connect = useCallback(() => {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          if (data.messages && data.messages.length > 0) {
            ...
            setMessages((prev) => prev.length === 0 ? restored.slice(-MESSAGE_CAP) : prev);
          }
        }
      } catch (err) {
    ...
    ws.onclose = () => {
      ...
      const delay = Math.min(1000 * Math.pow(2, reconnectAttemptsRef.current) + Math.random() * 1000, 30000);
      reconnectAttemptsRef.current++;
      console.log(`[WS] Disconnected, reconnecting in ${Math.round(delay)}ms (attempt ${reconnectAttemptsRef.current})...`);
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (msg.type === "tts_text") {
          // Mid-turn signal that an upcoming sentence contains "Sovereign" —
  ...
  const handleMessage = useCallback((msg: WSMessage) => {
    if (msg.type === "chat" && msg.payload?.source) {
      ...
      if (payload.source === "task_update" && payload.task && payload.action) {
        ...
        setTaskEvents((prev) => [...prev.slice(-(TASK_EVENT_CAP - 1)), event]);
        ...
        setContentEvents((prev) => [...prev.slice(-(CONTENT_EVENT_CAP - 1)), event]);
        ...
        setMessages((prev) => [...prev.slice(-(MESSAGE_CAP - 1)), noticeMessage]);
