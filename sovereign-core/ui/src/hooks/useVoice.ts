...
function getWakePrefix(normalized: string): "hey sovereign " | "sovereign " | null {
  if (normalized.startsWith("hey sovereign ")) return "hey sovereign ";
  if (normalized.startsWith("sovereign ")) return "sovereign ";
  return null;
}

/**
 * Strict matcher used during TTS playback (voiceState === "speaking").
 * Accepts bare wake phrases ("sovereign", "hey sovereign") or wake + a short
 * whitelisted interrupt command. Prevents Sovereign's own TTS from self-triggering
 * when the reply contains the word "sovereign" inside a sentence.
 */
export function matchesSpeechWakePhrase(transcript: string): boolean {
  const normalized = normalizeTranscript(transcript);
  if (!normalized) return false;
  if (normalized === "sovereign" || normalized === "hey sovereign") return true;
 ...
 * wake phrase, so natural "hey sovereign turn off the lights" wakes in one breath
 * without waiting for Chrome to emit a bare-wake interim first.
 * NOT safe to use while Sovereign is speaking — use matchesSpeechWakePhrase there.
 */
...
export function shouldSpeechWakeBeRunning(inputs: {
  ...
  // so "Sovereign" can interrupt mid-thought, not just kick off from idle.
...
export type UseVoiceReturn = {
  ...
  /** Mid-turn flip: a later sentence in the same TTS turn contains "Sovereign". */
...
export function useVoice({ wsRef, wakeWordEnabled = true, wakeEngine = "openwakeword", getCurrentRoom, speakingTailCooldownMs = 700 }: UseVoiceOptions): UseVoiceReturn {
  ...
  // True if the in-flight TTS turn contains "Sovereign" anywhere. The
  ...
  // stays running so a real human "Sovereign" can interrupt the reply.
  ...
  const startWakeWordEngine = useCallback(async () => {
    ...
    try {
      ...
      console.log("[Voice] Wake word engine ready — say 'Hey SOVEREIGN'");
    } catch (err) {
  ...
  const startSpeechWakeIfNeeded = useCallback((): void => {
    ...
    if (!speechWakeRef.current) {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        // so "Sovereign" can interrupt mid-thought.
        ...
        // During speaking with "Sovereign" in the TTS text: ignore wake
        ...
        // loose prefix matcher when idle so "hey sovereign <command>" wakes in one breath.
    ...
    try {
      ...
      console.log("[Voice] Speech wake recognizer starting — say 'Sovereign' or 'Hey Sovereign'");
    } catch {
  ...
  // "Sovereign" through speakers would self-trigger via the mic.
  useEffect(() => {
