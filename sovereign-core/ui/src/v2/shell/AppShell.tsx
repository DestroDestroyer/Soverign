...
function AppShellLive() {
  ...
  // shouldn't have to wait for Sovereign to finish thinking to talk over it.
  ...
  const latestAssistantReply = useMemo(() => {
    for (let i = live.items.length - 1; i >= 0; i--) {
      ...
      if (item.kind === "sovereign-speech" && item.text) {
        return {
          text: item.text,
  ...
  return (
    ...
    >
      ...
      <ShellLayout
        ...
        composerPlaceholder={
          live.isConnected
            ? "Ask Sovereign, or press / to summon a tool…"
            : "Waiting for daemon…"
        }
        onSubmit={(text) =>
