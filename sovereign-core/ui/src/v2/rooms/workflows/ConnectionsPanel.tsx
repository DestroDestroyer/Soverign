/**
 * Connections panel: list `app_connection` rows + registered Sovereign sources,
...
export function ConnectionsPanel(): React.ReactElement {
  ...
  return (
    <div className="wf-conn">
      <header className="wf-conn__header">
        <div>
          ...
          <p className="wf-conn__subtitle">
            {conn.loading
              ? "loading…"
              : `${conn.connections.length} stored · ${conn.sovereignSources.length} Sovereign source${conn.sovereignSources.length === 1 ? "" : "s"} registered`}
      ...
      {conn.sovereignSources.length > 0 ? (
        <div className="wf-conn__sources">
          <span className="wf-conn__sources-label">Reusable Sovereign credentials:</span>
          {conn.sovereignSources.map((s) => (
            <Chip key={s.id} tone="ok">
              <code>sovereign:{s.id}</code>
            </Chip>
          ))}
