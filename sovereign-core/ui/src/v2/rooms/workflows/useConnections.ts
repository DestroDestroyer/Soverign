 ...
 * exposes the list + registered Sovereign sources, and offers add/delete
...
export interface SovereignSourceMeta {
  /** Source id, e.g. "google". External ids it claims look like `sovereign:<id>` or `sovereign:<id>:*`. */
  id: string;
}
...
export interface ConnectionsState {
  ...
  sovereignSources: SovereignSourceMeta[];
...
export function useConnections(): ConnectionsState {
  ...
  const [sovereignSources, setSovereignSources] = useState<SovereignSourceMeta[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setSovereignSources(body.sovereignSources);
    } catch (e) {
  ...
  return { loading, error, connections, sovereignSources, refresh, create, update, remove };
}
