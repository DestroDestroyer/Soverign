 ...
 *      (Sovereign extension to AP) or `trigger.sampleData` (upstream-native).
 ...
 * 3 covers every Sovereign event taxonomy and the upstream piece samples we
 * ship.
 */
 ...
 * a downstream JSON.stringify / sovereign-tool input. Keeping the parent row
...
export function lookupDeclaredOutput(
  ...
  if (step.type === "PIECE_TRIGGER") {
    ...
    // value (sovereign-trigger:on_event payload depends on `eventType`).
