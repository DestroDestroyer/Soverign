 ...
 *     mode per primitive).
 *   - event: the Sovereign-side `@jarvispieces/piece-jarvis-trigger`
...
export const TRIGGER_KINDS: ReadonlyArray<TriggerKindMeta> = [
  ...
  },
  {
    kind: "event",
    label: "Event",
    description: "Fire on a Sovereign event (clipboard, email, awareness, ...)",
 ...
 * (e.g. if `sovereign-trigger` grew a second trigger and we reshuffled
