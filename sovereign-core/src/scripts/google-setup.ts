 ...
 *
 * Interactive CLI to authenticate SOVEREIGN with Google APIs.
 * Opens browser to Google consent screen, handles callback,
 * and saves tokens to ~/.sovereign/google-tokens.json.
 *
...
async function main() {
  console.log('');
  console.log('=== Google OAuth2 Setup for SOVEREIGN ===');
  console.log('');
  ...
  if (!clientId || !clientSecret) {
    ...
    console.log('');
    console.log('Add the following to your ~/.sovereign/config.yaml:');
    console.log('');
    console.log('google:');
  ...
  if (auth.isAuthenticated()) {
    console.log('Already authenticated! Tokens exist at ~/.sovereign/google-tokens.json');
    ...
    async fetch(req) {
      ...
      if (url.pathname === '/api/auth/google/callback') {
        ...
        try {
          ...
          console.log(`Tokens saved to ~/.sovereign/google-tokens.json`);
          ...
          return new Response(
            '<html><body><h1>SOVEREIGN Google Authorization Complete!</h1><p>Tokens saved. You can close this tab.</p></body></html>',
            { headers: { 'Content-Type': 'text/html' } }
          );
        } catch (err) {
