/**
 * Example usage of Sovereign Communication Layer
 *
...
async function main() {
  console.log('🤖 Starting Sovereign Communication Layer...\n');
  ...
  const simulatedStream = (async function* () {
    ...
    await new Promise(r => setTimeout(r, 100));
    yield { type: 'text' as const, text: 'Sovereign!' };
    await new Promise(r => setTimeout(r, 100));
    yield { type: 'done' as const, response: { content: 'Hello from Sovereign!', tool_calls: [], usage: { input_tokens: 0, output_tokens: 10 }, model: 'demo', finish_reason: 'stop' as const } };
  })();
