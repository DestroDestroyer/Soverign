# Sovereign Configuration System

Type-safe YAML configuration management for Project Sovereign
...
Default location: `~/.sovereign/config.yaml`
...
// Load from default location (~/.sovereign/config.yaml)
...
daemon: {
  ...
  data_dir: string;      // Data directory path (default: ~/.sovereign)
  db_path: string;       // SQLite database path (default: ~/.sovereign/sovereign.db)
}
```
...
Core personality traits that guide Sovereign behavior.
...
1. **Copy Example Config**:
   ```bash
   mkdir -p ~/.sovereign
   cp config.example.yaml ~/.sovereign/config.yaml
   ```

2. **Edit Configuration**:
   ```bash
   nano ~/.sovereign/config.yaml
   ```
...
1. **Never commit API keys**: Add `~/.sovereign/config.yaml` to `.gitignore`
...
```typescript
import type { SovereignConfig } from './config/index.ts';

function validateConfig(config: SovereignConfig): boolean {
