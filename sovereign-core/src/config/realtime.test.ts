...
function makeConfig(overrides: Partial<SovereignConfig> = {}): SovereignConfig {
  return { ...structuredClone(DEFAULT_CONFIG), ...overrides };
}

function withOpenAIProvider(config: SovereignConfig, key: string): SovereignConfig {
...
describe('resolveRealtimeVoice', () => {
  ...
  test('does not fall back to env vars - key must come from a configured provider', () => {
    ...
    const config = makeConfig();
    config.voice!.realtime = { enabled: true };
    const prevSovereign = process.env.SOVEREIGN_OPENAI_KEY;
    const prevOpenAI = process.env.OPENAI_API_KEY;
    process.env.SOVEREIGN_OPENAI_KEY = 'env-key';
    ...
    try {
      ...
      if (prevSovereign === undefined) delete process.env.SOVEREIGN_OPENAI_KEY; else process.env.SOVEREIGN_OPENAI_KEY = prevSovereign;
