/**
 * Process Lock Manager for Sovereign Daemon
 *
...
const LOG_DIR = join(SOVEREIGN_DIR, 'logs');
const LOCK_PATH = join(SOVEREIGN_DIR, 'sovereign.pid');
const LOG_PATH = join(LOG_DIR, 'sovereign.log');

/**
 * Resolve the lock-file path for a given data dir. The daemon itself always
 * runs against `~/.sovereign` (no override path today), but the encryption
...
export function lockPathFor(dataDir?: string): string {
  ...
  return join(dataDir, 'sovereign.pid');
}
...
export function acquireLock(pid: number): boolean {
  if (process.platform === 'win32') {
    try {
      mkdirSync(SOVEREIGN_DIR, { recursive: true });
 ...
 * default probes `~/.sovereign/sovereign.pid`.
 */
 ...
 * always targets the default `~/.sovereign/sovereign.pid` and is only used by the
 ...
 * daemon itself after port resolution so `sovereign stop` knows exactly which
