  ...
  private async handleChat(msg: WSMessage, ws?: ServerWebSocket<unknown>): Promise<WSMessage | void> {
    ...
    if (this.isSetupMode()) {
      return {
        ...
        payload: {
          code: 'setup_required',
          message: 'Finish first-run setup before chatting with Sovereign.',
        },
        id: msg.id,
        timestamp: Date.now(),
      };
...
1. **Explicit existing-project name.** If the user named an existing project (e.g. "in sovereign-landing", "edit the SP500 calculator"), use that one.
2. **Name keyword match.** If the user did NOT ask for a new project AND did NOT name one explicitly, but their request shares a meaningful keyword with an existing project's name (request mentions "landing page" + "sovereign-landing" exists → use sovereign-landing; request mentions "calculator" + "SP500 ROI Calculator" exists → use that), pick that project.
...
- Do NOT fabricate file paths, line counts, or "your file is now at X" descriptions without a successful site_write_file call backing them.`;
  ...
  private getRealtimeBudget(): RealtimeBudgetTracker {
    if (!this.realtimeBudget) {
      const dataDir = this.agentService.getConfig().daemon?.data_dir || join(homedir(), '.sovereign');
  ...
  private async handleInterviewMessage(
    ...
    // "Sovereign is thinking…" forever with the composer disabled.
    ...
    if (willSpeak && this.ttsProvider) {
      ...
      try {
        this.wsServer.sendToClient(ws, {
          ...
          // are short and we don't need "Sovereign" interrupts here.
...
function ackForRoomAction(ra: { room: string; action: string; args?: Record<string, unknown> }): string {
  ...
  switch (ra.action) {
    ...
    case 'enable_telegram':
      return `Enabling Telegram. Restart Sovereign to apply.`;
    case 'disable_telegram':
      return `Disabling Telegram. Restart Sovereign to apply.`;
    case 'enable_discord':
      return `Enabling Discord. Restart Sovereign to apply.`;
    case 'disable_discord':
      return `Disabling Discord. Restart Sovereign to apply.`;
    case 'set_stt_provider':
      return a.provider
        ? `Setting STT to ${String(a.provider)}. Restart Sovereign to apply.`
    ...
    case 'restart_daemon':
      return `Restarting Sovereign. The dashboard will reconnect in a few seconds.`;
