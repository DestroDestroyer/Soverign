...
Core daemon infrastructure for the SOVEREIGN system.
...
```bash
# Run with defaults (port 3142, ~/.sovereign data directory)
bun run src/daemon/index.ts

# Run with custom configuration
bun run src/daemon/index.ts --port 3142 --data-dir ~/my-sovereign-data
...
| `--data-dir` | `~/.sovereign` | Data directory |
| `--db-path` | `~/.sovereign/sovereign.db` | Database file path |
...
```
[2026-02-22T22:17:03.230Z] Initializing database at /home/user/.sovereign/sovereign.db
...
- **Location**: `~/.sovereign/sovereign.db` (configurable)

Schema is managed by `~/sovereign/src/vault/schema.ts`.
...
timeout 3 bun run src/daemon/index.ts --data-dir /tmp/test-sovereign
```
