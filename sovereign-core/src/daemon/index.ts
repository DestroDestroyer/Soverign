 ...
 * Sovereign Daemon
 *
 * Main entry point for the SOVEREIGN daemon process.
...
// Constants
const DEFAULT_PORT = 3142;  // SOVEREIGN port
...
Example:
  bun run src/daemon/index.ts --port 3142 --data-dir ~/.sovereign
        `);
        process.exit(0);
    }
  }
...
export async function startDaemon(userConfig?: Partial<DaemonConfig>): Promise<void> {
  ...
  try {
    ...
    console.error('[Daemon] Fix the YAML syntax in ~/.sovereign/config.yaml or delete it to use defaults.\n');
    process.exit(1);
  }
  ...
  // If user specified a custom data dir but no db path, use sovereign.db in that dir
  const dbPath = userConfig?.dbPath ?? sovereignConfig.daemon.db_path ?? path.join(dataDir, 'sovereign.db');

  // Merge configuration
  const port = userConfig?.port ?? sovereignConfig.daemon.port ?? DEFAULT_PORT;
  ...
  // Record the actual bound port in the lockfile so `sovereign stop` knows which
  // port to verify even when the daemon was started with --port, SOVEREIGN_PORT,
  ...
  try {
    ...
    // workflow_job / trigger_event) to the shared Sovereign DB. Idempotent.
    ...
    const sidecarManager = new SidecarManager(sovereignConfig.daemon.data_dir.replace('~', os.homedir()));
    ...
    const brainSource: 'env' | 'config' | 'default' = process.env.SOVEREIGN_BRAIN_DOMAIN
      ? 'env'
      : sovereignConfig.daemon.brain_domain
        ? 'config'
        : 'default';
    ...
    if (sovereignConfig.tts?.enabled) {
      ...
      const ttsProvider = createTTSProvider(sovereignConfig.tts);
      if (ttsProvider) {
        wsService.setTTSProvider(ttsProvider);
        console.log(`[Daemon] TTS enabled: ${sovereignConfig.tts.voice ?? 'en-US-AriaNeural'}`);
      }
    }
    ...
    if (sovereignConfig.stt) {
      ...
      const sttProvider = createSTTProvider(sovereignConfig.stt);
      if (sttProvider) {
        wsService.setSTTProvider(sttProvider);
        console.log(`[Daemon] STT for voice input: ${sovereignConfig.stt.provider}`);
      }
    }
    ...
    // canonical PieceCatalog. /v1/sovereign/* service backends are wired below
    ...
    // Build the credential resolver early so we can register Sovereign-managed
    // sources (Google OAuth file, etc.) before pieces start asking for
    // connections. Each `sovereign:<source>` external id dispatches to the
    ...
    if (googleAuth) {
      ...
      logWithTimestamp(
        "Workflow credential resolver: registered sovereign:google source",
      );
    }
    // sovereign:telegram bridges the same bot token the daemon's Telegram
    // adapter is using -- pieces (activepieces telegram-bot, custom flows)
    // can reference `sovereign:telegram` instead of asking the user to create
    ...
    if (sovereignConfig.channels?.telegram?.enabled && sovereignConfig.channels.telegram.bot_token) {
      ...
      logWithTimestamp(
        "Workflow credential resolver: registered sovereign:telegram source",
      );
    }
    try {
      ...
      console.error(
        [
          ...
          `           - Disk full or ~/.sovereign/cache unwriteable`,
    ...
    if (toolRegistry) {
      ...
      // legacy `SovereignLlmClient`/`SovereignToolRegistryAdapter` classes since
      ...
      const composerToolRegistry = toolRegistry
        ? {
            ...
            // correct `params` and reject a sovereign-tool:invoke step that omits a
      ...
      const manageWorkflowTool = createManageWorkflowTool({
        ...
        // Gmail/Calendar via Sovereign tools).
    ...
    // 10.1. Workflow runtime: wire `/v1/sovereign/*` service backends and start
    ...
    if (workflowSandboxApi && workflowEngineRuntime) {
      ...
      // `sovereign-agent.delegate` runs the full sub-agent loop (LLM + tool
      ...
      const backends = buildSandboxServiceBackends({
        ...
        auditTrail,
        emergencyController,
        // Give the sovereign-ask piece the same Sovereign-flavoured system
        // prompt the chat agent uses, so workflow LLM calls answer as
        // Sovereign rather than as the bare base model. `"workflow"` is the
    ...
    const startAwarenessService = async (): Promise<void> => {
      ...
      try {
              ...
              if (errorText.length > 5) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    sendDesktopNotification(`SOVEREIGN: Fix for ${appName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
              ...
              if (compositeScore >= 0.7 && (appCategory === 'code_editor' || appCategory === 'terminal')) {
                ...
                bgAgent.handleMessage(
                  ...
                  if (solution && solution.length > 10) {
                    ...
                    wsService.broadcastNotification(solutionText, 'urgent');
                    sendDesktopNotification(`SOVEREIGN: Help for ${sAppName}`, solution.slice(0, 200), { urgency: 'critical', expireMs: 15000 });
                    const voiceText = solution
                      .replace(/#{1,6}\s*/g, '')
                      .replace(/\*{1,2}([^*]+)\*{1,2}/g, '$1')
                      .replace(/`([^`]+)`/g, '$1')
        ...
        if (sovereignConfig.awareness?.overlay_autolaunch !== false) {
          try {
    ...
    logWithTimestamp(`SOVEREIGN daemon running on port ${config.port}`);
    console.log('');
