...
export class AgentService implements Service, IAgentService {
  ...
  private registerProviders(): void {
    ...
    if (this.llmManager.hasConversationTier()) {
      ...
      // gating, and Sovereign-specific feature knowledge. Uses processTaskCall
  ...
  private buildPersona(): string {
    ...
    const name = p?.assistant_name ?? 'SOVEREIGN';
    return [
   ...
   * paths (workflow's `sovereign-ask` piece, etc.) can give the LLM the same
   * Sovereign identity + role + personality + vault context that chat gets.
   *
   ...
   * live-voice framing (~100 tokens). Tools stay available, so SOVEREIGN can still
  ...
  buildRealtimeVoiceInstructions(): string {
    const name = this.config.personality?.assistant_name?.trim() || this.role?.name || 'SOVEREIGN';
