...
const READY_SIGNAL = join(tmpdir(), 'sovereign-test-lock-ready');
const HOLDER_SCRIPT = join(tmpdir(), 'sovereign-test-lock-holder.ts');
...
describe('Process Lock Manager', () => {
  ...
  describe('acquireLock', () => {
    ...
    test('creates ~/.sovereign dir if missing', () => {
  ...
  describe('path getters', () => {
    test('getPidPath returns ~/.sovereign/sovereign.pid', () => {
      expect(getPidPath()).toBe(join(homedir(), '.sovereign', 'sovereign.pid'));
    });

    test('getLogPath returns path and creates logs dir', () => {
      const logPath = getLogPath();
      expect(logPath).toBe(join(SOVEREIGN_DIR, 'logs', 'sovereign.log'));
      expect(existsSync(join(SOVEREIGN_DIR, 'logs'))).toBe(true);
    });

    test('getLogDir returns logs directory', () => {
      expect(getLogDir()).toBe(join(SOVEREIGN_DIR, 'logs'));
    });
  });
