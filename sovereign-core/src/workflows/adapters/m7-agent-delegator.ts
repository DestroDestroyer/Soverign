/**
 * `M7AgentDelegator` -- backs `sovereign-agent.delegate` with the full M7 sub-agent
 ...
 * action should expose it via a separate piece (e.g., `sovereign-tool` or
 * `sovereign-notify` + waitpoint), not piggy-back on the sub-agent.
 */
