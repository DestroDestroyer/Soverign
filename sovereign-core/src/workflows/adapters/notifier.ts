/**
 * Adapter: PieceNotifier over Sovereign' ChannelService + WebSocketService +
 ...
 *                  dropped here so a vanilla Sovereign without telegram doesn't
...
export interface NotifierDeps {
  ...
  /**
   * Optional voice / TTS surface. When provided, `sovereign-notify` calls with
   ...
   * connected (e.g. `{"telegram"}` on a Sovereign with only telegram wired).
...
export class SovereignNotifierAdapter implements PieceNotifier {
...
function expandChannels(requested: PieceNotifyChannel[], deps: NotifierDeps): Set<string> {
  ...
  for (const c of requested) {
    if (c === "auto") {
      ...
      // Sovereign without telegram/discord would report two failures the
      // user can't act on.
      out.add("dashboard");
...
function titleForPriority(p: "urgent" | "normal" | "low"): string {
  if (p === "urgent") return "Sovereign (urgent)";
  return "Sovereign";
}
