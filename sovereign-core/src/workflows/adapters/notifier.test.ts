/**
 * `SovereignNotifierAdapter` covers the four channels the workflow notifier
...
import { SovereignNotifierAdapter, type NotifierDeps } from "./notifier";
...
describe("SovereignNotifierAdapter: voice channel", () => {
  test("delivers via sendVoice when wired", async () => {
    const spoken: string[] = [];
    const adapter = new SovereignNotifierAdapter(
      makeDeps({
        sendVoice: async (t) => {
          spoken.push(t);
        },
  ...
  test("reports failed when sendVoice isn't wired", async () => {
    const adapter = new SovereignNotifierAdapter(makeDeps());
...
describe("SovereignNotifierAdapter: channel expansion", () => {
  ...
  test('"auto" only fans out to *connected* external channels when the dep is wired', async () => {
    // Sovereign with only telegram wired -> auto goes to dashboard + telegram,
  ...
  test('"auto" on a Sovereign with no external channels connected only delivers to dashboard', async () => {
    let m8Called = false;
    const adapter = new SovereignNotifierAdapter(
      makeDeps({
