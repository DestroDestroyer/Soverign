 ...
 * `/v1/sovereign/agent/delegate` and surface the agent's final message + tool-call
...
export const delegateAction = createAction({
  name: "delegate",
  displayName: "Delegate to a Sovereign sub-agent",
  description:
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/sovereign/agent/delegate";
    const goal = context.propsValue["goal"];
    if (typeof goal !== "string" || goal.length === 0) {
      throw new Error("sovereign-agent: goal is required and must be a non-empty string");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `sovereign-agent: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }
