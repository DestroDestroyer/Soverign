/**
 * `ask` action -- POST to the daemon's `/v1/sovereign/llm/chat` endpoint with
...
export const askAction = createAction({
  ...
  props: {
    ...
    system: Property.LongText({
      displayName: "System",
      description:
        "Extra system instructions APPENDED to the standard Sovereign system prompt. Use this to bias the reply (e.g. \"answer in JSON\"). Turn on Override to replace the Sovereign prompt entirely instead.",
      required: false,
    }),
    overrideSystem: Property.Checkbox({
      displayName: "Override Sovereign system prompt",
      description:
        "When ON, the System field above becomes the ONLY system prompt -- Sovereign's identity, role, personality, and vault context are not sent. Use for generic LLM tasks (text transforms, summarisation of plain inputs) where Sovereign context would bias the reply. Leave OFF (default) when you want the model to answer as Sovereign.",
      required: false,
      defaultValue: false,
    }),
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/sovereign/llm/chat";
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `sovereign-ask: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }
