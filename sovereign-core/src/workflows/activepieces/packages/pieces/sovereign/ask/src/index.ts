/**
 * `@sovereignpieces/piece-sovereign-ask` -- the production "ask the LLM" piece,
 * ported from the legacy `SovereignPiece` interface to upstream's `createPiece`.
 *
 * The action calls back to the daemon's `/v1/sovereign/llm/chat` endpoint via
...
export const sovereignAskPiece = createPiece({
