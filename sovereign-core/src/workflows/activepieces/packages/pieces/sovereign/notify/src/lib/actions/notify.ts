 ...
 * `/v1/sovereign/notify` and surface the delivery report.
 *
...
export const notifyAction = createAction({
  ...
  description:
    "Deliver a message through Sovereign's configured channels. Use 'auto' to let Sovereign pick a sensible default given the priority.",
  ...
  props: {
    ...
    channels: Property.StaticMultiSelectDropdown({
      ...
      description:
        "Empty / [auto] lets Sovereign pick a default fan-out across the user's connected channels.",
      required: false,
      defaultValue: ["auto"],
      options: {
        disabled: false,
  ...
  async run(context) {
    const url = trimSlash(context.server.apiUrl) + "/v1/sovereign/notify";
    const message = context.propsValue["message"];
    if (typeof message !== "string" || message.length === 0) {
      throw new Error("sovereign-notify: message is required and must be a non-empty string");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `sovereign-notify: daemon responded ${response.status}: ${text.slice(0, 500)}`,
      );
    }
