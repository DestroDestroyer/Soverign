 ...
 *   - No handler-side authn/authz: Sovereign is single-tenant, the dashboard is
...
export interface CreateWorkflowRoutesOptions {
  ...
  /**
   * Optional piece catalog. Either the legacy `SovereignPieceRegistry` (during
   ...
   * report which `SovereignConnectionSource` adapters are registered (e.g.
   * `sovereign:google` is wired) so the dashboard's piece-side auth picker
   * can highlight reusable Sovereign-managed credentials. The repo-backed
   ...
   *
   * When omitted, install/uninstall still mutate `~/.sovereign/pieces/` and the
 ...
 * race on the shared `~/.sovereign/pieces/package.json` + bun-install
...
export function createWorkflowRoutes(opts: CreateWorkflowRoutesOptions = {}): WorkflowRouteMap {
  ...
  return {
        ...
        trapErrors(() => {
          ...
          const list = opts.pieceRegistry.list().map((p) => ({
            ...
            actions: Object.values(p.actions).map((a) => ({
              ...
              // Optional declared output sample (Sovereign extension to AP).
            ...
            triggers: p.triggers
              ? Object.values(p.triggers).map((t) => ({
                  ...
                  // Dynamic-output triggers (sovereign-trigger:on_event):
    ...
    // *curated* (Sovereign-vetted) community piece the user can opt into
    ...
    //
    // Install / uninstall mutate `~/.sovereign/pieces/` (manifest + bun
    ...
    // Event-buffer overflow signal for the `sovereign-trigger:on_event`
    ...
    // ------------------------------------------------------------- connections
    // CRUD over `app_connection` rows + a list of registered Sovereign
    ...
    "/api/workflows/connections": {
      GET: () =>
        trapErrors(() => {
          ...
          return ok({ connections: list, sovereignSources: sources });
        }),
      POST: (req) =>
        trapErrors(async () => {
