/**
 * Pieces installer + manifest. Manages `~/.sovereign/pieces/`:
 *
...
const ROOT_ENV = "SOVEREIGN_PIECES_DIR";

/** Default `~/.sovereign/pieces`. Override with `SOVEREIGN_PIECES_DIR` for tests. */
export function piecesBaseDir(): string {
  const override = process.env[ROOT_ENV];
  if (override) return resolve(override);
  return resolve(homedir(), ".sovereign", "pieces");
}

export interface InstalledPiece {
  /** Sovereign-side id; matches the catalog entry's `id`. */
  id: string;
