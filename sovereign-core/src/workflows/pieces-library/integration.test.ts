 ...
 * `require()`d. Skipped unless `SOVEREIGN_TEST_PIECES_LIBRARY=1` because it
 ...
 *     the engine-end-to-end suite for Sovereign-native pieces.
 *
...
const ENABLED = process.env.SOVEREIGN_TEST_PIECES_LIBRARY === "1";
...
beforeEach(() => {
  if (!ENABLED) return;
  tempDir = mkdtempSync(join(tmpdir(), "sovereign-pieces-integration-"));
});
...
describe("pieces-library integration (gated on SOVEREIGN_TEST_PIECES_LIBRARY=1)", () => {
    ...
    test.skipIf(!ENABLED)(
      ...
      async () => {
        ...
        writeFileSync(
          ...
          JSON.stringify({
            name: "sovereign-pieces-integration",
            private: true,
            type: "commonjs",
