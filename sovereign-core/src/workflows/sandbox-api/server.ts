 ...
 *
 * Plus a parallel set of `/v1/sovereign/*` endpoints for our own ported pieces
 * (added in F-H).
 *
...
import {
  createSovereignLlmChatRoute,
  type LlmChatFn,
} from "./routes/sovereign-llm";
import {
  createSovereignToolsInvokeRoute,
  type ToolsInvokeFn,
} from "./routes/sovereign-tools";
import {
  createSovereignNotifyRoute,
  type NotifyFn,
} from "./routes/sovereign-notify";
import {
  createSovereignContextVaultSearchRoute,
  createSovereignContextVaultGetEntityRoute,
  createSovereignContextAwarenessRecentRoute,
  createSovereignContextCommitmentsListRoute,
  type SovereignContextProvider,
} from "./routes/sovereign-context";
import {
  createSovereignAgentDelegateRoute,
  type AgentDelegateFn,
} from "./routes/sovereign-agent";
import {
  createSovereignEventsPollRoute,
  type EventsPollFn,
} from "./routes/sovereign-events";
import {
  createSovereignWorkflowsStartRoute,
  type WorkflowsStartFn,
} from "./routes/sovereign-workflows";
...
export interface SandboxApiServices {
  ...
  /**
   * LLM chat backend for the `sovereign-ask` piece. If unset, the endpoint
   * returns 503; the daemon wires this in at startup with the LLMManager.
   */
  llmChat?: LlmChatFn;
  /**
   * Tool invocation backend for the `sovereign-tool` piece. If unset, the
   * endpoint returns 503. The daemon wires this in with `ToolRegistry`.
   */
  toolsInvoke?: ToolsInvokeFn;
  /**
   * Notification backend for the `sovereign-notify` piece. If unset, returns 503.
   */
  notify?: NotifyFn;
  /**
   * Read-only Sovereign state provider for the `sovereign-context` piece. If unset,
   * each context route returns 503.
   */
  contextProvider?: SovereignContextProvider;
  /**
   * Sub-agent delegation backend for the `sovereign-agent` piece. If unset, the
   * endpoint returns 503.
   */
  agentDelegate?: AgentDelegateFn;
  /**
   * Recent-events poll backend for the `sovereign-trigger` `on_event` trigger.
   * If unset, returns 503.
   */
  eventsPoll?: EventsPollFn;
  /**
   * Workflow start backend for the `sovereign-trigger` `run_workflow` action.
   * If unset, returns 503.
   */
...
export class SandboxApi {
  ...
  /**
   * Patch service backends after construction. Each `/v1/sovereign/*` route's
  ...
  private registerRoutes(): void {
    this.routes.push(
      ...
      // /v1/sovereign/* routes pass `this.services` itself (not a snapshot) so
      ...
      {
        path: "/v1/sovereign/llm/chat",
        method: "POST",
        handler: createSovereignLlmChatRoute(this.services),
      },
      {
        path: "/v1/sovereign/tools/invoke",
        method: "POST",
        handler: createSovereignToolsInvokeRoute(this.services),
      },
      {
        path: "/v1/sovereign/notify",
        method: "POST",
        handler: createSovereignNotifyRoute(this.services),
      },
      {
        path: "/v1/sovereign/context/vault-search",
        method: "POST",
        handler: createSovereignContextVaultSearchRoute(this.services),
      },
      {
        path: "/v1/sovereign/context/vault-get-entity",
        method: "POST",
        handler: createSovereignContextVaultGetEntityRoute(this.services),
      },
      {
        path: "/v1/sovereign/context/awareness-recent",
        method: "POST",
        handler: createSovereignContextAwarenessRecentRoute(this.services),
      },
      {
        path: "/v1/sovereign/context/commitments-list",
        method: "POST",
        handler: createSovereignContextCommitmentsListRoute(this.services),
      },
      {
        path: "/v1/sovereign/agent/delegate",
        method: "POST",
        handler: createSovereignAgentDelegateRoute(this.services),
      },
      {
        path: "/v1/sovereign/events/poll",
        method: "POST",
        handler: createSovereignEventsPollRoute(this.services),
      },
      {
        path: "/v1/sovereign/workflows/start",
        method: "POST",
        handler: createSovereignWorkflowsStartRoute(this.services),
      },
    );
  }
