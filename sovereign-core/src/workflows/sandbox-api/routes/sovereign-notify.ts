/**
 * `/v1/sovereign/notify` -- backs the `sovereign-notify` piece's `notify` action.
 *
...
export interface SovereignNotifyRouteDeps {
  notify?: NotifyFn;
}

export function createSovereignNotifyRoute(
  deps: SovereignNotifyRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.notify) {
      return err("sovereign notify not configured", 503);
    }
