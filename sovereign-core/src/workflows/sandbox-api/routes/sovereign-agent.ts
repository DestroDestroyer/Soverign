/**
 * `/v1/sovereign/agent/delegate` -- backs the `sovereign-agent` piece's `delegate`
...
export interface SovereignAgentRouteDeps {
  agentDelegate?: AgentDelegateFn;
}

export function createSovereignAgentDelegateRoute(
  deps: SovereignAgentRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.agentDelegate) {
      return err("sovereign agent.delegate not configured", 503);
    }
