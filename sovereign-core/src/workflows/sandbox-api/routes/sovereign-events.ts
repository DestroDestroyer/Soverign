/**
 * `/v1/sovereign/events/poll` -- backs the `sovereign-trigger` `on_event` polling
 * trigger.
 *
 * Two request shapes:
 *
...
export interface SovereignEventsRouteDeps {
  eventsPoll?: EventsPollFn;
}

export function createSovereignEventsPollRoute(
  deps: SovereignEventsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.eventsPoll) {
      return err("sovereign events.poll not configured", 503);
    }
