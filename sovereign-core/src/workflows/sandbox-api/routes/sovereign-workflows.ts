/**
 * `/v1/sovereign/workflows/start` -- backs the `sovereign-trigger` piece's
...
export interface SovereignWorkflowsRouteDeps {
  workflowsStart?: WorkflowsStartFn;
}

export function createSovereignWorkflowsStartRoute(
  deps: SovereignWorkflowsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.workflowsStart) {
      return err("sovereign workflows.start not configured", 503);
    }
    ...
    try {
      ...
      // Typed errors from SovereignWorkflowRunnerAdapter carry a `code`
