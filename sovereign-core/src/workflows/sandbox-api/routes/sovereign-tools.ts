/**
 * `/v1/sovereign/tools/invoke` -- backs the `sovereign-tool` piece's `invoke` action.
 *
...
export interface SovereignToolsRouteDeps {
  toolsInvoke?: ToolsInvokeFn;
}

export function createSovereignToolsInvokeRoute(
  deps: SovereignToolsRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.toolsInvoke) {
      return err("sovereign tools.invoke not configured", 503);
    }
