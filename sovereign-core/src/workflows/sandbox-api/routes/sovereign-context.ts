/**
 * `/v1/sovereign/context/*` -- backs the `sovereign-context` piece's four read-only
 * actions. One route per action so each can validate its own envelope; all
 * four delegate to a single injected `SovereignContextProvider` shaped exactly
 * like the legacy adapter, so the daemon can supply a `SovereignContextProviderAdapter`
 * verbatim.
 *
 * Routes:
 *   POST /v1/sovereign/context/vault-search
 *   POST /v1/sovereign/context/vault-get-entity
 *   POST /v1/sovereign/context/awareness-recent
 *   POST /v1/sovereign/context/commitments-list
 */
...
export interface SovereignContextProvider {
...
export interface SovereignContextRouteDeps {
...
export function createSovereignContextVaultSearchRoute(
  deps: SovereignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("sovereign context not configured", 503);
...
export function createSovereignContextVaultGetEntityRoute(
  deps: SovereignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("sovereign context not configured", 503);
...
export function createSovereignContextAwarenessRecentRoute(
  deps: SovereignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("sovereign context not configured", 503);
...
export function createSovereignContextCommitmentsListRoute(
  deps: SovereignContextRouteDeps,
): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.contextProvider) return err("sovereign context not configured", 503);
