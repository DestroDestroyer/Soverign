 ...
 *   1. `sovereign:*` external ids -> live Sovereign-managed credential sources
 ...
 *
 * Per CredentialResolver contract for Sovereign-managed Google connections, the
...
export function createConnectionsRoute(deps: ConnectionsRouteDeps): RouteHandler {
  return async (ctx: RouteContext) => {
    ...
    // needs the pieceName for app_connection lookups. For sovereign:* external
