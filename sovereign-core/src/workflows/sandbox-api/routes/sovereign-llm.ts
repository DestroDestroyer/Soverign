/**
 * `/v1/sovereign/llm/chat` -- backs the `sovereign-ask` piece's `ask` action.
 *
 ...
 *   - default                   : Sovereign identity + role + personality
 *   - `system` set              : Sovereign prompt + "\n\n" + `system`
 *   - `system` + overrideSystem : `system` only (Sovereign context dropped)
 *
...
export interface LlmChatRequest {
  ...
  system?: string;
  /**
   * When true, the `system` field replaces the Sovereign system prompt
   * entirely. When false / unset, `system` is appended to the Sovereign
   * prompt. Default off so the common case ("ask Sovereign to do X") still
   * carries the Sovereign identity.
   */
...
export interface SovereignLlmRouteDeps {
  /**
...
export function createSovereignLlmChatRoute(deps: SovereignLlmRouteDeps): RouteHandler {
  return async (ctx: RouteContext) => {
    if (!deps.llmChat) {
      return err("sovereign llm chat not configured", 503);
    }
