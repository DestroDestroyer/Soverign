 ...
 * legacy `sovereign-trigger:on_event` direct-subscribe path inside
 * `TriggerManager` (kept as a fallback when no engine runtime is wired) and
 * the workflow event buffer mirror used by `/v1/sovereign/events/poll`.
 *
 * Replaces the previous `SovereignEventBusAdapter` in `adapters/` (deleted with
...
export class WorkflowEventBus {
   ...
   * `/v1/sovereign/events/poll`, so engine-managed `on_event` triggers see the
  ...
  /** Publish from Sovereign daemon code. Errors in handlers are swallowed-and-logged. */
