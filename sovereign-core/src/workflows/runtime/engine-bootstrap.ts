 ...
 * bundle, ensures Sovereign-native pieces are compiled to dist/, starts a
...
export interface BootstrapWorkflowEngineOptions {
  /** Service backends for the `/v1/sovereign/*` routes. Each unset slot returns 503. */
   ...
   * `~/.sovereign/cache/piece-metadata.json`. Tests can pass their own path.
   */
  cacheFile?: string;
  /**
   * Optional override for the piece root dirs scanned during catalog build.
   * Default: the vendored Sovereign piece tree only. Future community pieces
   * will be added here.
   */
  pieceRoots?: string[];
}
...
const DEFAULT_CACHE_FILE = resolve(homedir(), ".sovereign", "cache", "piece-metadata.json");
...
export async function bootstrapWorkflowEngine(
  ...
  // `~/.sovereign/pieces/installed.json`. Empty manifest = no-op. Failures are
  ...
  const [cached] = await Promise.all([
    ...
    labelled("piece-compile", buildAllSovereignPieces()),
  ...
  //
  // customPiecesPaths includes the user pieces dir (`~/.sovereign/pieces`)
  // alongside the vendored Sovereign pieces. The engine's piece-loader walks
  ...
  //   1. vendored Sovereign-native pieces (always present)
  //   2. user-installed community pieces under `~/.sovereign/pieces/node_modules/@activepieces/`
  //      (may be missing; discoverPieces handles a missing dir gracefully)
  const pieceRoots = opts.pieceRoots ?? [
    resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/sovereign"),
