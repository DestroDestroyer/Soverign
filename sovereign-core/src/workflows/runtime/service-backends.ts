/**
 * Glue layer: wraps existing Sovereign adapters into the function-shape that the
 * SandboxApi service-backend slots expect. Each `/v1/sovereign/*` route takes a
...
import { SovereignLlmClient } from "../adapters/llm-client";
import { SovereignToolRegistryAdapter } from "../adapters/tool-registry";
import { SovereignNotifierAdapter, type NotifierDeps } from "../adapters/notifier";
import { SovereignContextProviderAdapter } from "../adapters/context-provider";
import { LlmOnlyAgentDelegator } from "../adapters/agent-delegator";
import { M7AgentDelegator } from "../adapters/m7-agent-delegator";
import { SovereignWorkflowRunnerAdapter } from "../adapters/workflow-runner";
import type { LlmChatFn } from "../sandbox-api/routes/sovereign-llm";
import type { ToolsInvokeFn } from "../sandbox-api/routes/sovereign-tools";
import type { NotifyFn } from "../sandbox-api/routes/sovereign-notify";
import type { SovereignContextProvider } from "../sandbox-api/routes/sovereign-context";
import type { AgentDelegateFn } from "../sandbox-api/routes/sovereign-agent";
import type { EventsPollFn } from "../sandbox-api/routes/sovereign-events";
import type { WorkflowsStartFn } from "../sandbox-api/routes/sovereign-workflows";
...
export interface BuildServiceBackendsOptions {
  ...
  /** Recent-events buffer for `sovereign-trigger:on_event` polling. */
  ...
  /**
   * M7 sub-agent dependencies. When all of these are supplied, `sovereign-agent.delegate`
  ...
  /**
   * Optional callback that builds the Sovereign-flavoured system prompt for
   * a workflow LLM call. When set, the `sovereign-ask` piece will pass this
   * prompt to the LLM so the model knows it's Sovereign (role, personality,
   ...
   */
  buildSovereignSystemPrompt?: (userMessage: string) => string;
}

export function buildSandboxServiceBackends(
  opts: BuildServiceBackendsOptions,
): SandboxApiServices {
  const llmClient = new SovereignLlmClient(opts.llmManager);
  const llmChat: LlmChatFn = async (req) => {
    // System-prompt composition:
    //   - overrideSystem=true       : use `req.system` only. Sovereign
    ...
    //                                 by Sovereign's identity).
    //   - `req.system` set, default : Sovereign prompt + "\n\n" + req.system.
    //                                 Lets the user steer the reply (e.g.
    //                                 "respond in JSON") while keeping the
    //                                 Sovereign identity.
    //   - no `req.system`           : Sovereign prompt alone. Default for
    //                                 plain "ask Sovereign" steps.
    ...
    const sovereignSystem = opts.buildSovereignSystemPrompt
      ? opts.buildSovereignSystemPrompt(req.prompt)
      : undefined;
    let system: string | undefined;
    if (req.overrideSystem) {
      system = req.system;
    } else if (req.system && sovereignSystem) {
      system = `${sovereignSystem}\n\n${req.system}`;
    } else {
      system = req.system ?? sovereignSystem;
    }
  ...
  const toolAdapter = opts.toolRegistry
    ? new SovereignToolRegistryAdapter(opts.toolRegistry)
    : null;
  ...
  const notifierAdapter = new SovereignNotifierAdapter(notifierDeps);
  ...
  const contextAdapter = new SovereignContextProviderAdapter();
  const contextProvider: SovereignContextProvider = {
    vaultSearch: (input) =>
  ...
  const eventsPoll: EventsPollFn = async (req) => {
    ...
    // The route's `SovereignEvent` types `id` as a string (consistent with all
  ...
  const runnerAdapter = new SovereignWorkflowRunnerAdapter();
