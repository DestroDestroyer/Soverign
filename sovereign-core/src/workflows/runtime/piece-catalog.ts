/**
 * `PieceCatalog` -- canonical view of installed pieces (Sovereign-native + future
 ...
 * `SovereignPieceRegistry` (`{name, displayName, description, actions, triggers}`
...
/**
 * Piece-name + trigger-name pair for sovereign-trigger:on_event. Hoisted so
 * the dynamic-sample injector and the composer prompt agree on the key.
 */
export const SOVEREIGN_ON_EVENT_TRIGGER = {
  piece: "@sovereignpieces/piece-sovereign-trigger",
  trigger: "on_event",
} as const;
...
export interface PieceCatalogAction {
   ...
   * `createAction({ outputSample: ... })` (Sovereign extension to upstream AP).
...
export interface PieceCatalogTrigger extends PieceCatalogAction {
   ...
   * sovereign-trigger `on_event` is the canonical case (the `payload`
 ...
 * sovereign-ask, schedule) leave it undefined and the editor renders no
...
export interface PieceCatalogEntry {
  /** Upstream package name -- e.g. `@sovereignpieces/piece-sovereign-ask`. */
  name: string;
  displayName: string;
  description: string;
   ...
   * (sovereign-ask, schedule, webhook, code).
   */
  auth?: PieceCatalogAuth;
}

/**
 * Structural interface that both `PieceCatalog` and `SovereignPieceRegistry`
 ...
 *         resolution for sovereign-trigger:on_event).
 *   v4 -- promoted sovereign-trigger:on_event's `eventType` input field
 ...
 *         dropdown).
 *   v6 -- promoted sovereign-trigger:run_workflow's `flow` input from
...
export function computeCatalogCacheKey(opts: {
  ...
  // sovereign-trigger:on_event projection synthesizes its
...
/** Convert an upstream `PieceMetadata` blob into the legacy `SovereignPiece`-style entry. */
export function metadataToCatalogEntry(meta: RawPieceMetadata | unknown): PieceCatalogEntry {
  ...
  // Apply any Sovereign-specific catalog enrichments after the generic
  // projection finishes. Today this covers two upgrades on the
  // sovereign-trigger piece (on_event eventType -> enum, run_workflow
  // flow -> flow_ref). Centralising the gating keeps the generic
  // projection above piece-agnostic and gives the next "Sovereign tweak"
  // a single named home.
  enrichSovereignCatalogEntry(out);
...
/**
 * Apply Sovereign-specific catalog enrichments AFTER the generic
 ...
 * trigger, so an upstream rename (or a Sovereign-piece source edit that
 * drops a sub-action) silently no-ops rather than crashing the catalog
 * build.
 *
 * The set of enrichments here is small and well-known:
 *   - sovereign-trigger:on_event   -- attach `dynamicSampleData` and
 *                                  promote `eventType` to enum.
 *   - sovereign-trigger:run_workflow -- promote `flow` to flow_ref.
 *
 ...
 */
export function enrichSovereignCatalogEntry(entry: PieceCatalogEntry): void {
  if (entry.name !== SOVEREIGN_ON_EVENT_TRIGGER.piece) return;
  const onEvent = entry.triggers?.[SOVEREIGN_ON_EVENT_TRIGGER.trigger];
  if (onEvent) {
...
/**
 * Build the `dynamicSampleData` map for sovereign-trigger:on_event. Each
...
/**
 * Rewrite the `flow` input field on sovereign-trigger's `run_workflow`
...
function rawActionToCatalogAction(
  ...
  // (Sovereign extension); triggers use upstream `sampleData`. Both can be
