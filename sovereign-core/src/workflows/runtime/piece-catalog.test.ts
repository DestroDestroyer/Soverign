 ...
 *   - Engine-level (gated on SOVEREIGN_TEST_ENGINE_BUILD=1): spawn the real
 *     engine, run EXTRACT_PIECE_METADATA against the seven vendored Sovereign
...
import { SovereignPieceRegistry } from "../sovereign-pieces/types";
...
import { buildAllSovereignPieces } from "../runner/engine-runtime/build-pieces";
...
describe("PieceCatalog (unit)", () => {
  test("PieceCatalog.list/get round-trips initial entries", () => {
    ...
    expect(catalog.get("@sovereignpieces/piece-sovereign-test")?.displayName).toBe("Sovereign: Test");
  ...
  test("metadataToCatalogEntry maps actions + triggers verbatim", () => {
    ...
    expect(entry.name).toBe("@sovereignpieces/piece-x");
  ...
  test("metadataToCatalogEntry attaches dynamicSampleData to sovereign-trigger:on_event", () => {
    // sovereign-trigger's on_event surfaces an envelope whose payload shape
  ...
  test("SovereignPieceRegistry structurally satisfies the PieceLookup interface", () => {
    // Compile-time + runtime check: assigning a SovereignPieceRegistry instance
    ...
    const reg = new SovereignPieceRegistry();
    reg.register({
      name: "p",
      displayName: "P",
      description: "p",
...
const buildOptIn = process.env.SOVEREIGN_TEST_ENGINE_BUILD === "1";
...
const piecesAlreadyBuilt = existsSync(
  resolve(
    ...
    "pieces/sovereign/test/dist/src/index.js",
  ),
);
...
describe("PieceCatalog (engine end-to-end)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (!piecesAlreadyBuilt && buildOptIn) await buildAllSovereignPieces();
  ...
  test.skipIf(skipE2eTests)(
    "extracts metadata for every Sovereign-native piece via the real engine",
    async () => {
      const root = resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/sovereign");
      ...
      try {
        ...
        expect(names).toContain("@sovereignpieces/piece-sovereign-ask");
        expect(names).toContain("@sovereignpieces/piece-sovereign-tool");
        expect(names).toContain("@sovereignpieces/piece-sovereign-notify");
        expect(names).toContain("@sovereignpieces/piece-sovereign-context");
        expect(names).toContain("@sovereignpieces/piece-sovereign-agent");
        expect(names).toContain("@sovereignpieces/piece-sovereign-trigger");
        expect(names).toContain("@sovereignpieces/piece-sovereign-test");
        // ask: single action `ask` with at least the `prompt` field.
        const ask = catalog.get("@sovereignpieces/piece-sovereign-ask");
        ...
        const trigger = catalog.get("@sovereignpieces/piece-sovereign-trigger");
