 ...
 *
 *   1. If the externalId starts with `sovereign:`, dispatch to a registered
 *      `SovereignConnectionSource` that pulls live from Sovereign' existing stores
 *      (Google OAuth file, Telegram/Discord bot token in config, etc.). This
 *      means a user who already wired Gmail into Sovereign sees a "Sovereign Gmail"
 ...
 *
 * The adapter does not import Sovereign modules directly; sources are injected so
...
/** Prefix that marks a connection as "managed by Sovereign itself". */
export const SOVEREIGN_PREFIX = "sovereign:";
...
/**
 * A source of Sovereign-managed credentials. One per backing store (Google file,
 * channel config, etc.). Sources only handle externalIds with the SOVEREIGN_PREFIX.
 */
export interface SovereignConnectionSource {
...
/** Composite resolver: Sovereign sources first, then the workflow DB. */
export class CredentialResolver {
  private sources: SovereignConnectionSource[] = [];

  register(source: SovereignConnectionSource): void {
  ...
  list(): readonly SovereignConnectionSource[] {
    return this.sources;
  }

  async resolve(input: ResolveInput): Promise<ResolvedConnection | null> {
    if (input.externalId.startsWith(SOVEREIGN_PREFIX)) {
...
// Live Sovereign credential sources -- one per backing store -- live in their
