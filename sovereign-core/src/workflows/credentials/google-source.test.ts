/**
 * Coverage for `SovereignGoogleConnectionSource`. The refresh-token dance
...
import { SovereignGoogleConnectionSource, SOVEREIGN_GOOGLE_PREFIX } from "./google-source";
...
describe("SovereignGoogleConnectionSource", () => {
  test("returns null when GoogleAuth is unauthenticated", async () => {
    ...
    const src = new SovereignGoogleConnectionSource(auth);
    expect(await src.resolve(SOVEREIGN_GOOGLE_PREFIX)).toBeNull();
  });

  test("canResolve matches sovereign:google + sovereign:google:<sub>", () => {
    const src = new SovereignGoogleConnectionSource(makeFakeAuth({ authed: false, freshAccessToken: "x", tokens: null }));
    expect(src.canResolve("sovereign:google")).toBe(true);
    expect(src.canResolve("sovereign:google:gmail")).toBe(true);
    expect(src.canResolve("sovereign:telegram")).toBe(false);
  });
  ...
  test("calls getAccessToken so an expired cache surfaces the refreshed token", async () => {
    ...
    const src = new SovereignGoogleConnectionSource(makeFakeAuth(state));
    const resolved = await src.resolve(SOVEREIGN_GOOGLE_PREFIX);
