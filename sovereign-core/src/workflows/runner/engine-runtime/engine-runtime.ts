...
import type { FlowVersion as SovereignFlowVersion } from "../../db/repos/flow-version";
...
export interface EngineRuntimeOptions {
   ...
   * `~/.sovereign/workflow-codes`. The engine appends `${flowVersionId}/${stepName}/index.js`.
   */
  baseCodeDir?: string;
  /**
   ...
   * `packages/pieces/sovereign/<name>/dist/package.json`.
   */
  cwd?: string;
  /** Search roots for vendored pieces. Default: vendored `packages/pieces` tree. */
  customPiecesPaths?: string[];
  /** CSV of dev-piece names exposed via AP_DEV_PIECES. Default: sovereign pieces. */
  devPieces?: string[];
...
export interface ExecuteFlowOnHandleOptions {
  ...
  flowVersion: SovereignFlowVersion | UpstreamFlowVersion;
...
/**
 * Walk the vendored Sovereign pieces directory and return each piece's
 * short alias (the part the engine's `getPieceNameFromAlias` strips
 * `piece-` from). Used as the default `devPieces` list so adding a
 * new Sovereign piece doesn't require an engine-runtime edit.
 *
 ...
 * dir").
 */
function discoverSovereignDevPieces(): string[] {
  ...
  for (const name of readdirSync(root).sort()) {
    ...
    try {
      ...
      if (typeof pkg.name === "string" && pkg.name.length > 0) {
        ...
        // expects: `@sovereignpieces/piece-sovereign-foo` -> `sovereign-foo`.
...
export class EngineRuntime {
  ...
  constructor(opts: EngineRuntimeOptions) {
    ...
    // packages/pieces/sovereign/*/dist/package.json without further env setup.
    ...
    //
    // Auto-discover Sovereign pieces from the vendored tree so adding a
    ...
    this.devPieces = opts.devPieces ?? discoverSovereignDevPieces();
  }
...
function isUpstreamFlowVersion(
  v: SovereignFlowVersion | UpstreamFlowVersion,
