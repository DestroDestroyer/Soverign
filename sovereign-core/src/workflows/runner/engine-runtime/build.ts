 ...
 *
 * Staging lives outside the repo (under `~/.sovereign/cache/engine-build`) so
...
const CACHE_ROOT = resolve(homedir(), ".sovereign/cache");
 ...
 * from SOVEREIGN (e.g. axios in pieces/common feeds the HTTP node, which webhook
...
function buildStagingPackageJson(): string {
  ...
  return JSON.stringify(
    {
      name: "sovereign-engine-build-staging",
      private: true,
      type: "commonjs",
      dependencies: deps,
...
const PATCHED_VENDOR_SOURCES = [
  ...
  // Sovereign-only `outputSample` extension on actions + the matching
  ...
  // Sovereign-only BranchOperator additions (TEXT_MATCHES_REGEX +
  ...
  // Sovereign: upstream sets process.env.NODE_TLS_REJECT_UNAUTHORIZED='0'
