/**
 * Phase L plumbing smoke (gated on SOVEREIGN_TEST_ENGINE_BUILD=1).
 *
 ...
 * dragging in `googleapis` and friends. Uses the `sovereign-validate` test
 ...
 *   - The engine resolves `{{connections['sovereign:validate']}}` against the
...
const piecesAlreadyBuilt = existsSync(
  resolve(
    ...
    "pieces/sovereign/validate/dist/src/index.js",
  ),
);
