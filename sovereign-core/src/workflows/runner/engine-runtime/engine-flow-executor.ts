 ...
 *
 * Override via `SOVEREIGN_WORKFLOW_TERMINAL_TIMEOUT_MS` for unusual environments.
 */
const DEFAULT_TERMINAL_TIMEOUT_MS = parsePositiveIntEnv(
  process.env["SOVEREIGN_WORKFLOW_TERMINAL_TIMEOUT_MS"],
  60_000,
);
...
export interface EngineFlowExecutorOptions {
   ...
   * (`~/.sovereign/workflow-logs/`). Tests pass a per-test temp dir to avoid
   * mutating `process.env.SOVEREIGN_WORKFLOW_DATA_DIR`.
   */
...
export class EngineFlowExecutor implements FlowExecutor {
  ...
  async execute(ctx: FlowExecutorContext): Promise<FlowExecutorResult> {
    ...
    try {
      ...
      // On Sovereign's local-first, single-user scale this is negligible
      ...
      //
      // Override via `SOVEREIGN_WORKFLOW_STREAM_STEP_PROGRESS=NONE` for
      ...
      const streamStepProgress: "WEBSOCKET" | "NONE" =
        process.env["SOVEREIGN_WORKFLOW_STREAM_STEP_PROGRESS"] === "NONE"
          ? "NONE"
          : "WEBSOCKET";
