/**
 * Phase F end-to-end smoke: spawn the engine, load Sovereign pieces from disk,
 ...
 * of the Sovereign pieces is mechanical. Skipped when the engine bundle isn't on
...
import { buildAllSovereignPieces } from "./build-pieces";
...
const PIECE_TEST_NAME = "@sovereignpieces/piece-sovereign-test";
const PIECE_ASK_NAME = "@sovereignpieces/piece-sovereign-ask";
const PIECE_TOOL_NAME = "@sovereignpieces/piece-sovereign-tool";
const PIECE_NOTIFY_NAME = "@sovereignpieces/piece-sovereign-notify";
const PIECE_CONTEXT_NAME = "@sovereignpieces/piece-sovereign-context";
const PIECE_AGENT_NAME = "@sovereignpieces/piece-sovereign-agent";
const PIECE_TRIGGER_NAME = "@sovereignpieces/piece-sovereign-trigger";
...
describe("Engine end-to-end (F gate)", () => {
  ...
  test.skipIf(skipE2eTests)(
    "manual trigger + sovereign-ask action calls daemon's /v1/sovereign/llm/chat",
    async () => {
      llmCalls = [];
      const flow = createFlow({ projectId: DEFAULT_IDS.project });
      const trigger: FlowTriggerNode = {
...
describe("Engine end-to-end (G+H pieces)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!cached) return;
    if (buildOptIn) await buildAllSovereignPieces();
    runtime = new EngineRuntime({ api, bundlePath: cached.bundlePath });
  });
