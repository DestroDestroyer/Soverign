/**
 * Bundle each Sovereign-authored piece (under `packages/pieces/sovereign/*`) into
...
async function ensureStagingInstalled(): Promise<void> {
  ...
  // `buildAllSovereignPieces()` in parallel, and if pieces won the race
...
/**
 * Build every piece directly under `packages/pieces/sovereign/`. Returns the
 ...
 */
export async function buildAllSovereignPieces(
  opts: BuildPieceOptions = {},
): Promise<BuildPieceResult[]> {
...
export const PIECE_BUILD_PATHS = {
  sovereignPiecesRoot: resolve(ENGINE_BUILD_PATHS.VENDOR_PACKAGES, "pieces/sovereign"),
} as const;
