 ...
 * against an optional HMAC-SHA256 secret (X-Sovereign-Signature header).
 */
...
export class WebhookManager {
  ...
  async handleRequest(workflowId: string, req: Request): Promise<Response> {
    ...
    if (route.secret) {
      const signature = req.headers.get('x-sovereign-signature') ?? req.headers.get('X-Sovereign-Signature');
