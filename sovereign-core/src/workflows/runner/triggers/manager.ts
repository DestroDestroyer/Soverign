 ...
 *         direct-subscribe path for `sovereign-trigger:on_event` (kept until
...
export interface TriggerManagerDeps {
  /**
   * In-process event bus. Used by the legacy `sovereign-trigger:on_event`
   ...
   * supported is `sovereign-trigger:on_event`, which falls back to direct
...
export class TriggerManager {
  ...
  private async register(flow: FlowRow): Promise<void> {
    ...
    if (trigger.type === "PIECE_TRIGGER") {
      ...
      // Engine-less fallback for sovereign-trigger:on_event. In
      ...
      // `manager.test.ts:"TriggerManager: sovereign-trigger on_event"`.
      ...
      // engine.
      if (pieceName === "sovereign-trigger") {
        return this.registerSovereignEvent(flow.id, versionId, trigger);
      }
      this.log(
  ...
  private registerSovereignEvent(flowId: string, versionId: string, trigger: TriggerNode): void {
    if (trigger.settings?.triggerName !== "on_event") {
      this.log(
        `flow ${flowId}: sovereign-trigger has triggerName="${trigger.settings?.triggerName}"; only "on_event" is supported`,
      );
      return;
    }
    const input = (trigger.settings?.input ?? {}) as Record<string, unknown>;
  ...
  /**
   * Engine-managed trigger fire (polling sources, e.g. sovereign-trigger
