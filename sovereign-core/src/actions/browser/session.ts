...
const SNAPSHOT_SCRIPT = `(() => {
  ...
  window.__sovereign_elements = els.map(e => e._el);
...
export class BrowserController {
  ...
  async type(elementId: number, text: string, submit: boolean = false): Promise<string> {
    ...
    const focusResult = await this.cdp.send('Runtime.evaluate', {
      expression: `(() => {
        const el = window.__sovereign_elements && window.__sovereign_elements[${elementId - 1}];
   ...
   */
  async screenshot(filePath: string = '/tmp/sovereign-screenshot.png'): Promise<string> {
    await this.ensureConnected();
