...
function localCaptureScreen(): string {
  const os = platform();
  const tmp = `/tmp/sovereign-screenshot-${Date.now()}.png`;
  if (os === 'darwin') {
...
function isPrivateHost(hostname: string): boolean {
  // Literal loopback / link-local / private ranges
  const BLOCKED = [
    /^localhost$/i,
    /^127\.\d+\.\d+\.\d+$/,
    /^0\.0\.0\.0$/,
...
export const fetchUrlTool: ToolDefinition = {
  name: 'fetch_url',
  description: 'Fetch the text content of a public URL (HTTP/HTTPS only) and return it. Private IPs and localhost are blocked. Useful for reading web articles, documentation, or JSON APIs.',
  category: 'general',
  parameters: {
    url: {
      type: 'string',
      description: 'The public HTTP/HTTPS URL to fetch (private/internal addresses are rejected)',
      required: true,
    },
  },
  execute: async (params) => {
    ...
    try {
      ...
      return `Error: Invalid URL — could not parse "${rawUrl}"`;
    }
    if (!['http:', 'https:'].includes(parsed.protocol)) {
      return `Error: Unsupported protocol "${parsed.protocol}". Only http:// and https:// are allowed.`;
    }

    // --- Security: SSRF — block private / internal hosts ---
    if (isPrivateHost(parsed.hostname)) {
      return `Error: SSRF protection — "${parsed.hostname}" resolves to a private/internal address and cannot be fetched.`;
    }
    ...
    try {
      ...
      const response = await fetch(rawUrl, {
        ...
        headers: {
          'User-Agent': 'Sovereign/1.0 (compatible; Mozilla/5.0)',
      ...
      if (text.length > 20000) {
        text = text.slice(0, 20000) + '\n... [truncated, content was ' + text.length + ' chars]';
      }
      return text || '[empty response]';
    } catch (err) {
      if (err instanceof Error && err.name === 'AbortError') {
        return `Error: Request timed out after 15 seconds for "${rawUrl}"`;
      }
