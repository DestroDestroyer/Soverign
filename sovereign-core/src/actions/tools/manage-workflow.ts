...
export interface ComposerToolRegistry {
   ...
   * `sovereign-tool:invoke` step can't omit a tool's required param (e.g. `action`)
...
export interface ManageWorkflowDeps {
   ...
   * Sovereign tools so the LLM can wire `sovereign-tool { toolName: '...' }` correctly
   ...
   * roles in its prompt and rejects a `sovereign-agent:delegate` step whose `role`
...
export function createManageWorkflowTool(deps: ManageWorkflowDeps = {}): ToolDefinition {
  return {
    ...
    description: [
      "Create, run, and manage the user's Sovereign workflows (automations).",
      "",
