 ...
 * by prompting the configured Sovereign LLM with the piece catalog + a schema
...
const AGENT_PIECE_SHORT = "sovereign-agent";
...
const TOOL_PIECE_SHORT = "sovereign-tool";
 ...
 * catalog AND rejected in validation -- the LLM was picking `sovereign-test:echo`
 ...
 */
const COMPOSER_EXCLUDED_PIECES = new Set(["sovereign-test", "sovereign-validate"]);
...
/** One parameter of a Sovereign tool, as surfaced to the composer. */
...
/**
 * A Sovereign tool's invocation contract: its name plus the parameters it accepts.
 * The composer lists these so the LLM wires `sovereign-tool { toolName, params }`
...
export interface ComposeDeps {
  ...
  /**
   * Optional list of registered Sovereign tool names. When present, surfaced in
   * the planner prompt so the LLM can wire `sovereign-tool { toolName: '...' }`
   ...
   * `sovereign-tool:invoke` step's `params`. Falls back to `toolNames` (names
   * only) when absent.
   */
   ...
   * a `sovereign-agent:delegate` step, and (b) rejects any `delegate` step whose
...
function buildSystemPrompt(catalog: string, toolsText: string, rolesText: string): string {
  return [
    "You are the Sovereign workflow composer. Convert the user's description into a workflow definition.",
    "",
    ...
    "  - For event triggers: pieceName='sovereign-trigger', triggerName='on_event', input.eventType='<event type>'.",
    ...
    "  - For sovereign-trigger:on_event, the trigger output is an event envelope shaped",
    ...
    "  - When the user asks for an integration that isn't a registered piece (Gmail, Slack, ...), use the `sovereign-tool` piece with `toolName` set to a registered Sovereign tool. Available tools are listed below.",
    rolesText
      ? "  - To hand a goal to a sub-agent, use the `sovereign-agent` piece's `delegate` action. Its optional `input.role` MUST be one of the specialist role ids listed below VERBATIM (e.g. `research-analyst`, not `researcher`). If none fits, OMIT `role` and the default agent handles it."
      : "  - To hand a goal to a sub-agent, use the `sovereign-agent` piece's `delegate` action with `input.goal`. Omit `input.role` to use the default agent.",
...
/**
 * Render the Sovereign tool listing for the prompt. Prefers the richer `tools`
...
function renderTools(
  ...
  if (tools && tools.length > 0) {
    const header =
      "Available Sovereign tools (call via `sovereign-tool { toolName, params: { ... } }`; include EVERY param marked REQUIRED):";
    const lines = [header];
  ...
  if (toolNames && toolNames.length > 0) {
    return [
      "Available Sovereign tools (call via `sovereign-tool { toolName, params }`):",
      ...toolNames.map((n) => `  - ${n}`),
    ].join("\n");
  }
  return "";
...
/**
 * List the valid specialist role ids for a `sovereign-agent:delegate` step. The
...
function renderSpecialistRoles(roles: ComposerSpecialistRole[] | undefined): string {
  ...
  return [
    "Available specialist sub-agent roles (for `sovereign-agent:delegate` -> `input.role`; use the id verbatim):",
    ...roles.map((r) => {
...
function renderCatalog(registry: PieceLookup): string {
  ...
  for (const piece of registry.list()) {
    ...
    for (const trigger of Object.values(piece.triggers ?? {})) {
      ...
      // also set `outputSample` (Sovereign extension). Either is a valid
  ...
  // Workflow event-type catalog (used by sovereign-trigger:on_event flows).
  lines.push("");
  lines.push("Available event types for sovereign-trigger:on_event (settings.input.eventType):");
...
function validateStep(
  ...
  // tells the LLM to use forms like `sovereign-trigger` and `sovereign-tool`
  // (because that's the short identity humans use), but the catalog
  // is keyed by the full npm package name like
  // `@sovereignpieces/piece-sovereign-trigger`. Without this resolution the
  // composer would lose every Sovereign-piece flow on a validation error
  ...
  // Delegate-role check: a `sovereign-agent:delegate` step's optional `role` must
  ...
  // Invoke-params check: a `sovereign-tool:invoke` step names a tool in
