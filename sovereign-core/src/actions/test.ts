...
async function testActionLayer() {
  console.log('Testing Sovereign Action Layer\n');
  ...
  try {
    const result = await executor.execute('echo "Hello from SOVEREIGN"');
  ...
  try {
    const result = await registry.execute('echo', { message: 'Hello SOVEREIGN!' });
