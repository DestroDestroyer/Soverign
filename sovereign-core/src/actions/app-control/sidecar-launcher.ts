 ...
 * Checks: ~/.sovereign/sidecar/ then repo build output.
 */
  ...
  if (WSLBridge.isWSL()) {
    ...
    try {
      ...
      if (userProfile && !userProfile.includes('%')) {
        ...
        const wslPath = `/mnt/${drive}${rest}/.sovereign/sidecar/desktop-bridge.exe`;
    ...
    candidates.push('/mnt/c/Users/' + (process.env.USER ?? 'user') + '/.sovereign/sidecar/desktop-bridge.exe');
  } else {
    // Native Windows
    candidates.push(join(homedir(), '.sovereign', 'sidecar', 'desktop-bridge.exe'));
  }
