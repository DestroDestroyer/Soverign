 ...
 *
 * SOVEREIGN ships through three install paths — a published bun/npm package,
 * a git-clone installer script, and a Docker image — plus a fourth "developer
 * checkout" case. `sovereign update` and `sovereign uninstall` need different
...
/**
 * Detect how SOVEREIGN was installed. Pure function — all environmental inputs
...
export function detectInstallMethod(
  ...
  if (env.SOVEREIGN_INSTALL_METHOD === 'docker') {
    return { method: 'docker', reason: 'SOVEREIGN_INSTALL_METHOD=docker' };
  }
  ...
  const scriptInstallRoot = join(home, '.sovereign', 'daemon');
...
/**
 * Human-readable one-liner for the given method. Used by `sovereign doctor`.
 */
export function describeInstallMethod(info: InstallMethodInfo): string {
  const labels: Record<InstallMethod, string> = {
    docker: 'Docker container',
    'bun-global': 'Bun global install (@usesovereign/brain)',
    script: 'install.sh (git clone under ~/.sovereign/daemon)',
 ...
 * `sovereign update` / `sovereign uninstall` delegate to these internally — the
...
export function getMethodCommands(method: InstallMethod): MethodCommands {
  switch (method) {
    case 'docker':
      return {
        update: 'docker pull <image> && docker rm -f sovereign && docker run -d ... <image>',
        uninstall: 'docker rm -f sovereign   # and `docker volume rm sovereign-data` to delete data',
      };
    case 'bun-global':
      return {
        update: 'sovereign update   # runs `bun update -g @usesovereign/brain`',
        uninstall: 'sovereign uninstall   # runs `bun uninstall -g @usesovereign/brain` and cleans up',
      };
    case 'script':
      return {
        update: 'sovereign update   # checks out the latest release tag + `bun install`',
        uninstall: 'sovereign uninstall   # removes ~/.sovereign and the CLI wrapper',
      };
    case 'dev':
      return {
        update: 'git pull && bun install   # manage your dev checkout yourself',
        uninstall: 'rm -rf <your checkout>   # `sovereign uninstall` only cleans side effects',
      };
    case 'unknown':
      return {
        update: '(install method unknown — see `sovereign doctor`)',
        uninstall: '(install method unknown — see `sovereign doctor`)',
      };
  }
}
