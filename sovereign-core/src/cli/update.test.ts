...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'sovereign-update-test-'));
  // Provide a package.json so getInstalledVersion doesn't return '0.0.0'.
  writeFileSync(join(workDir, 'package.json'), JSON.stringify({
    name: '@usesovereign/brain',
    version: '1.0.0',
  }));
});
...
describe('runUpdate — bun-global', () => {
  test('dispatches `bun update -g @usesovereign/brain`', async () => {
    ...
    expect(calls[0]!.cmd).toEqual(['bun', 'update', '-g', '@usesovereign/brain']);
  });
