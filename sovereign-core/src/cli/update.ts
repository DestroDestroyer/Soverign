/**
 * `sovereign update` — install-method-aware updater.
 *
 * Dispatches based on how SOVEREIGN was installed:
 *   docker      refuse, point user at host-side `docker pull`
 *   bun-global  `bun update -g @usesovereign/brain`
...
/**
 * Spawn the daemon in a detached process, same pattern as `sovereign start -d`.
...
function restartDaemonDetached(packageRoot: string): void {
  ...
  const child = spawn('bun', [join(packageRoot, 'bin/sovereign.ts'), 'start', '--no-open'], {
    detached: true,
...
function updateDocker(info: InstallMethodInfo): UpdateResult {
  console.log(c.yellow('SOVEREIGN is running inside a Docker container.'));
  console.log('');
  console.log('  `sovereign update` does not apply to container installs — the container is the unit of update.');
  console.log('  From your host, run:');
  console.log(c.dim('    docker pull <your-sovereign-image>'));
  console.log(c.dim('    docker rm -f sovereign && docker run -d ... <your-sovereign-image>'));
  console.log('');
...
function updateDev(info: InstallMethodInfo, packageRoot: string): UpdateResult {
  console.log(c.yellow('SOVEREIGN is running from a developer checkout.'));
  console.log('');
  console.log('  `sovereign update` does not manage dev checkouts — updating your working tree');
...
function updateUnknown(info: InstallMethodInfo): UpdateResult {
  console.log(c.red('Could not determine how SOVEREIGN was installed.'));
  console.log('');
  console.log('  `sovereign update` needs to know whether to run `bun update -g`, `git pull`,');
  console.log('  or redirect you to `docker pull`. Run `sovereign doctor` to see what was detected.');
  console.log('');
  console.log('  Update manually with one of:');
  console.log(c.dim('    bun update -g @usesovereign/brain              # if installed via bun'));
...
function updateBunGlobal(
  ...
  console.log(c.dim('Running `bun update -g @usesovereign/brain`...\n'));

  const result = spawn(['bun', 'update', '-g', '@usesovereign/brain']);
