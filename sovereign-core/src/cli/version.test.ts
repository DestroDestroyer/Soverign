...
function makeTempDir(name: string): string {
  const dir = mkdtempSync(join(tmpdir(), `sovereign-version-test-${name}-`));
...
async function writePackageJson(dir: string, version: string): Promise<void> {
  await Bun.write(join(dir, 'package.json'), JSON.stringify({ name: '@usesovereign/brain', version }, null, 2));
}
...
async function withFakeGit<T>(
  ...
  const previousGitBin = process.env.SOVEREIGN_GIT_BIN;
  const gitExtension = process.platform === 'win32' ? '.cmd' : '';
  process.env.SOVEREIGN_GIT_BIN = join(dir, 'bin', `git${gitExtension}`);
  try {
    return await run(dir);
  } finally {
    if (previousGitBin === undefined) {
      delete process.env.SOVEREIGN_GIT_BIN;
    } else {
      process.env.SOVEREIGN_GIT_BIN = previousGitBin;
    }
  }
}
...
if [ "$3" = "describe" ] && [ "$4" = "--tags" ] && [ "$5" = "--always" ]; then
  ...
  test('falls back to package.json when not a git checkout', async () => {
    ...
    // Point SOVEREIGN_GIT_BIN at a non-existent file so rev-parse fails →
    // isGitCheckout returns false and we never call describe.
    const previousGitBin = process.env.SOVEREIGN_GIT_BIN;
    process.env.SOVEREIGN_GIT_BIN = join(dir, 'no-such-git-binary');
    try {
      expect(getInstalledVersion(dir)).toBe('3.2.1');
    } finally {
      if (previousGitBin === undefined) delete process.env.SOVEREIGN_GIT_BIN;
      else process.env.SOVEREIGN_GIT_BIN = previousGitBin;
    }
  });
...
// Run the real `bin/sovereign.ts --version` / `version` paths so the banner
...
describe('CLI --version output', () => {
  ...
  const SOVEREIGN_BIN = join(REPO_ROOT, 'bin', 'sovereign.ts');

  async function runCli(args: string[]): Promise<{ exitCode: number; stdout: string }> {
    const proc = Bun.spawn(['bun', SOVEREIGN_BIN, ...args], {
      stdout: 'pipe',
      stderr: 'pipe',
      cwd: REPO_ROOT,
    });
  ...
  test('`sovereign --version` prints v-prefixed version with no doubling', async () => {
  ...
  test('`sovereign version` matches `--version` output', async () => {
  ...
  test('`sovereign -v` matches `--version` output', async () => {
  ...
  test('`sovereign help` banner contains exactly one v-prefixed version (no vv)', async () => {
