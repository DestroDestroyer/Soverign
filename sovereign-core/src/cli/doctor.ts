/**
 * Sovereign Doctor — Environment Diagnostics
 *
...
const CONFIG_PATH = join(SOVEREIGN_DIR, 'config.yaml');
...
export async function runDoctor(): Promise<void> {
  ...
  if (existsSync(SOVEREIGN_DIR)) {
    results.push({ name: 'Data Directory', status: 'ok', message: SOVEREIGN_DIR });
  } else {
    results.push({ name: 'Data Directory', status: 'warn', message: `${SOVEREIGN_DIR} not found. Run: sovereign start (then finish setup at http://localhost:3142)` });
  }
  ...
  if (existsSync(CONFIG_PATH)) {
    ...
    results.push({ name: 'Config File', status: 'fail', message: 'Not found. Run: sovereign start (then finish setup at http://localhost:3142)' });
  }
  ...
  if (failCount > 0) {
    console.log(c.red('\nSome checks failed. Run "sovereign start" and finish setup at http://localhost:3142 to fix configuration.\n'));
  } else if (warnCount > 0) {
    console.log(c.yellow('\nAll critical checks passed, but some optional features need setup.\n'));
  } else {
    console.log(c.green('\nAll checks passed! SOVEREIGN is ready.\n'));
  }
