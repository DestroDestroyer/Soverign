...
function getSovereignPath(): string {
  // When installed globally, import.meta.dir points to the package
  return join(import.meta.dir, '../../bin/sovereign.ts');
}
...
const SYSTEMD_SERVICE = join(SYSTEMD_DIR, 'sovereign.service');

function generateSystemdUnit(): string {
  const bunPath = getBunPath();
  const sovereignPath = getSovereignPath();

  return `[Unit]
Description=Sovereign Daemon
After=network.target

[Service]
Type=simple
ExecStart=${bunPath} ${sovereignPath} start --foreground
Restart=on-failure
RestartSec=5
...
async function installSystemd(): Promise<boolean> {
  try {
    ...
    const enable = Bun.spawnSync(['systemctl', '--user', 'enable', 'sovereign.service']);
    if (enable.exitCode !== 0) {
      printErr('Failed to enable service. You may need to run: systemctl --user enable sovereign.service');
      return false;
    }
...
async function startSystemdService(): Promise<boolean> {
  try {
    const start = Bun.spawnSync(['systemctl', '--user', 'start', 'sovereign.service']);
    if (start.exitCode !== 0) {
      printErr('Failed to start systemd service. You may need to run: systemctl --user start sovereign.service');
      return false;
    }

    printOk('SOVEREIGN keepalive service is running.');
    return true;
  } catch (err) {
...
export function scheduleSystemdRestart(spawnSync: SpawnSyncFn = defaultSpawnSync): boolean {
  ...
  try {
    const res = spawnSync(['systemctl', '--user', '--no-block', 'restart', 'sovereign.service']);
...
async function uninstallSystemd(): Promise<boolean> {
  try {
    Bun.spawnSync(['systemctl', '--user', 'stop', 'sovereign.service']);
    Bun.spawnSync(['systemctl', '--user', 'disable', 'sovereign.service']);
...
const LAUNCHD_PLIST = join(LAUNCHD_DIR, 'ai.sovereign.daemon.plist');

function generateLaunchdPlist(): string {
  const bunPath = getBunPath();
  const sovereignPath = getSovereignPath();
  const logDir = join(homedir(), '.sovereign', 'logs');
...
<dict>
  <key>Label</key>
  <string>ai.sovereign.daemon</string>
  <key>ProgramArguments</key>
  <array>
    <string>${bunPath}</string>
    <string>${sovereignPath}</string>
    <string>start</string>
  ...
  <string>${logDir}/sovereign.log</string>
  <key>StandardErrorPath</key>
  <string>${logDir}/sovereign-error.log</string>
  ...
  try {
    ...
    if (typeof uid === 'number') {
      ...
      if (bootstrap.exitCode === 0 || isLaunchdAlreadyLoaded(bootstrap)) {
        printOk('SOVEREIGN launch agent is running.');
        return true;
      }
...
function scheduleLaunchdRestart(): boolean {
  ...
  const command = uid != null
    ? `sleep 1; launchctl kickstart -k gui/${uid}/ai.sovereign.daemon >/dev/null 2>&1`
    : `sleep 1; launchctl kickstart -k gui/$(id -u)/ai.sovereign.daemon >/dev/null 2>&1`;
