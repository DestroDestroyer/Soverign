/**
 * `sovereign uninstall` — install-method-aware uninstaller.
 *
...
function resolveDataDir(env: NodeJS.ProcessEnv, home: string): string {
  return env.SOVEREIGN_HOME ? resolve(env.SOVEREIGN_HOME) : join(home, '.sovereign');
}
...
function resolveWrapperCandidates(env: NodeJS.ProcessEnv, home: string): string[] {
  ...
  return ['sovereign', 'sovereign.cmd', 'sovereign.ps1'].map((name) => join(bin, name));
}
...
export function createCleanupPlan(
  ...
  if (method === 'script') {
    // dataDir already covers resolvedPackageRoot at ~/.sovereign/daemon, so only
  ...
  return {
    ...
    // ~/.sovereign-uninstall.log sits next to, not inside, the data dir so it
    // survives the removal and is there for the user to inspect on failure.
    logPath: join(home, '.sovereign-uninstall.log'),
  };
}
...
// must be self-contained — no imports from the SOVEREIGN codebase, since the
...
// Give the parent sovereign CLI 1.5s to fully exit and release the wrapper /
...
if (payload.runBunUninstall) {
  try {
    const r = spawnSync(payload.bunPath, ['uninstall', '-g', '@usesovereign/brain'], {
      stdio: 'pipe',
      env: { ...process.env },
    });
...
// sovereign-uninstall-* directories across repeated uninstall attempts.
try {
...
async function schedulePackageRemoval(plan: CleanupPlan): Promise<void> {
  const tempDir = mkdtempSync(join(tmpdir(), 'sovereign-uninstall-'));
...
function printPlan(plan: CleanupPlan): void {
  ...
  if (plan.runBunUninstall) {
    console.log(c.dim(`  • Run: bun uninstall -g @usesovereign/brain`));
  }
  if (plan.method === 'dev') {
...
function refuseDocker(plan: CleanupPlan): void {
  console.log(c.yellow('\nSOVEREIGN is running inside a Docker container.\n'));
  console.log('  `sovereign uninstall` does not apply to container installs — the');
  console.log('  container is the unit of uninstall.');
  console.log('');
  console.log('  From your host, run:');
  console.log(c.dim('    docker rm -f sovereign'));
  console.log(c.dim('    docker volume rm sovereign-data   # if you want to delete data too'));
  console.log('');
