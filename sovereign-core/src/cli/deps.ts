...
/**
 * Check for core CLI tools SOVEREIGN relies on for common workflows.
 */
...
export function checkGoogleAuth(): DepStatus {
  const tokensPath = join(homedir(), '.sovereign', 'google-tokens.json');
...
export async function setupGoogleOAuth(config: any): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    try {
        ...
        async fetch(req) {
          ...
          if (url.pathname === '/api/auth/google/callback') {
            ...
            try {
              ...
              return new Response(
                '<html><body><h1>SOVEREIGN Google Authorization Complete!</h1><p>You can close this tab.</p></body></html>',
                { headers: { 'Content-Type': 'text/html' } }
              );
            } catch (err) {
              clearTimeout(timeout);
      ...
      printInfo('Stop the SOVEREIGN daemon first, or run later with: bun run setup:google');
      resolve(false);
      return;
    }
