...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'sovereign-install-method-'));
...
describe('detectInstallMethod — docker signals', () => {
  test('SOVEREIGN_INSTALL_METHOD=docker wins even when marker says otherwise', () => {
    const root = join(workDir, 'app');
    writeMarker(root, { method: 'bun-global' });
    const info = detectInstallMethod(root, {
      env: { SOVEREIGN_INSTALL_METHOD: 'docker' },
      dockerEnvPath: noDockerEnv,
      homeDir: fakeHome,
    });
    expect(info.method).toBe('docker');
    expect(info.reason).toContain('SOVEREIGN_INSTALL_METHOD');
  });
...
describe('detectInstallMethod — path inference', () => {
  test('package root under ~/.bun/install/global is bun-global', () => {
    const root = join(fakeHome, '.bun', 'install', 'global', 'node_modules', '@usesovereign', 'brain');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,
  ...
  test('~/.sovereign/daemon with .git is script install', () => {
    const root = join(fakeHome, '.sovereign', 'daemon');
  ...
  test('~/.sovereign/daemon without .git is unknown (half-installed)', () => {
    const root = join(fakeHome, '.sovereign', 'daemon');
    mkdirSync(root, { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
  ...
  test('arbitrary git checkout is dev', () => {
    const root = join(workDir, 'work', 'sovereign');
    mkdirSync(join(root, '.git'), { recursive: true });
    const info = detectInstallMethod(root, {
      env: {},
      dockerEnvPath: noDockerEnv,
