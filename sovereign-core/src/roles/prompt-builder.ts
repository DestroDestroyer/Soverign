...
export function buildSystemPrompt(role: RoleDefinition, context?: PromptContext): string {
  ...
  sections.push('# Reasoning Guidelines (Chain-of-Thought)');
  sections.push('To solve complex tasks, make decisions, or coordinate activities, you MUST use structured reasoning.');
  sections.push('Before taking any action, executing tool calls, or providing final answers for complex tasks:');
  sections.push('1. Decompose the goal into distinct steps.');
  sections.push('2. Identify dependencies, safety risks, and required tools.');
  sections.push('3. Think step-by-step to plan the optimal sequence.');
  sections.push('4. Adjust your plan dynamically as you receive feedback and outputs from tool executions.');
  sections.push('5. LOCAL-FIRST PRIVACY: You run strictly locally. Do NOT attempt to send any personal files, history, or credentials to external APIs or networks. Keep all processing and operations local to the machine.');
  sections.push('');
