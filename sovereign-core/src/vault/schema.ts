...
function createTables(db: Database): void {
  ...
  // Content pipeline: per-stage notes from user or SOVEREIGN
  db.run(`
  ...
  // Documents table: vault-stored documents created by SOVEREIGN
  db.run(`
  ...
  // Model metadata pool: stores info about all known models without downloading weights.
  // Models are fetched from Ollama, OpenRouter, OpenAI, Nvidia NIM daily.
  db.run(`
    CREATE TABLE IF NOT EXISTS models (
      ...
      provider TEXT NOT NULL,
      parameter_count REAL NOT NULL DEFAULT 0,
      context_length INTEGER NOT NULL DEFAULT 4096,
      min_ram INTEGER NOT NULL DEFAULT 4,
      min_vram INTEGER NOT NULL DEFAULT 0,
      speed_rank INTEGER NOT NULL DEFAULT 99,
      is_local INTEGER NOT NULL DEFAULT 0,
      download_url TEXT NOT NULL DEFAULT '',
      download_command TEXT NOT NULL DEFAULT '',
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_models_speed ON models(speed_rank ASC)`);
