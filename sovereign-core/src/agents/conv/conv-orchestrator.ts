...
export class ConvOrchestrator {
  ...
  private buildSystemPrompt(context: ConvSystemContext): string {
    ...
    parts.push(
      ...
      '`delegate` tool to send the work to a task tier that has access to all of Sovereign\'s ' +
      'real tools (workflows, browser, file ops, vault, calendar, commitments, goals, etc.).',
    );
    parts.push('');

    parts.push('# CRITICAL: You have NO direct knowledge of Sovereign state');
    parts.push(
    ...
    parts.push('- The user asks about real Sovereign state (workflows, files, vault, calendar) - tier=medium, template=general');
    ...
    parts.push('- The user asks who you are / what you can do / how Sovereign works (general capabilities, not specifics)');
    parts.push('');
    ...
    parts.push(
      ...
      'in the summary. If the summary mentions Sovereign features (workflows, ' +
      'commitments, goals), say so - do not suggest generic alternatives like ' +
      '"you could write a Python script" when the task already used a Sovereign ' +
