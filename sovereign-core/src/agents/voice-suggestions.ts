...
const SYSTEM_PROMPT = `You generate 3 to 5 short voice-suggestion phrases for a SOVEREIGN dashboard's "Try saying" panel.
...
export async function generateVoiceSuggestions(
  ...
  const contextLines = trimmedTurns
    .map((t) => `${t.role === 'user' ? 'USER' : 'SOVEREIGN'}: ${t.text.replace(/\s+/g, ' ').slice(0, 200)}`)
    .join('\n');
