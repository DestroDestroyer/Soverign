...
describe('resolveTelemetryEnabled', () => {
  ...
  test('SOVEREIGN_TELEMETRY opt-out overrides an enabled config', () => {
    for (const v of ['0', 'false', 'no', 'off', '']) {
      expect(resolveTelemetryEnabled(ON, { SOVEREIGN_TELEMETRY: v }).enabled).toBe(false);
    }
  });

  test('SOVEREIGN_TELEMETRY opt-in overrides a disabled config', () => {
    expect(resolveTelemetryEnabled(OFF, { SOVEREIGN_TELEMETRY: '1' }).enabled).toBe(true);
  });

  test('DO_NOT_TRACK takes precedence over everything', () => {
    const d = resolveTelemetryEnabled(ON, { DO_NOT_TRACK: '1', SOVEREIGN_TELEMETRY: '1' });
    expect(d.enabled).toBe(false);
