...
// SOVEREIGN own log lines — filter these out from error detection
const SOVEREIGN_LOG_PREFIX = /\[(CaptureEngine|ObserverManager|file-watcher|clipboard|processes|notifications|email|calendar|Daemon|ServiceRegistry|AgentService|WSService|OCREngine|Awareness|DesktopController|Executor|HealthMonitor|ChannelManager|TelegramAdapter|BackgroundAgent|WebSocketServer|EventReactor|Orchestrator|ChannelService|ObserverService)\]/;
...
export class ContextTracker {
  ...
  processCapture(captureId: string, ocrText: string, rawWindowTitle?: string, capturedAt?: number): {
    ...
    // First, filter out lines that are SOVEREIGN's own log output
    const filteredOcrText = ocrText
      .split('\n')
      .filter(line => !SOVEREIGN_LOG_PREFIX.test(line))
      .join('\n');
