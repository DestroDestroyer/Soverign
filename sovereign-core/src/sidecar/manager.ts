...
export class SidecarManager implements Service {
  ...
  setBrainUrl(url: string, source?: 'env' | 'config' | 'default'): void {
    ...
    if (source) {
      ...
      const sourceLabel = source === 'env' ? 'SOVEREIGN_BRAIN_DOMAIN env var'
        : source === 'config' ? 'config.yaml daemon.brain_domain'
        : `default fallback — set daemon.brain_domain in config.yaml or SOVEREIGN_BRAIN_DOMAIN env to override`;
