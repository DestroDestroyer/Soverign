# SOVERIGN — Codebase Map & Dev Guide
...
```
Soverign/
  soverign-core/          ← Daemon engine, triggers, and workflow runtime (Bun/TypeScript)
    src/
      ...
      workflows/          ← Workflow executors, triggers, and sandbox APIs
        sandbox-api/      ← HTTP & WebSockets worker endpoint surface
          routes/         ← Router endpoints (soverign-llm.ts, connections.ts, etc.)
    ui/                   ← Front-end assets and templates
    roles/                ← System assistant role definitions
  soverign-desktop/       ← Desktop Control Console interface (Electron/JS/CSS)
  ...
  run_soverign.bat        ← Main launcher: starts the daemon on port 3142 and opens Electron UI
  run-standalone.bat      ← Offline server mode launcher
  stop_soverign.bat       ← Utility to find and kill port 3142
```
...
4. **Validation**: Run `node validate.js` in `soverign-desktop/` after every edit to verify integrity.
...
* Profile configurations are stored natively under `%USERPROFILE%\.soverign\config.yaml` or managed by the background quota watchdog.
